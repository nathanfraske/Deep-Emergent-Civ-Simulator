...
- **R-TOM-UPDATE.** How the nested mental model is populated from second-order evidence so it diverges from projection, and what is assumed at the depth bound. Ready: substrate is the resolved evidence engine (R-EVIDENCE) applied recursively to whether a target believes a thing. Couples to Parts 9, 21, 25, 28, 37, 54. Red/green audit done 2026-06-29 (`audits/tom-update-redteam-2026-06-29.md`): reachable now, conditioned on six fixed-Rust type additions (the SecondOrderEvidence enum disjoint from the closed Evidence enum, the restricted nested-store write API, the access-versus-fact distinction, the relational shared-but-not-common common ground, the depth-bound no-projection-of-an-access-fed-frame rule, and the (listener, modeled-subject) conserved second-order projection), the reserved-value hard constraints, and the test battery as the acceptance gate. Eleven reserved decisions surfaced. Awaiting owner sign-off to resolve.
