...
- **M4: emergent dialogue (implement R-CONVERSE) and the loop.** The move and force registries, the content gate, move-as-event, the conversation query, speak-as-intent, the response loop, grounding as said-evidence (regression-tested first), repair, the LOD gate, the four determinism pins, and the two Steering Audit invariants. Stand up the gating factors so they are visible and controllable: co-location and channel reach (can a being speak, and to whom), felicity conditions (does a move land or misfire, off role/trust/value-distance/channel state, gating the act not weighting it), the level-of-detail significance gate with its promotion triggers and demotion hysteresis (move-by-move vs one-pass vs aggregate), the per-tick dialogue budget, the speaking-versus-acting arbitration, and the content gate plus the two invariants. Ship a narrated conversation scene (a sibling of the dawn-band example) that shows each gate firing, in the two layers (canonical gist plus non-authoritative flavor), replaying bit for bit. Defer the deliberation, persuasion, and negotiation couplings.
