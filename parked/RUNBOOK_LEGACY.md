...
STALE BLOCKAGE EDITION (owner ruling, effective 2026-07-17), the optimistic twin of the same class. Where the premise line catches a builder reaching for a shelf that is empty, this catches a note that says a shelf is empty when the thing is already on it. A `BLOCKED` or `pending` or `gated` or `unbuilt` marker the code has since outrun reads to every later reader as "not covered", so it hides finished work exactly as a false "already built" hides missing work, and it is the more insidious of the two because a pessimistic label attracts no scrutiny. So these four words JOIN THE CLAIMS-AUDIT GREP CLASSES: a claims sweep greps for `blocked`, `pending`, `gated`, and `unbuilt` alongside the win-words, checks each hit against the code it describes, and corrects in place with a dated line any marker the code contradicts, never leaving it to rot. The grep EXPLORES; the code DECIDES (section 6). THE FOUNDING CASE: `docs/working/GEOTHERM_ARC_SCOPE.md` carried "THE ROW IS BLOCKED and does not ship until Byerlee 1978 resolves a source conflict" long after `rock_friction_law` shipped as canonical (Byerlee 1978, values fetch-confirmed, twice verified by blind panels). The row was not blocked; the note was. Its dated correction records that the row shipped and preserves the part that stayed true, that the low-stress regime owed roughness and cohesion bands, now itself built as a reserved field (`low_stress_band`, commit `13acaf6`). A stale blockage often carries a live remainder, so correcting the tense must keep the remainder.
...
## 13. Derive is not render, and a sweep validates a mechanism while its number stays un-eyeballed (owner-ratified, 2026-07-17)

Two distinctions the mid-band arc proved executable, ratified because they generalize past it.

DERIVE IS NOT RENDER. A mechanism that DERIVES a quantity has not RENDERED it: the value can be computed on demand while nothing yet shows it to a viewer, and those are two levels that must not be conflated in either direction. The `T_e` assembler derives a silicate elastic thickness today, but the viewer draws no depth-resolved geotherm, so the path is DORMANT and "the payoff is available now" is true only at the derivation level. This is the exact conflation the stale blockage (section 10) commits in reverse: there a note calls a shipped mechanism blocked, here a claim calls a dormant derivation rendered. Same two levels, opposite errors. The held line: say DERIVES when the value can be computed, say RENDERS only when a consumer reads it, and never let one borrow the other's credit.

A SWEEP VALIDATES THE MACHINE; THE NUMBER STAYS UN-EYEBALLED. The way to trust a derivation without trusting its output is to prove the MECHANISM across a hostile sweep of inputs while blessing no single number it emits. The assembler was swept across cold cratonic, thin young, Mars-gravity, Moon-gravity, and cooler-mantle columns and derived a `T_e` in every case, refusing only the material with no creep row: that validates the machine (it derives where it should and refuses where it must) and leaves every specific value un-eyeballed for the calibration ledger (section 12) to judge against observation. The mechanism-versus-instance line is the whole reason a derivation can be adversarially audited before it has a single trusted output. It is the complement of section 9: there a mechanism may not calibrate against the value it retires; here a mechanism earns trust from its BEHAVIOUR across a sweep, never from a value matching.

---

## 14. The measured-versus-derived question exists only at the floor; above it, derive is the vertical (owner-ruled, 2026-07-17, SUPERSEDES the granite ruling)

A ruling earned by a wrong-altitude error the owner named precisely, and it corrects a standing ladder ruling rather than adding one. The GRANITE RULING held that a MEASURED rock modulus outranks a derived one, argued from the ladder precedents (Hofmeister, Hirth and Kohlstedt, Hess), and every precedent was true. But every one of those anchors sits AT THE FLOOR, the level where the universe stops varying: a mineral lattice, a single-phase flow law, an element's enthalpy. The granite ruling applied floor-level reasoning one level UP, to rock, which VARIES PER WORLD. Rationale transfer, right mechanism, wrong altitude.

THE DOCTRINE, stated flat. THE LADDER'S MEASURED-VERSUS-ESTIMATED QUESTION EXISTS ONLY AT THE FLOOR. Above the floor, derive is NOT a rung, it is THE VERTICAL. Cited rows feed a derivation FROM BELOW (a mineral's measured `K` and `G` are the aggregation's input); Earth's aggregates referee it FROM OUTSIDE (a handbook granite is a hindcast row on the output side); and NOTHING CITED EVER SUBSTITUTES FOR A DERIVATION STEP IN BETWEEN. A cited aggregate on the INPUT side of a derivation is the defect, findable by grep on the rock's name.

WHY THE FITTING-TRAP ARGUMENT SURVIVES INTACT, because it was always about the floor. A floor-level datum CANNOT fit an outcome, because it was measured in ignorance of every outcome (a mineral's stiffness knows nothing of the rock it will sit in, still less the world). An AGGREGATE datum is outcome-shaped, so it belongs on the OUTPUT side as a referee, never the input side as a substitute. The trap the granite ruling meant to avoid is real; it lives one level down from where the ruling placed it.

THE SUPERSESSION IS NARROW. Every other standing ladder ruling was re-checked against this and all survive, because their anchors are all floor-level (a re-derived published constant, section 11; a mechanism's un-eyeballed number, section 13). The granite gate was the one whose anchor was not, and it alone is superseded. The consequence is immediate: a world's rock moduli are DERIVED by aggregating its own census of cited MINERAL moduli, never cited as a rock, and the relief map becomes stiffness made visible, an iron-rich crust flexing differently from a feldspathic one because its own minerals say so, not because a handbook was consulted.
