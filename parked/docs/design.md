...
The rule for contributors: if a value can influence canonical state, it is `Fixed` or an integer. No `f32` or `f64` crosses into authoritative components. The two rounding directions above are the pinned canonical convention: the multiply floors (the arithmetic shift) and the divide truncates toward zero, an asymmetry the owner preserves as the oracle defines it, which the GPU reproduces bit for bit (Section 5.4, record 62.23).
...
The order-sensitive and unbounded keying questions are resolved as a canonical draw-keying schema (R-RNG-COORD). The fold primitive is unchanged: a stream is a pure function of an ordered coordinate list folded with the SplitMix64 finalizer, so a draw never depends on allocation or thread order. Over it sits one fixed schema that every draw in the engine uses. A draw coordinate is the ordered tuple of a region, a primary locus, a secondary locus, the tick, a phase, and a slot: the region and the two loci are the entities or places the draw concerns, the tick is the canonical clock so the same entity in the same phase draws differently on different ticks (the coordinate the bare per-entity key omitted, which Parts 9, 16, 33, and 54 assume), the phase is drawn from a registry assigned once rather than as a magic number per call site, and the slot namespaces distinct draw sites within one phase so two of them do not collide on counter zero. A coordinate that does not apply to a draw folds as an absent sentinel rather than a present zero, the degrade rule, so a missing secondary locus never aliases secondary locus zero. The per-draw counter remains the within-stream ordinal. The keying stays integer and counter-based.

> Decided and reserved. The mechanism is settled and signed off. The canonical draw-keying schema folds a uniform coordinate (region, primary locus, secondary locus, tick, phase, slot) over the existing order-sensitive primitive, with a registered phase set assigned once, a per-phase slot that separates draw sites so they do not collide on counter zero, an absent-coordinate sentinel as the degrade rule, and the tick always present. The mechanism is fixed Rust, and the phase and slot registry is engine mechanics, the RNG-core exemption Principle 11 grants, so it is code rather than data. Nothing here is a reserved owner value: the phase numbers are arbitrary but stable engine constants folded into canonical streams, not calibrations. The honest limits stand: the schema guarantees draw-site separation and tick presence, but it cannot itself guarantee that a locus is a camera-independent canonical id rather than an allocation-order one, which the id-minting discipline (R-CMD-ORDER, Part 54) must hold; and the sibling reduction-order item (R-REDUCE-ORDER) still pins the order of any non-associative canonical combine that consumes these draws. Implemented in `crates/core` as the `DrawKey` and `Phase` schema and adopted at every draw site in the simulation tick. Couples to Parts 9, 11, 16, 33, 54.
...
Which fields can be canonical on the GPU is decided empirically, not assumed: the Stage 0 spike confirms that each candidate kernel produces bit-identical output across the CUDA, Vulkan, and Metal backends, and any operation that diverges or is too costly in fixed-point falls back to the CPU, which is still deterministic, or stays a non-authoritative quantized field. The pins these two modes rest on are settled and built (R-GPU-CANON-PIN, record 62.23): the canonical multiply and divide, the transcendental algorithms, and the noise and eikonal rulings are fixed in Section 5.4, and the multiply, divide, and diffusion kernels are built in `crates/gpu` on CubeCL and proven bit-identical to the CPU oracle on the reference GPU, with the multi-vendor Stage 0 run the standing residual.
...
> Needs research, item R-CANON-WALK in the research backlog. The state hash above requires a `StableId`-sorted walk, but several canonical containers are hash maps with no ordered iteration: the registry locations (Part 2), the belief stores (Part 9), the market price, supply, and demand tables (Part 19), the event provenance index (Part 7), and the pool sets (the culture unit maps of Part 55 are now the id-ordered `MeasurementSystem`, resolved under R-UNITS-PIN, record 62.25). No current path walks the remaining ones order-sensitively, so there is no live divergence, but nothing structurally prevents one. A session should specify a canonical-walk discipline: a `StableId`-sorted iteration interface on the registry and the log, a requirement that every canonical container is a sorted structure or carries an ordered accessor, an ordering on the id newtypes, and a single canonical-walk helper so the sorted order is the only path a hash is built over. Determinism and data-driven constraints hold. Couples to Parts 2, 7, 9, 19, 55. Flagged, not changed.

> Needs research, item R-HARNESS-COVER in the research backlog. The harness above must exercise the paths that can break, rather than a pure per-entity accumulation that is order-independent by construction and so cannot catch a command-ordering, gossip-partition, tier-transition, or iteration-order regression. A session should extend it to build the real tick phases, the command buffers, the gossip speaker partition, and structural mutation, to compute a `StableId`-sorted state hash over the two-tier world, and to assert bit-identity across a thread-count sweep and a save, load, and replay cycle, with a near-overflow associativity case among the inputs. The harness stays the standing continuous-integration contract of this section. Couples to Parts 4, 7, 9, 11, 54. Flagged, not changed.
...
> Needs research, item R-CMD-ORDER in the research backlog. `command_sort_key` is named here as `StableId`-based and stable, but `Promote` and `EmitEvent` carry no `StableId`, so the key is not total, and a stable sort then breaks ties by per-worker concatenation order, which moves with the thread count. `Spawn` also carries an id minted during the parallel stage from a mutable registry, so the id depends on which worker ran first, and `EmitEvent` as a command sits against the rule that the event log is written only in the single-threaded events phase. A session should define a total command order (tick, then primary `StableId`, then kind, then an emission ordinal), mint spawn ids at the single-threaded apply barrier in that order, and reconcile event emission so an `EventId` is a deterministic function of canonical content rather than of scheduling. Determinism is the whole point of this phase. Couples to Parts 2, 7. Flagged, not changed.
...
The pins are settled (record 62.23). One canonical Q32.32 multiply and one divide are fixed as sign-magnitude limb emulations over a confined shader op set (u32 wrapping add and subtract, a u16-by-u16-to-u32 product, bitwise operators, shifts by a compile-time constant below thirty-two, comparison, and a branchless select), with no native sixty-four-bit type, no hardware divide, and no float: the multiply keeps bits thirty-two through ninety-six of the exact signed one-hundred-twenty-eight-bit product and floors, the divide is a ninety-six-step restoring long division that truncates toward zero, and each reproduces the `crates/core` `Fixed` oracle bit for bit by the unique-result argument (an exact integer result, rounded once at the pinned step, wrapped once at the narrowing). The transcendentals are pinned as one algorithm apiece, an exact integer square root, range-reduced series for the exponential and the logarithm, and CORDIC for the trigonometric and inverse-trigonometric family, with a real power as the exponential of the product with the logarithm, each carrying one reserved precision target rather than a fabricated iteration count and each built as the CPU oracle sibling to the square root. The worldgen noise (Part 12) is canonical fixed-point, its per-cell sequence already meeting the discipline; the eikonal flow-field (Part 13) is canonical under a monotone-min update run to its fixed point, or a quarantined non-authoritative quantized field otherwise. The Stage 0 gate covers the launcher's selectable runtimes (CUDA, wgpu lowering to Vulkan, Metal, and DX12, ROCm where present, the CubeCL CPU backend as an oracle, and any software adapter), and the multiply-floors and divide-truncates asymmetry is preserved as the oracle defines it. The pin is built: `crates/gpu` carries the multiply, divide, and diffusion kernels as CubeCL `#[cube]` functions, proven bit-identical to the oracle on the reference GPU (the multiply and divide over every corner pair and a million random cases each, the diffusion field over the whole grid and invariant to workgroup and tile size), with the transcendental GPU kernels and the multi-vendor Stage 0 run the named remaining build.

> Decided and reserved. The mechanism is settled and signed off, and on your condition it is general and per-field differentiable. One canonical Q32.32 multiply (sign-magnitude, u16-partial limb product, middle-64 extraction, floor) and one canonical divide (sign-magnitude, 96-step restoring long division, truncate toward zero) reproduce the `crates/core` oracle bit for bit, and one algorithm per transcendental (integer isqrt, range-reduced series for the exponential and logarithm, CORDIC for the trig family, real power as the exponential of the log product) is pinned with its rounding fixed; every canonical kernel is confined to an op set with identical semantics on WGSL, SPIR-V, Metal, and DX12, and bit-identity follows from the unique-result argument. The mechanism is fixed Rust; the precision targets, the per-field variant and window selections, and the backend enumeration are data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the exponential, logarithm, and trigonometric precision targets (the first dropped series or CORDIC term falls below one Q32.32 unit); the per-field checked-versus-wrapping variant and the transcendental argument window (the field's declared range against the Q32.32 bound and the roughly nineteen-order window, coupling to R-UNITS-PIN); the divide-by-zero policy per field (whether the producing field can be zero); the eikonal convergence criterion (the map diameter against the per-step propagation); and the full backend and adapter set the Stage 0 gate covers (the launcher's selectable runtimes). The reserved list is in the audit log. The honest limits stand: cross-vendor bit-identity of compiled kernels is proven by the Stage 0 gate rather than on faith, and only the CUDA backend is proven so far; the field kernel uses native sixty-four-bit arithmetic where the backend has it, so its cross-vendor identity is a Stage 0 matter for each further backend rather than covered by the u32-limb argument; the Q32.32 representability windows require some laws to be reformulated (coupling to R-UNITS-PIN); the mul-floors and div-truncates asymmetry is preserved by your call; and the general canonical multiply-folding reduction rides R-REDUCE-ORDER. Built in `crates/gpu` on CubeCL, record 62.23. Couples to Parts 3, 12, 13, 60.
...
> Needs research, item R-SAVE-SCHEMA in the research backlog. `WorldSnapshot` carries no schema version and no migration hook, while rkyv zero-copy decoding is tied to an exact memory layout, so a save written by one build can become unloadable or mis-decoded by the next, the catastrophe Part 58 names. A session should add an explicit schema-version discriminant and a migration table to the snapshot, specify that every id high-water mark (the registry next-id and the pool and handle counters) is persisted and restored as an authoritative value independent of the live-entity count, so a reload never reuses an id, and pin the on-disk endianness and a layout-version rule so a snapshot moved between machines decodes identically. This is the save half of the Part 58 foundation. Couples to Parts 2, 40, 58. Flagged, not changed.

> Inconsistency note (Inconsistency 6 in the audit log). Section 7.2 fetches an event by using its id as a dense storage index, while this section truncates the fine-grained events of demoted entities. Once compaction removes events, the dense-index assumption breaks and an id no longer maps to its slot. The two are reconciled before compaction is built (a stable id-to-slot map, tombstoned slots, or an id that is not the storage index), not settled here. Flagged, not changed.
...
The repertoire above is the interface of the sentient, deliberative tier: for a sentient being it is the fast shape of a per-tick decision, its curves keyed off the race's own trait axes. It is not, however, how behaviour arises for a non-sentient creature or the pre-sentient substrate. A repertoire authored as data is still authored, and a fixed action-and-drive policy chosen from outside the simulation is steering at the level of behaviour, which Principle 9 reserves to physics. For those beings the policy is not written at all but evolved from physics and selection, specified in 8.4 (R-BEHAVIOR-EVOLVE); the evolved controller underlies this deliberative tier rather than replacing it.
...
### 8.4 Evolved behaviour: the homeostatic controller (R-BEHAVIOR-EVOLVE)

For a non-sentient creature and the pre-sentient substrate the policy of 8.1 is not authored but evolved, so that behaviour arises the way anatomy does, from physics and selection. It has five layers, the first three authored physics, the fourth heritable, the fifth the selection the pre-dawn epoch already performs (Part 25).

First, homeostatic physiology. A being carries internal reserves that are the physical bookkeeping of a living body: an energy store depleted by basal metabolism and by exertion and restored by ingesting matter it can digest, a water level, and, for an exotic creature, whatever else a world declares (an arcane charge, a heat store). Each reserve is a Part 15 stock that drains without self-regeneration, keyed by a data-defined homeostatic-axis registry, sibling to the value, semantic, and institution-function substrates. "Thirst" is not a drive in this model; it is the state of low water. When any reserve crosses its physical floor the being dies. Death is the only consequence that matters, and it is physical rather than a score.

Second, affordances. A being's options are the primitive physical operations its morphology permits, a data-defined affordance registry whose gates read the Part 25.14 anatomy categories: moving, if the body bears a locomotion organ (the walking-tree rule, mobility a property of the body and not the kingdom); ingesting, which any body does; and others such as grasp or strike as its parts warrant. What is authored is what a body physically can do, never what it should do; each affordance carries the shape of its parameter (a directional operation is aimed by a heading, a scalar one is not), which is the physics of the operation rather than a behaviour.

Third, sensing. The being perceives within the reach its sense organs and acuity imply (Part 9, the non-omniscient perception the movement layer already needs), reading the matter near it and its edibility, water, terrain passability, other beings, and its own reserves. The percept is a bounded vector of physical readings, the controller's input, and a being senses what its body can sense and no more.

Fourth, the controller, the one heritable layer. The mapping from the percept and the reserves to an affordance and its parameters is a controller whose parameters are a new genome channel, a sibling of the cognition, build, and composition channels (Part 25), expressed per individual by the same expression mechanism, so a being's behaviour is its inheritance the way its size and acuity are. The controller is evaluated in integer fixed-point with a piecewise-linear clamped activation and no randomness, so no float enters canonical state and its output reproduces bit for bit (Principle 3). Its weight matrix connects every percept feature to every output, so an adaptive coupling (a low water reserve to the water-source heading, or ingesting the matter underfoot when a reserve is low) is discovered by selection rather than wired in. The representation is staged, both sharing the plumbing: a linear reaction norm is the substrate and the determinism proof, and a small recurrent network, its topology fixed Rust and its weights the heritable data, graduates it to gate a response on internal state and recent history, the step between them the reserved hidden width and not a rewrite.

Fifth, selection. The controller's alleles are selected by a coefficient that is a consequence of homeostatic survival: a lineage whose controller keeps its bodies alive and reproducing fixes its adaptive controller alleles, and one whose controller lets its bodies starve or stray from water is selected against, through the same aggregate-tier recurrence any other channel is selected by. The fitness is survival and reproduction, which fall out of the physical floors of the first layer; it is not an authored objective and, in particular, not a resemblance to expected behaviour. A controller that keeps a body viable by a strategy no one anticipated scores well; measuring survival, rather than measuring "did it do the expected thing," is what keeps the behaviour emergent. Scoring behaviour over deep time is the crux, since behaviour is a time-extended individual property while the epoch runs pools as allele frequencies: the answer is the significance-and-time allocation the engine already uses for detail (Principle 1, Part 54), a cheap homeostatic-viability proxy for quiet pools and full sampled episodes at the dawn, under fast-changing pressure, and for any observed or promoted lineage.

The evolved controller sits at the low end of the intelligence dial (Part 20), the reflex-and-drive policy of a non-sentient creature. It does not replace the sentient layers above it (the values of Part 21, the axioms of Part 28, the theory of mind of Part 37); it underlies them, the substrate on which, for a sentient race, the deliberative and cultural layers of 8.1 to 8.3 are built.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general and per-race differentiable and data-driven. A being's needs are homeostatic reserves that drain by metabolism and are restored by intake, its options are the affordance primitives its morphology permits, and the mapping from state to primitive is a heritable controller expressed from a new genome channel and selected by homeostatic survival, so behaviour is evolved rather than authored (Principle 9). The mechanism is fixed Rust; the homeostatic-axis registry, the affordance registry, and the controller's weights are data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the homeostatic rates per axis (the reserve capacity relative to body mass, the basal metabolic drain, the exertion coupling, and the death floor, from the Part 20 physiology mapped onto the base tick); the intake yield per bite (the net-nutrition and water-content the resolved edibility floor measures, R-PHYS-BIO, interim as a fraction of capacity until the floor's measure is wired to the located matter); the controller representation and, for the network, its hidden width (the smallest network that represents the conditional foraging the ecology needs, against the per-tick budget; set to 4 as a tuneable lever on a compute-tax exploration, audit 6d); the controller-allele init spread, mutation rate, and mutation step (the mutation scales the epoch already uses for the other channels, adjusted for the larger controller parameter space); and the behavioural-scoring policy (the population size, generation count, and episode length, and the proxy-versus-episode significance allocation of Part 54, a performance bound rather than a realism one, with the proxy's honesty the thing to validate). The reserved list is in the audit log. The honest limits stand: whether adaptive behaviour evolves in feasible compute over a whole world's deep time is an empirical result to be shown at scale, not proven on paper; the proxy's honesty (that scoring well predicts surviving in the world) is the central risk and must be validated by cross-checking proxy-selected lineages against full episodes; the pure-frequency deep-time tier expresses a controller from allele frequencies, which couples to the quantitative breeding-value tier the genome still defers (25.10) and to the open temporal level of detail (Part 32), so this work builds and proves the individual and sampled-episode tier and wires the coefficient into the pool recurrence, and scopes the pure-frequency tier as that reserved coupling. The reaction norm cannot gate a response on internal state through a product (the recurrent network lifts that ceiling), and the couplings to Part 32 and R-TIER-CONSIST (Part 54) are real, the behavioural-scoring proxy being a temporal-LOD device that must stay consistent with the full-episode tier.

> Needs research, item R-EVOLVE-STEER in the research backlog. The controller of this section is selected by homeostatic survival scored in a sampled episode, and the episode's environment (where the being's water, food, and hazards are placed, and the episode geometry) is itself an authored input to selection. An asymmetry in that environment imprints a behaviour the world did not: a scorer that placed a being's water in one fixed direction was found to select a fixed-heading overfit rather than a percept-following forager, the environment authoring a direction where Principle 9 reserves behaviour to the being's own physics. This is the steering audit applied to the selection signal rather than to the controller, and it is distinct from the reserved proxy-honesty check above (which asks whether the proxy predicts world-viability) in that it asks whether the proxy is behaviour-neutral in the first place. A session should establish how the scoring environment for evolved behaviour is constrained so it authors no behaviour: an isotropy-or-symmetry invariant that certifies an episode is behaviour-neutral (a sibling of the modality-swap invariant of record 62.13 and the value-metric basis-independence test of Part 21), and how that invariant carries into the deferred pure-frequency pool tier, where selection must score against the real world or a proven-symmetric proxy and never a fixed-geometry fixture. The determinism and data-driven constraints hold: the scoring draws are integer and seeded, and the environment set is data. Couples to R-BEHAVIOR-EVOLVE (resolved, this section), the temporal level of detail (Part 32) and R-TIER-CONSIST (Part 54) the proxy tier rides, and the Steering Audit (Part 41). A first instance is fixed in the development-time proxy `crates/sim/src/evolve.rs`, whose scorer now places water over a balanced, seed-rotated direction set. Flagged, not changed.
...
The promoted-tier generalisation of this loop is modelled dialogue (R-CONVERSE), and it is the same mechanism at higher fidelity rather than a second one. The one-pass loop above transmits a belief from a speaker to a listener and records the outcome; at the promoted and significant tier the engine instead records the communicative acts that produce that outcome, the assertion, the doubt, the repair, the uptake, as first-class canonical events, and the one-pass integration becomes the aggregate fallback for everyone the move log does not cover. A dialogue move is a canonical event (Part 7): a force drawn from a data registry, a speaker, one or more addressees, a content reference, an optional reference to the move it answers, and the channel it travels (Part 33.3). Its content is never authored text but a handle into state the engine already holds: a belief facet (9.2), an inference frame the speaker holds open (9.13), a concept-and-form pairing (33.1, 33.9), a conditional intent the hearer will model (Part 37), or a prior move it responds to. A conversation is then an emergent query over co-referenced move events linked by participants, topic, and an in-reply-to chain inside a time window, the way an artifact's saga is a query over its tagged events (Principle 5), rather than a stored object or an authored script (Principle 8).

The move kinds, assertion, question, proposal, acceptance, refusal, command, promise, threat, greeting, and the rest, are recognised over a data registry, never a closed enum, the same hardening applied to the trace-kind registry (9.9), the access-channel registry (Part 37, Part 40), the institution-function substrate (Part 36), the value substrate (Part 21), and the semantic substrate (33.1). A move kind is a `MoveKindDef`: the primitive effects its force fires, the responses it conventionally expects (the adjacency a reply points back along), whether the deception frame judges its sincerity, the felicity conditions under which it lands, and a deterministic gloss handle. The force a move carries bottoms out at a small etic floor of primitive effects, themselves a registry and not a hardcoded list (a starting menu, not a fixed fact, as the institution-function and interface substrates are): tell a told-evidence facet (9.5), raise an inquiry goal (9.13), propose a form (the naming-game coordination move of 33.9), register an uptake of either sign, write a conditional intent into the hearer's model (Part 37), or open a channel of contact. Each primitive is a call into a mechanism the engine already has, so a move adds no new authored behaviour: an assertion is a told-evidence injection, a question seeds the inquiry goal of 9.13 in the hearer, a form proposal is the dawn coordination move, an acceptance is an uptake and a refusal the same with the opposite sign, a promise or threat is a conditional intent the hearer reasons over, and a lie is an insincere assertion the Part 37 sincerity frame judges rather than a separate kind. Which compositions of that floor a culture conventionalises into recognised moves is the emic repertoire, and it can emerge: a recurring effect-bundle a community comes to recognise and respond to as one move is a conventionalised move kind, minted under the same discrimination-and-coordination dynamic that fixes a concept or a word (33.9), from a reserved starter repertoire so a world is legible from the first tick, capped by the same hard round cap so the growth stays deterministic and bounded.

Felicity conditions decide whether a move's force lands at all, never how hard it lands. A `FelicityCond` probes state the world already carries, a role held in an institution (Part 36), a trust band (Part 37), a value-distance band (Part 21), a channel capability (33.3), and a move whose conditions fail misfires: a command from one who holds no commanding role lands as a bare attempt with no deontic force, the Austin felicity condition made structural. Felicity may gate the act; it may never set a graded multiplier on how much an assertion moves a belief or a persuasion moves a value, because that magnitude is the consumer's computation under the unmodified inference engine (9.10), opinion kernel (Part 28), and deception frame (Part 37). What a culture says and how readily emerges from its drives and the world; how well a said thing works on a hearer is physics the kernels already own.

Speaking becomes an intent the decision layer selects, closing the gap this loop left open between the co-location pairing here and the goal-directed interview of inquiry (9.13). A communicative act is an ordinary `ActionDef` (Part 8) whose considerations read the belief and theory-of-mind facts the agent already holds (the salience of a facet it could share, an open question it is motivated to resolve, the modelled trust and value-distance toward a candidate hearer, the gap between what it believes and what it believes the hearer believes) and whose target is an addressee and a content reference. Co-location, or the reach of the chosen channel for a signed or written move, gates whether speaking is possible; utility decides what is worth saying and to whom, so a quiet farmer with nothing pressing and no one worth addressing never selects a speak action and costs nothing. The directed interview of 9.13 becomes one move kind on this promoted path, and the co-location loop above remains the aggregate-tier fallback, so the two stop being two rules for one behaviour. Whether a communicative intent competes with a physical action in one utility pass or runs as a parallel channel, so an agent can act and speak in one tick as people walk and talk, is a reserved decision for the owner, with the parallel channel recommended on the basis of realism and the existing deterministic within-tick two-stream precedent (the code-blend of 33.3), gated so its cost falls only on the promoted few.

A response is itself a move, the addressee of one move becoming the speaker of the next, and the listener-side update is the single-utterance procedure of Part 37 applied to a move that happens to be a reply: the second-order update from what was said, the sincerity frame judged against a phase-frozen snapshot, then the first-order world update gated by the deception verdict. Both parties co-update: on receiving an uptake, a doubt, or a counter-assertion, the original speaker runs the same Part 37 update on its model of the hearer, the response being access evidence about whether the hearer believed the claim, admitted under the unchanged anti-projection rule (only access evidence about the target, never the speaker's own world belief). Uptake, doubt, counter-assertion, and refusal are recognised moves composing from these primitives, so the conversation is a sequence of three-frame updates with no new machinery. Grounding and repair compose the same way, and the grounding case carries the load-bearing constraint this resolution turns on: a thing two parties merely say and accept in conversation is said evidence (the deception channel of Part 37) and must never enter the co-witnessed common ground that Part 37 walls off for physical co-witnessing alone, or two colluding speakers could manufacture a shared-belief artefact that a third party's nested model reads. So an acceptance updates the first-order store (gated by the deception verdict and source weight) and the second-order model of the speaker, and nothing more; the convergence that conversation analysis calls grounding is each party's second-order model of the other approaching agreement through ordinary access evidence, and a "we both accepted this in talk" fact is a derived query over the move log, never a stored prior on a nested store, with said-derived facets tagged so a deception probe tells them from co-witnessed ones. Repair is a move that re-asserts or clarifies a prior move within the language-distance error budget (33.7), triggered when the substrate distance between what the speaker meant and what the hearer integrated exceeds a reserved threshold; a re-assertion re-rolls the 33.7 budget rather than zeroing it, so a distant pair can fail to repair, which is the intended outcome.

The fidelity at which all of this runs is allocated by significance, not by the camera (Principle 1, Part 54). Full move-by-move dialogue, with the response loop and the per-move theory-of-mind co-update, runs only at the promoted and significant tier, exactly where individual belief stores (9.7) and nested models (Part 37) already exist; below it a promoted pair keeps the one-pass outcome of this loop, and the quiet majority keep the aggregate diffusion of 9.14, with no individual move at all. A pair or a forming conversation promotes when an active scheme runs through it (the schemer-and-mark hold of Part 37), a significant figure takes part, or a consequential exchange is under way, where consequential is a function of canonical outcome (a belief crossing a commit threshold, an institution norm changing) and never of how interesting the talk is to watch. A per-tick dialogue budget caps how many conversations run move-by-move, and over budget the lowest-significance live conversations degrade to the one-pass outcome in canonical id order; on demotion the divergent per-move texture is lost (it has no sufficient statistic, the limit Part 37 already states), while the committed beliefs fold back by the conserved projection (Part 54), which is why the parties to a live load-bearing conversation stay promoted while it runs. The move salience that decides this gate affects only the processing fidelity of a move, never the magnitude of the force it delivers.

The canonical record of a conversation is the move sequence and its deterministic consequences and nothing more, which fixes how far the non-authoritative interpretation layer may go. The deterministic gist of 33.2 renders every move's content with no model in the loop, so a legible account of any conversation exists unaided; the optional external interpretation layer may elaborate that real sequence of real moves into dialogue prose in the culture's voice, carried as the view-only type 33.2 already specifies that no canonical function accepts, so it can invent no move, no belief change, and no motive, and cannot backfeed. This is the payoff the move log buys: under the one-pass outcome there is no exchange to render and any dialogue prose is wholesale invention, while under a move log the flavour layer dresses real moves with real consequences, so the elaboration is faithful rather than confabulated, the canonical gist carrying the substance and the flavour layer carrying only voice and texture.

A move is a candidate row in the event log expressible under either resolution of the open event-schema question (R-EVENT): its force is a data-registry identifier in the payload rather than a new closed `EventKind` variant, so it is a registered schema under the registry option and a structurally-recognised communicative event under the generic-payload option, and the conversation query keys off references (participants, topic, in-reply-to, tick) that survive either choice, so this resolution composes with R-EVENT and does not decide it. Because an in-reply-to reference is a stored event id and demoted-entity events are compacted (7.3), a reference into a compacted move resolves deterministically to that move's conversation-summary record rather than dangling, which is the obligation this places on the reconciliation of Inconsistency 6.

Determinism is pinned at this site and, where it extends the engine's cross-cutting invariants, it is added to the open items those invariants belong to rather than assumed from them. Moves are appended in the single-threaded Events phase, selected in the Cognition pass over the frozen buffer, applied in canonical (speaker id, same-tick move ordinal) order, with their belief and theory-of-mind effects landing in the Gossip phase, and all canonical dialogue state integer and fixed-point with no float and no hashed walk. Four pins are stated here because they are specific to dialogue: each of the move's distinct stochastic draws (which hearer, which kind on a tie, the 33.7 error-budget roll, the heard-perception roll) carries its own draw-site slot in the counter key, with move-scoped draws keyed without the addressee and addressee-scoped draws keyed with it, and one move ordinal per move with addressees expanded in stable-id order; the Gossip partition is the union of the constraints, no two speakers in a batch sharing a listener for the first-order write or a (listener, modelled-subject) pair for the nested write, since one three-frame move writes the hearer's first-order store, the hearer's model of the speaker, and possibly a third party's model; the deception-verdict snapshot is the phase-frozen state, uniform across all of a tick's moves, so no move's verdict sees another same-tick move's write; and a minted move kind is allocated at the single-threaded barrier in culture-id then genesis-ordinal order, the device 33.4 already uses. These pins compose with, and extend the scope of, the open determinism items the dialogue path leans on: the per-draw-site counter namespacing of R-RNG-COORD, the sorted-walk accessor of R-CANON-WALK over the provenance index the conversation query reads, the total command order of R-CMD-ORDER over the move events, and the non-associative combine order of R-REDUCE-ORDER over conflicting writes, so the dialogue determinism closes when those close and not before.

The deeper consumers of dialogue are named and deferred, not built. Multi-party deliberation can feed a collective decision through the emergent governing institution and its norm grammar (Part 36, R-INST), with proposal, objection, and assent as moves, but the decision procedure itself must crystallise rather than be templated, so it waits on the institution-crystallisation tuning. Persuasion over a value or an axiom is a told-evidence move whose effect is the unmodified opinion kernel already adopted (Part 28: stubbornness, the confidence band, source weight, and dogmatism), so its efficacy is a function of existing reserved parameters and carries no authored persuasion curve, but the per-individual move-driven path and its level-of-detail gate wait on that kernel being run at the agent tier. Negotiation, alliance, and terms of peace are offer, counter-offer, and acceptance moves reasoned from the modelled intentions of Part 37, with the bargaining results of Nash and Rubinstein as the convergence the emergent exchange approximates rather than a protocol to run, and this is the deepest-deferred, because its offer and concession policy lives in the utility AI (Part 8) and the war-cost computation (Part 24), the decision-layer machinery least built. The Steering Audit gains two invariants for this layer, the analogues of its basis-independence and modality-swap tests (Part 41): a content-blindness-of-force invariant, that paired worlds differing only by a permutation of the force decomposition, the move-kind labels, and the felicity encodings must show invariant transmission fidelity, persuasion success, deception detection, and coordination convergence, so a move's effect is a pure function of the affordance it realises and never of its label; and a channel-swap invariant, that an equal-capacity change of the channel a conversation runs over leaves dialogue outcomes invariant. The content gate classifies every move kind and felicity condition at load as an affordance or an outcome and refuses any that sets a graded persuasion or fidelity weight.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general, per-race differentiable, and data-driven. Modelled dialogue is the promoted-tier refinement of the gossip loop: communicative acts are first-class canonical events over a data-defined dialogue-move registry, a conversation is an emergent query over co-referenced move events rather than an authored script, speaking is a utility-AI intent, the listener-side update is the Part 37 single-utterance procedure applied per move, grounding is said-evidence into the existing first-order and second-order channels with no new common-ground prior, and the non-authoritative interpretation layer elaborates the real move sequence without entering canon. The mechanism is fixed Rust; the move kinds and their force compositions, the felicity conditions, the communicative drives, and a culture's emergent repertoire are data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the asserted-claim told-weight (basis: the gossip told-weight already set, kept below a witnessed observation so a contrary assertion is out-ranked); the uptake strength step (basis: the reinforcement-on-retelling rate of 9.3); the raised-inquiry motive salience (basis: the inquiry motive scale of 9.13); the contact trust touch (basis: the trust scale of Part 37, far below the betrayal penalty); the felicity bands (basis: the existing role, trust, and value-distance scales, read not weighted); the conversation contiguity window (basis: the turn-transition timing of conversation analysis mapped to the tick rate, a recogniser boundary); the move-logging salience threshold (basis: the salience-as-familiarity signal of 9.5 against the per-tick move budget); the repair-trigger substrate distance (basis: the language-distance scale of 33.5 and the error budget of 33.7); the self-repair selection bias (basis: the conversation-analytic preference for self-correction); the grounding criterion sufficiency (basis: the principle of least collaborative effort against the 9.5 salience signal); the communicative-drive rise and satisfy parameters (basis: the Part 8 drive shape and the 9.13 motives); the per-tick dialogue budget and the conversation promotion thresholds and demotion hysteresis (basis: the Part 37 theory-of-mind budget and schemer-hold, a performance bound); and the speaking-versus-acting arbitration (basis: a reserved owner decision between one utility pass and a parallel speech channel, the parallel channel recommended). The reserved list is in the audit log. The honest limits stand: the divergent per-move texture is conservation-not-identity across demotion and is lost unless the parties stay promoted; the dialogue determinism is pinned at its site but closes only when the open determinism cluster it extends closes (R-RNG-COORD, R-CANON-WALK, R-CMD-ORDER, R-REDUCE-ORDER), and it places an obligation on the reconciliation of Inconsistency 6 that an in-reply-to into a compacted move resolve to that move's summary; and the deeper couplings (deliberation to Part 36, persuasion to Parts 21 and 28, negotiation to Parts 37, 24, and 8) are scoped and reserved, with negotiation the deepest because its policy lives in the still-unbuilt decision layer.
...
Canonical promotion is driven by in-world significance and the seed, never by the camera (Part 54, Principle 10). When the viewer zooms into a quiet region (Part 14), the engine may instantiate a non-authoritative detailed elaboration of it for viewing, a sample of members rendered at high detail, which is discarded when the camera leaves and never writes back to canonical state. Watching a region therefore shows its detail without changing its fate.
...
Zooming in is the act that requests a detailed view of a region, not the act that promotes it in canonical truth. Canonical fidelity is driven by in-world significance and the seed (Part 54, Principle 10): a region the engine already runs in detail renders directly, and a quiet region the camera descends into is shown through a non-authoritative elaboration instantiated for viewing and discarded when the camera leaves, which never writes back (Part 11). The viewer's attention allocates rendering and elaboration effort, not canonical compute, which is what lets the zoomable view of Principle 6 coexist with the reproducibility of Principle 3.
...
> Needs research, item R-SOURCE-VECTOR in the research backlog. `GrowthInput` here, the fauna `FoodSource` (Part 17), and the material property set (Part 19) are closed structures the biology floor's source-vector generalization will subsume: when the eater half is lifted from a composition vector of fresh tissue to a draw over a source vector (R-PHYS-BIO, Part 41), what an organism draws on becomes a point over the physics-substrate axis registry (Part 58) rather than an enum, so a field-and-gradient feeder (photovore, thermovore, lithotroph, mana-feeder) is first-class rather than a `Resource` escape hatch. A session should lift the supply-axis unit off mass-fraction-of-fresh-tissue per source kind and route these draws through the axis registry, keeping the determinism and the demand-closure of the floor. The mechanism is fixed Rust; the source kinds and axes are data. Couples to Parts 16, 17, 19, 41, 58. Flagged, not changed.
...
The `FoodSource` enum is the fauna sibling of the Part 16 `GrowthInput` flag, subsumed by the same source-vector generalization (item R-SOURCE-VECTOR, R-PHYS-BIO in Part 41): a consumer's source becomes a point over the physics-substrate axis registry (Part 58) rather than a closed enum, so what it eats is measured composition rather than a category.
...
Seasons are the climate fields cycling annually, with amplitude set by latitude and by the world's axial tilt through the insolation substrate (18.1), driving plant growth cycles (Part 16), animal migration (Part 17), and the agricultural calendar (Part 19). Fantasy weather rides the same field machinery as additional layers: arcane storms over a magic field, blights, an unnatural darkness or perpetual winter tied to a historical event such as a great evil's rise, and elemental phenomena. Over deep time the climate baseline itself drifts, warm ages and cold ages and the occasional volcanic winter (Part 26) shifting biomes across the whole map and forcing the mass migrations and collapses that punctuate a world's history; the orbital cause of that drift (the Milankovitch secular variation of the tilt and orbit) is the celestial substrate's flagged follow-on (R-CELESTIAL-SECULAR, 18.1), derived from the world's planetary system rather than authored.

### 18.1 The celestial insolation substrate (the sky that drives the climate)

The climate baseline above reads a per-cell insolation, and that insolation is not authored: it is the ground-level irradiance a rotating, tilted world on an elliptical orbit receives from its star, stood up in causal order from the star outward (the genesis-forward build). The substrate is the world's DATA sky, a star-list plus the orbital geometry, and a fixed-Rust law that turns the two into the daylit signal the productivity path reads and the physical watts the surface-temperature baseline reads. The reference world is a zero-obliquity single-star circle; the Mirror world is Earth's real sun, tilt, and orbit; an alien world is a different data row, which is the admit-the-alien contract holding at the top of the physical stack.

The stellar flux is DERIVED from the stellar source, not carried as a number. A star's bolometric luminosity follows the main-sequence mass-luminosity relation `L = L_sun * (M_star / M_sun)^exponent`, anchored so that a solar-mass star radiates exactly the solar luminosity; the flux delivered to a world at orbital distance `d` is `L / (4 * pi * d^2)`, the inverse-square spreading of that luminosity over the sphere at the world's distance. For Mirror (a solar-mass star at one astronomical unit) this reconstructs Earth's real total solar irradiance, about 1361 W/m^2, from the cited anchors rather than from a hardcoded constant. The computation crosses a representability gap: the solar luminosity (about 3.828e26 W) and the square of the orbital distance in metres overflow the canonical Q32.32 grid by many orders of magnitude, so the derivation runs in an exact arbitrary-precision rational arithmetic (with pi computed to reference precision by a Machin series) and rounds once, at the end, to the fixed-point solar constant. The per-world inputs (the star's mass as a fraction of the sun, the world's orbital distance in astronomical units, the mass-luminosity exponent) are scenario-set world data; the physical anchors (the solar luminosity, the solar mass, the astronomical unit) are cited reference constants.

On top of the flux scale rides the daily and seasonal geometry, all of it derived from the world's own rotation and orbit. The diurnal phase is the world's sidereal spin, and the solar (synodic) day the sun-angle law reads is that spin minus the orbital advance, so a tidally-locked world (rotation equal to orbit) comes out as a permanent day face rather than as a special case. The world's axial tilt (obliquity) sets the declination, `declination = obliquity * sin(2 * pi * orbital_phase)`, so the sub-solar latitude swings north and south across the year, giving seasons, midnight sun, polar night, and the mid-latitude summer-winter contrast; a zero-obliquity world reduces to a clean day-night swing with dark poles. The orbit's eccentricity modulates the delivered flux over the year: the distance to the star varies as `d / a = 1 - e * cos(E)`, with the eccentric anomaly `E` solving Kepler's equation `M = E - e * sin(E)` for the mean anomaly by a deterministic fixed-point iteration under a fixed cap, and the flux scales as the inverse square of that distance, `(a / d)^2 = 1 / (1 - e * cos E)^2`. The precession term is the perihelion phase, the orbital phase at which the world is closest to its star: measuring the mean anomaly from perihelion (`M = 2 * pi * (orbital_phase - perihelion_phase)`) decouples the distance minimum from the seasonal reference, so whether the eccentricity brightening falls in a hemisphere's summer or its winter (the seasonal-amplitude asymmetry) is a per-world datum, not a fixed relation. A circular orbit, a zero tilt, and a zero perihelion phase each reduce their term to the identity, so the reference world is the clean baseline and every added geometry is a byte-neutral extension until a world declares the datum.

The whole substrate keys on per-world data so the alien is a data row, never a rewrite: the star-list, the two orbital periods in world-seconds (the day and year, which the tick cadence derives from through the canonical seconds-to-ticks bridge rather than a hardcoded calendar), the obliquity, the eccentricity, the perihelion phase, the Bond albedo, and the atmospheric back-radiation floor are all read, and the reference world sets each to the inert value so an undeclared world is the pre-substrate baseline. Mirror carries Earth's measured values; a Mars, a tidally-locked world, or a binary-star world sets its own.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general and per-race (here per-world) differentiable. The stellar flux derives from the star's mass, distance, and the mass-luminosity relation through the inverse-square law computed in exact rational arithmetic; the day-night, seasonal, distance, and precession geometry derive from the world's rotation period, orbital period, obliquity, eccentricity, and perihelion phase through the sidereal-synodic hour angle, the declination, and a deterministic Kepler solve. The mechanism is fixed Rust; the star-list, the orbital periods, and the five orbital-and-surface data (obliquity, eccentricity, perihelion phase, albedo, back-radiation) are world data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the mass-luminosity exponent (the empirical main-sequence slope, anchored so that luminosity at one solar mass equals the solar luminosity; the reference dev fixture is 3.5); the atmospheric back-radiation floor (the downwelling longwave a night surface still absorbs, whose full derivation from the atmosphere's greenhouse optical depth is the flagged follow-on; Mirror is Earth's real value, an airless world zero); the planetary Bond albedo (the fraction of stellar flux reflected, which lands the equilibrium temperature; Mirror is Earth's measured 0.306, an icy world higher, a dark world near zero); the uniform surface emissivity (interim, the per-material floor read is the follow-on); the representability temperature cap (an engine bound, not a realism one); the obliquity, eccentricity, and perihelion phase (each a measured per-world orbital datum; Mirror is Earth's 23.44 degrees, 0.0167, and the perihelion about 13 days after the December solstice); and the two per-world stellar inputs (the star's mass fraction and the world's orbital distance). The Kepler iteration cap and the pi precision are fixed integer engine-accuracy bounds, not world values, and the solar luminosity, solar mass, and astronomical unit are cited reference constants. The reserved list is in the audit log. The honest limits stand: the surface emissivity and albedo are uniform planetary values, not yet the per-cell per-material floor read; the back-radiation is a reserved floor, not yet the derived greenhouse; and one orbital phase serves both the seasonal declination and the mean anomaly, so the finer equation-of-center for the seasonal angle is a later refinement while the precession offset on the distance term is exact. The deep-time secular drift of these orbital data (the Milankovitch envelope) is flagged below, not authored.

> Needs research, item R-CELESTIAL-SECULAR in the research backlog. The orbital data above (eccentricity, obliquity, the perihelion phase) are carried as a world's current contingency, and the paragraph on deep-time climate drift in Part 18 and the catastrophe punctuations in Part 26 name warm ages, cold ages, and a drifting baseline as outcomes with no physical cause. The settled derive map is explicit that a world's orbital elements are authored contingency but their EVOLUTION must be derived, not authored: eccentricity damps, and obliquity stability is computed from the secular frequency spectrum (the Laplace-Lagrange secular perturbation theory whose eigenfrequencies set the Milankovitch periods, the result Laskar established when he showed a moonless Earth would wander chaotically in tilt while the Moon pins ours). A data-defined harmonic envelope of the Terran periods, whether hardcoded or carried as reserved per-world terms, would author that evolution as an outcome curve and is rejected on the value-authoring line: the periods and amplitudes are specific to the Solar System's planetary configuration, not a floor fact. The derive-correct envelope needs a secular-perturbation substrate the simulation does not yet carry: the other planets' masses and orbits and the world's oblate-figure torque from its star and moons, from which the secular spectrum and the drift rates emerge. A session should build that celestial-mechanics substrate (the many-body secular dynamics plus the figure torque, under the determinism discipline of a fixed iteration cap tested by an integer tolerance) so the Milankovitch envelope emerges rather than being fit, keying every rate on the world's own planetary system so an alien system drifts on its own frequencies. Determinism and data-driven constraints as for the rest of the substrate. Flagged, not changed.
...
The `MaterialProps` named-field set (density, hardness, sharpness, and the rest) is subsumed by the physics-substrate axis registry rather than extended field by field: a material is a vector of points over the registry (Part 58), and the wave-1 mechanical-and-materials floor (R-PHYS-MECH, resolved in Part 41) populates those bulk-property axes, mapping density to the density axis, hardness to an indentation pressure, sharpness to an edge-radius capacity length, and fuel value to the energy axis, with value-density refused as physics and kept the emergent market price below. The full lift of the struct into source-vector points is the open R-SOURCE-VECTOR.
...
### 25.11 The biosphere: generating the living world (R-BIOSPHERE)

A world's flora and fauna are generated, not hand-authored. The generative space is authored (the trait axes and their ranges, the niche and food-web constraints, the biome-fit law) and a species is a sampled point over it, drawn through counter-RNG keyed on the world seed, so the biosphere is part of the world's reproducible identity. The trait axes are one extensible registry, sibling to the value (Part 21), semantic (Part 33), institution-function (Part 36), and physics (Part 58) substrates: the mechanism is fixed Rust, the membership (which axes exist, per plant, animal, or shared) is data that grows by demand-closure, set by what the food web, drives, agriculture, medicine, materials, anatomy, and disease need to evaluate rather than by enumerating biology. The composition and edibility axes are the resolved biology floor (Part 41, R-PHYS-BIO), so a species varies over the ecological, morphological, behavioural, and life-history axes plus the floor's per-tissue composition, and every stored verdict becomes a measured reading: successional role, fire response, trophic level, biome membership, and domesticability are derived at read time, never flags.

The generator emits a closed food web, not a random bag of organisms. Generation is the ecological analogue of the floor's demand-closure: a candidate is sampled over the axes (each axis drawn at a distinct counter and mapped onto its reserved range with the multiply guarded so the product never precedes the interval clamp), then validated in a fixed order, cheapest and hardest first. Fixed-point representability comes first: a candidate that cannot be represented in the fixed-point substrate is a failed candidate, never coerced. Biome-fit is a suitability computed as the Liebig minimum of per-axis piecewise-linear tent responses over the world's environmental fields, the terrain elevation, moisture, and temperature of Part 12 and the soil-fertility stock of Part 15 read through the stock's occupancy, accepted above a reserved cutoff. Food-web closure is a monotone least-fixed-point graph walk that grounds a region's pools upward from its abiotic sources, an edge resolving only against a present source or an already-grounded pool, and resolving against the pool and the Part 58 axis registry rather than a closed source enum, so the later source-vector lift is a data change rather than a rewrite (R-SOURCE-VECTOR); a pool outside the fixed point is an orphan. Niche openness is a trait-space distance to every accepted co-resident of at least a reserved limiting-similarity floor. Validation resamples at the next ordinal up to a hard bound that guarantees deterministic termination, then falls back to the best candidate that at least closes topologically, closure being the one invariant never relaxed, or logs an empty niche as history. Closure is the hybrid: topological at seed time, with quantitative feasibility left to the Part 15 stock dynamics, which cull under-supplied pools over the early ticks and log the die-offs as Part 17 extinction rather than rejecting them at seed time.

### 25.12 The pre-dawn epoch and the world-genesis sequence

The seeded founders are not the final roster; the variety of a world is the product of generation and radiation. The genome, selection, and speciation engine of this Part runs over deep time as a bounded pre-dawn epoch on its own generation counter, before the dawn, so the people arrive into a mature, self-made ecology with established niches, edibility, and toxicity to discover. Each generation drifts the allele-frequency pools by the exact Wright-Fisher step (25.7) and applies selection through a piecewise-linear environment-to-coefficient kernel: a per-locus coefficient derives from the climate-envelope mismatch, the resource-scarcity deficit (the Part 15 carrying-capacity gap, saturating to the maximum where supply is absent), and the pathogen-load gradient, clamped to an interval bounded so the built selection divide cannot wrap. Founders fork by binomial sampling at a small effective size (the founder effect), and daughter lineages recover toward their biome's carrying capacity. Speciation is declared rather than scripted (25.7): when a lineage's distance or its accumulated Dobzhansky-Muller incompatibility count crosses the reserved threshold, a species identity is minted into a dedicated parent-pointer species-lineage tree, the genome engine staying pure mechanism, and the incompatibility table grows by an Orr-snowball roll keyed on the ordered pair and the generation so the count accumulates per sweep rather than re-rolling once. Extinction is deterministic at the pool tier, from demographic collapse to the stock floor, from sustained maladaptation below the replacement-fitness level, or from genetic diversity below the response floor, and is recorded as an event. The world-genesis sequence is therefore worldgen, then the pre-dawn biosphere epoch, then the dawn, where the mature pools are promoted and the sentient races seed fresh (the epoch is flora and fauna only), then play, with individual organisms promoted from the pools on demand and replayed from the pool seed and slot so a promotion never alters canonical pool history (Principle 10).

### 25.13 Edibility as a living relation

Edibility is measured, never stored: whether a tissue is food, poison, or medicine, and to whom, is the tuple the floor law returns, net nutrition, net harm, the margin, and a relational medicinal value, read against a consumer's physiology. An organism carries a per-tissue composition vector over the floor axes, a mass-fraction simplex plus a derived inert residual summing to one exactly, each partial lump carrying a residual child; the heritable payload is the stick-breaking coordinates of that simplex rather than the absolute fractions, so composition drifts and responds to selection while additivity stays exact, and defensive toxins under grazing and their loss under domestication fall out of selection rather than a script, through the axis-keyed composition genome channel of this Part. A consumer's physiology is a per-race heritable vector over the floor's relation kinds, derived at genesis from its drives' satisfaction sources (Part 20, the requirement magnitudes a per-race binding, optionally allometric) and its digestive capability (primitive per-race data until the data-driven anatomy model resolves it, R-BUILD-PHYS, Part 35), and evolved thereafter. Medicinal value is the consumer's own deficiency share filled by the tissue, attenuated as harm rises, so the same tissue is medicine to a deficient consumer and inert to a replete one, with no substance-side benefit map. Toxicity is dose-dependent and preparation-dependent: a reserved per-class preparation-lability matrix (cook, soak, ferment) makes a tuber that is poison raw become safe processed, a discovered technique (Part 23, riding the open Inconsistency 5), and dietary knowledge is emergent and fallible through the evidence system (Part 9), able to be true or false, to spread, to distort, and to be lost. The gain-versus-danger valuation lives in the agent layer (Parts 8, 20), heritable per race, so the authored floor carries no risk attitude, and the harm the tuple reports routes into sickness and death through a unified affliction substrate shared with wounds and disease (R-WOUND, Parts 22, 35, 52), which R-BIOSPHERE reads around rather than resolves.

### 25.14 The anatomy of a generated species

A generated species carries its anatomy the way it carries its niche: generated, not hand-authored. At the aggregate tier where the biosphere lives, a species' anatomy is a structured body plan of typed parts drawn from data-defined registries, sibling to the biome and trait-axis substrates (Principle 11): which natural weapons it bears (claws, teeth, horns, sting, and their kin), what it is clad in (hide, fur, scales, carapace, shell), which senses it has and at what acuity, and how it moves, each part carrying a development level, together with a temperament personality over the Part 20 palette (boldness or shyness, exploration, activity, sociability, aggressiveness) and the scalar life-history traits (body mass the master trait, encephalization, diet breadth). The registries are the fixed mechanism; their membership is data that grows with the world, and the magical kinds (a mana-lash, a curse-touch, a phase-hide, a mana-sight) are Provenance-fantasy entries drawn only when the world profile carries magic (Part 34), so a grounded world (Mirror, Tempest) bears none and a magical world (Arcanum, Confluence) does. The body plan is heritable and drifts and responds to selection over the pre-dawn epoch, so a prey line is pressured toward heavier armour and a predator toward better weaponry. The trophic label a creature reads by (plant, herbivore, carnivore) is derived from what it eats, its resolved food-web edges, never a stored type (fork F11), the same recognized-not-enumerated move as edibility. This is the level-of-detail principle applied to bodies (Principle 1, Part 35): the masses carry a body plan, and the full per-part Part 35 Body, an assembly of layered tissues with functions and fluids, is instantiated only when a notable organism is promoted, built from the plan and the race's data-defined anatomy (Part 40).

The deeper horizon, and the honest ceiling. The part kinds are a data-defined registry, so a world's creatures grow from a bounded but extensible floor rather than a closed enum, yet the primitives (claws, teeth, horns) are still authored membership. The on-ideology endpoint, sibling to materials-as-points-over-physics and edibility-as-a-measured-relation, is a natural weapon that is a body part of a shape and a material whose lethality is measured from the physics floor (the cut, pierce, and blunt laws of Part 41), so a lineage could evolve an unauthored weapon, a novel shape or placement no designer named, and its danger would fall out of the physics rather than a table, with claws and teeth becoming recognized descriptions rather than authored kinds. That physics-grounded morphology is now resolved as the body arc: R-BUILD-PHYS derives the mechanical build stats from the promoted per-part body (Part 20, Part 35) and R-WOUND measures a wound's propagation through the data-defined anatomy from the physics floor (Part 35, record 62.20), with a fully unauthored natural weapon (its shape and material evolved) the remaining reach that the deep-technology and morphology horizon carries. The reach of that emergence, as everywhere, equals the reach of the authored physics floor (Principle 9): the deepest primitives are always an authored layer, and what emerges is novel structure over it.

> Decided and reserved. The mechanism is settled and signed off, and on your condition it is general across organisms and per-race differentiable and data-driven. R-BIOSPHERE generates a world's living content over the resolved biology floor (Part 41) and the deep-time genome engine (this Part), re-resolving neither: a seed-keyed generate-and-validate sampler emits species as points over an extensible trait-axis registry, validated by fixed-point representability, biome-fit (a Liebig minimum of piecewise-linear tent responses), least-fixed-point food-web closure (the hybrid, feasibility left to the Part 15 stocks), and niche openness, with a bounded resample and a closure-preserving fallback; a bounded pre-dawn epoch radiates the founders by drift, an environment-to-coefficient selection kernel, founder-fork, declared speciation into a parent-pointer lineage tree, and deterministic extinction, before the dawn; and edibility is the floor's measured tuple with a relational medicinal credit, dose-and-preparation dependence, and heritable stick-breaking composition. The mechanism is fixed Rust (the validation order, the tent, closure, and additivity kernels, and the four new counter-RNG phases); the trait axes, the food-web edges, the relation kinds, the per-race bindings, and every threshold are data (Principle 11), and the kernels are race-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the generator parameters (the biome-fit cutoff, the hard resample bound, the founder counts per world and per biome, the diversity and limiting-similarity targets, and the pre-dawn generation count and its exact-path draw budget; basis: the profile's biome area and productivity, the founder-to-radiated ratio of real adaptive radiations, and the determinism termination and performance bounds); the trait-axis and composition ranges (basis: the real ecology, nutrition, and toxicology envelopes at or just above the documented maximum, already reserved in the trait-axis catalogue, fantasy axes profile-gated); the consumer bindings (the drive-to-requirement magnitudes and their allometric coupling and the primitive digestion founder values and variance; basis: the requirement tables and comparative digestive physiology for an anchoring taxon scaled allometrically, per-race data); the harm and medicinal forms (the integer Hill exponent, saturation ratio, and caps already reserved in the floor, the relational medicinal-credit form, the biphasic hormesis curve, and the preparation-lability matrix; basis: the floor determinism ceilings, the consumer's deficiency state, and the class-by-operation detoxification literature, the curve shapes reserved modelling choices); the selection and speciation scales (the climate, resource, and pathogen selection strengths and the coefficient clamp, the founder effective size and the daughter recovery capacity, the incompatibility accumulation rate and severity split, and the speciation thresholds; basis: the selection-differential and population-genetics envelopes and the divide-wrap determinism bound); the relocated gain-versus-danger risk weights and their founding prior (basis: the consumer's heritable risk tolerance, surfaced so a prior centred near a one-to-one attitude does not reinstate the authored attitude the floor removed); and the four new phase ids and their draw slots (basis: engine-mechanic RNG coordination under the R-RNG-COORD rule, reserved not fabricated). The reserved list is in the audit log. The honest limits stand: closure is topological and seed-time only, so quantitative feasibility and the extinction cascades are left to the Part 15 stock dynamics as history rather than repaired; the epoch's reproducibility is bounded to the interim biallelic genetic-distance measure until the fixation-index-versus-Nei and multi-allele choice of 25.10 is set (a stamped measure identity), with the incompatibility-count speciation criterion the measure-independent backbone; the exact-drift path gives the pre-dawn generation count a performance ceiling until the cheap large-Ne integer-Gaussian path is built (R-GPU-CANON-PIN); digestion is primitive per-race data until R-BUILD-PHYS and the anatomy model resolve it; the medicinal-credit and hormesis forms and the preparation routing are reserved against the open Inconsistency 5; and three pieces are fixed Rust to add during the build, the soil stock, the axis-keyed composition genome channel, and the founder-fork pool method, the stock substrate and the species-lineage container being already built. This resolves R-BIOSPHERE as the distinct reader of the biology floor and a major advance of the living-world track; it reads around the open siblings R-SOURCE-VECTOR, R-BIO-REGISTRY, R-WOUND, Inconsistency 5, and R-VIEW-ELAB (R-GPU-CANON-PIN and R-UNITS-PIN, formerly siblings, are now resolved, records 62.23 and 62.25), and full sentient pre-dawn co-evolution is recorded as a future candidate exploration.
...
> Needs research, item R-TEMPORAL-LOD in the research backlog. A scoping pass (`docs/temporal_lod_research.md`) decomposes this into five sub-problems and grounds each against primary literature, and it should be read before any build. The findings, in brief: the coarse population advance is binomial tau-leaping, whose exact-conservation cap is the temporal analogue of R-TIER-CONSIST's conserving restrict (Part 54), tuned by a critical-firing threshold; cross-region reconciliation is conservative parallel discrete-event simulation (Chandy-Misra-Bryant lookahead, no optimistic rollback, which would be a determinism hazard), with the lookahead derived from the world's own travel and propagation delays (Part 46); the promote and demote schedule is the significance-and-seed schedule Part 54 already defines, extended to the time axis, never the camera (Principle 10); and the camera's separate wish to watch a coarse region in full is the companion item R-VIEW-ELAB (Part 54). What remains genuine build research: the per-subsystem coarse-step registry (the conserving coarse advance and its declared projections for each subsystem beyond the ecology's existing coarse tier, each determinism-audited), and the conservative cross-region synchronization core wired to the event-driven promotion and demotion of a region's resolution, both prototyped in isolation first. The bounded limit stands unchanged and is a property of nonlinear dynamics, not a gap to close: cheap fast-forward through a busy, significant span is unattainable, because coarsening is faithful only while the leap condition holds and the region is not canonically active. Determinism and data-driven constraints: every coarse draw is counter-based RNG keyed on the region, the covered span, the seed, and a registered phase, so a coarse step and the fine steps it replaces draw from a shared reproducible source; all inter-region events apply in a total canonical order (R-CMD-ORDER, R-REDUCE-ORDER); the coarse-step registry and the subsystem projections are data, not a hardcoded enum (Principle 11). Reserved values are surfaced with their bases in the scoping pass (the base-tick duration, the leap-condition tolerance, the critical-firing threshold, the per-region coarsening idle threshold, and the cross-region lookahead floor), none fabricated. The scoping pass also enumerates the concrete build factors (the per-subsystem coarse-step registry and the catalogue of which systems get a coarse advance versus are fine-only, the event-channel and rate model, the fixed-point Poisson and binomial sampler, the conservative cross-region core, the stiffness policy with promote-on-stiffness recommended, and the entity-handoff-across-clocks protocol). Prerequisites, open items a build depends on rather than merely couples to: R-PROJ-REGISTER is the load-bearing one, since a coarse step conserves exactly only what each subsystem registers as an exact projection and today only population and wealth do; R-HARNESS-COVER, the harness proving coarse-then-fine agrees with fine-throughout on conserved state; R-GPU-CANON-PIN (now resolved, record 62.23), which supplies the fixed-point sampler the coarse draw needs; and R-CMD-ORDER, R-REDUCE-ORDER, and R-SAVE-SCHEMA for the inter-region order and mixed-resolution saves. Couplings: R-TIER-CONSIST (Part 54, resolved) is the conservation contract every transition must meet, R-VIEW-ELAB and R-LIVELINESS are the view-side siblings, and the conservative cross-region synchronization core shares its event-queue substrate with event-driven agent execution (Part 57's second foundation). Flagged, not changed.
...
pub struct Concept {
    pub id: ConceptId,                      // stable, minted through the registry; culture and
                                            //   birth event kept as provenance, not the id (33.4)
...
Modality is now a first-class, data-defined property, resolved by a focused facet fan-out with a closed-enum, Steering Audit, and determinism verification pass, on the owner's condition that it be general, data-driven, and per-race differentiable, and that it add only data so no R-LANG-DET determinism rule reopens.

The channel substrate. The single vocal-auditory assumption is replaced by four data registries, siblings of the value substrate (Part 21), the semantic substrate (33.1), and the access-channel registry (Part 40): production modalities (acoustic, manual, chromatic, tactile, chemical, electric, magical emission), reception senses (audition, vision, somatosensation, chemoreception, electroreception, magical sense), feature dimensions (the contrast axes a modality builds primitives from, place and manner and voicing for the acoustic modality, handshape and location and movement and orientation and non-manual marking for the manual one, each carrying its own value set as data), and media (the physical carrier a signal propagates through). A communication channel is not an enumerated kind but a derived pairing of a production modality and a reception sense over a shared medium, recognised the way an institution's kind is recognised from its function vector rather than declared, so the named modalities and any a race evolves are data instances and a new channel needs no engine change. The former phonetic substrate becomes one production modality, the acoustic one. Following the closed-enum audit, the medium is itself a registry (a medium is a physical affordance, authorable under Principle 9, but its membership is data so a seismic or aetheric carrier is denotable), and a feature dimension carries its admissible values as data rather than a bare count.

Form primitives and articulation. A form primitive generalises the phoneme to an integer-indexed segment, and a segment is a bundle of simultaneous feature values over its modality's dimensions, canonicalised at construction (one value per dimension, every obligatory dimension present with an explicit unspecified value for a genuine absence, sorted by dimension id) so two machines build a bit-identical bundle. A word form stays a sequence of segments, sequential across segments and simultaneous within, which carries a spoken word (a long sequence of thin bundles), a sign (a short sequence of thick bundles), and an exotic channel alike. The R-LANG-DET intra-form scan runs left to right over the segment sequence, and the bundle, being a canonical sorted set, carries no order of its own, so the form-change ordering, the same-tick canonical key (which serialises each bundle through that canonicaliser), and the distance metric all hold unchanged. A regular form change rewrites feature values in an environment, so a signed handshape assimilation and a spoken consonant shift are one mechanism, and feeding and bleeding stay a function of chronology (the documented regular formational change of signed languages, Frishberg, confirms the carry-over).

Per-being channels. Each being carries, per channel, a production capability and a reception capability with a congenital ceiling, computed at birth from the resolved genome and anatomy through a data-defined capability query (the body functions and gene-fed phenotypes a channel needs), taking the binding gate; a non-vocalising race or a congenitally deaf individual falls out as an ordinary phenotype with no special case, recessive loss hiding and resurfacing by the genome's own dominance, and a lineage gains or loses a channel over deep time as its anatomy evolves. The live capability can only be lowered from its ceiling, never raised above it, which is the production ceiling of 33.6 generalised to every channel and to reception. Acquired loss by injury is a reserved one-directional coupling to the now-resolved anatomy and wound model (Part 35, R-WOUND): a single recompute reads the impaired body-part functions and clamps the capability down, with no wound-model content presumed here. Production and reception are independent, so a being can perceive a channel it cannot produce (the comprehending mute) or, after a wound, lose one and keep the other (the deafened speaker who shifts to a visual tongue). Capability keys off per-being data, never a sentient or personhood notion.

Cross-modal contact, learning, and emergence. A being with no reception capability in a language's modality cannot learn it directly and is reached only through a bridge; otherwise learning cost consumes the resolved distance decomposition, so a disjoint form substrate gives no help while shared grammar and concepts still lower the cost, and a cross-modal contact never collapses to maximum distance. Proficiency splits into a comprehension track bounded by reception and a production track bounded by the production ceiling, so receptive command of a tongue one cannot voice falls out. The bridges are the emergent bi-modal interpreter (the interpreter role of 33.6 with a channel condition) and the written record (a script in a perceivable modality standing for concepts on the shared substrate, so a signer reads a spoken culture's script without ever perceiving its channel). A being with two production channels may blend them simultaneously or switch serially, a fixed mechanism choice (use one, alternate, overlay) whose selection is a data-driven function of the parties' channels and a reserved blend propensity that governs the form of mixing and never a fidelity bonus. Cross-modal transmission distortion is the existing error budget with the form channel removed, so the false-cognate error vanishes while concept-snapping and nuance loss intensify. The dawn dynamic settles a seed form system over whatever inventory a race produces, with the dispersion game's geometry a per-modality perceptual-distance table whose contrast budget is a read-out of the channel's cited physical capacity rather than a free weight; the homesign-to-community-language arc emerges from the dawn fired at a coordinating-unit tier on a channel-mismatch condition, then conventionalised by the existing contact, grammaticalisation, and iterated-learning machinery (the Nicaraguan and Al-Sayyid Bedouin cases as in-world events); and a modality shift is one in-world event that relexifies and calques structure while the modality-free concepts survive, logged as a single append-only entry, so a signed daughter stays a reconstructable descendant of its spoken parent. The dawn coordinates a differentiated prime floor under a modality-blind safety cap (33.9), so a language's thinness reflects its community's shallow transmission depth (fewer lagging primes closed over fewer generations), never its channel, the founding modality accounting for no thinness of its own.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general, data-driven, per-race differentiable, and adds only data so no R-LANG-DET determinism rule reopens. Modality is a first-class data property: production modalities, reception senses, feature dimensions, and media are registries, a channel is a derived production-and-reception pairing over a shared medium, a form primitive is an integer-indexed canonical bundle of simultaneous feature values, and each being carries congenitally-ceilinged produce and perceive capabilities derived from the resolved genome and anatomy. Cross-modal distance, learning, bridging, code-blending, distortion, and emergence are compositions of the resolved language machinery, and the only new in-world event is modality migration, which lands as a row in the eventual event registry (the open R-EVENT). The mechanism is fixed Rust; the channel registries, the feature dimensions and their values, the per-race anatomy-to-channel mapping, and the rule content are data (Principle 11). Two Steering Audit holds are structural: a channel carries no complexity or sophistication field and no privileged status (Hockett's channel is a descriptive feature, ranking none), and a new modality-swap invariant requires paired worlds that differ only by an equal-capacity channel swap to show invariant emergent richness and trajectory, so a difference physics does not force is surfaced as a designer attractor. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the channel-registry memberships and each modality's feature dimensions and values (basis: the sensory-ecology channel record as a starting menu, the sign-phonology record for the manual modality, the articulatory record for the acoustic one); the per-modality perceptual-distance geometry (basis: each channel's cited confusability and capacity, dispersed to the same target separation as the acoustic modality, the contrast budget a read-out of cited physics and never a free weight, validated under the convergence audit's permuted-basis test); the capability-gate floors and curves (basis: the reserved anatomy interface, the floor set equal to the wound model's function-loss threshold for consistency); the comprehension-and-production acquisition split and its rate ratio (basis: the receptive-bilingual asymmetry, production capped by channel capacity); the cross-modal blend propensity per culture and being (basis: the measured blend-versus-switch rate, addressee and register dependent, a form-of-mixing knob and never a fidelity multiplier); the dawn round cap as a modality-blind determinism safety bound (basis: the existing performance-against-emergence budget, the same scaling for every modality; the founding coverage is the differentiated prime floor of 33.9, the resilient primes at the dawn and a lagging subset closing over generations); and the channel-mismatch trigger thresholds for a homesign dawn and a modality shift (basis: the population-fraction-over-window form the engine already uses, the recency window reused from R-LANG-DET). The cross-modal form distance reuses the no-shared-form ceiling that any form-disjoint pair hits, with no distinct authored cross-modality ceiling, and the acquired injury-loss magnitude is reserved to R-WOUND, not here. The reserved list is in the audit log. The honest limits stand: the per-being channel capability exists in full only for promoted beings, an aggregate at the pool tier as the rest of the model is; acquired channel loss waits on R-WOUND (now resolved, Part 62.20), congenital loss being complete now; the multi-modal and sign articulation contents are reserved data the owner supplies; and the grammar typological parameters remain the open R-LANG-TYPOLOGY.
...
The order-sensitive and unbounded procedures are now pinned, on the owner's condition that the result be general, data-driven, and modality-neutral, and the pins are worded over the generic form substrate so the modality generalization (R-LANG-MODALITY, since resolved in record 62.13) adds only data and reopens no determinism rule.

Identity and ordering. A concept id is minted through the registry (the stable-id allocator of Part 3, the sibling of the trace-kind registry of 9.9 and the access-channel registry of Part 40), not hashed from content; the culture and birth event are kept as provenance, so a concept carries a referential identity that resolves and cannot collide, and a merge retires the absorbed id rather than freeing it under the never-reused contract, folds its salience into the survivor, and leaves the lineage-minimal birth-event id as the stable tiebreak. When several cultures lexicalise in one tick the allocation order is the canonical culture-id then event-ordinal order under a single lexicalisation pass, never a per-thread mint order. A regular form change (the rewrite rule of the drift engine, over form primitives rather than sounds specifically) applies at once to every lexicon form that meets its environment, and the rules of a generation apply in sequence by their innovation index, the output of each feeding the input of the next, so feeding, bleeding, and the opacity of counterfeeding and counterbleeding are a deterministic function of relative chronology. Rules innovated in the same tick are sorted before they are appended, by the canonical key of their content (target primitive, output primitive, environment-primitive sequence, and rule kind) with the producing id and the canonical slot ordinal of that producer's same-tick emissions as the final tiebreak, and the log is append-only and never reordered, so the index is a total order over the whole history. Within a single form a rule scans left to right with non-overlapping consumption. The operators of a generation run in a fixed phase order: any modality migration and tier-fired dawn first, so the rest feed on the resulting form substrate (the slot R-LANG-MODALITY fills), then form-change passes, then borrowings in canonical set order, then lexical replacement and grammaticalisation, then semantic-shift prototype migration, then concept split and merge, then language splitting last, so a daughter inherits the generation's regular changes.

Accumulation, distance, and the dawn. Concept salience is a leaky accumulator carried at the widened internal scale and narrowed only when it is read for comparison or hashing, which removes the per-tick re-quantisation that would otherwise strand a disused concept above the merge floor; usage enters as a normalised rate over a recency window, and the saturating add is the declared overflow policy of Part 55, never a wrap. Language distance keys off the shared semantic substrate rather than the per-culture ids, exactly as the value metric of Part 21 projects two partitions into the same etic coordinates before measuring: the shared-concept correspondence is read by coverage over a canonical quantised walk of the shared substrate, symmetric by construction and using no per-region nearest-neighbour matching, with the walk key the lexicographic tuple of quantised constraints, quantised prototype, and lineage-minimal birth event. The three components accumulate in the widened domain with exactly one rounding at the final narrowing, each feature difference made absolute before its weight is applied, and the component weights normalised to sum to one with the rounding residual assigned to the dominant lexical component. A form substrate one side does not share contributes the reserved incommensurability ceiling rather than a division by zero, and the cross-modality case (a spoken and a signed tongue, whose form substrates are disjoint) is that same ceiling at the form component while the grammatical component still computes, so two cultures that share concepts and typology stay measurably closer than two that share neither, and the distance never collapses to its maximum on a modality difference alone. The dawn coordination fires once per culture on the culture-genesis event, in a fixed language phase slot under a per-culture has-run guard, keyed by counter RNG on the seed, the culture, the birth event, and the phase, and settles a seed form system over the race's producible form inventory, whatever its modality.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general, data-driven, and modality-neutral. Concept ids are minted through the registry rather than content-hashed; form-change rules apply at once and compose in innovation-index order under a canonical same-tick tiebreak and a fixed within-generation phase order; salience is a widened leaky accumulator; language distance is a symmetric, matching-free measure over the shared semantic substrate with a single canonical rounding; and the dawn dynamic fires deterministically on the culture-genesis event over the race's producible form inventory. The mechanism is fixed Rust; the form-substrate membership, the concept partitions, and the rule content are data (Principle 11), and the determinism pins are worded over the generic form primitive so the modality generalization (R-LANG-MODALITY, since resolved) adds only data. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the salience decay rate (basis: the usage-recency half-life, with a hard lower bound so the usage term cannot underflow to zero, and with the concept-merge threshold required to sit above the salience scale's resolution floor); the usage-normalisation recency window (basis: the window the retention record uses); the substrate quantisation granularity (basis: the coarsest grid that keeps distinct concept regions separable, consistent with the Part 21 ground-metric granularity, one knob reused for the walk, the walk key, and the correspondence tie window); the per-cell incommensurability contribution for one-sided coverage (basis: set equal to the existing language-distance floor rather than a new constant); and the no-shared-form ceiling, which applies identically to any form-disjoint pair, a cross-modality one included, so cross-modal difficulty emerges from form giving no help rather than from a separate authored constant (the optional distinct cross-modality ceiling once reserved here is retired on the Steering Audit ground that it would author non-vocal isolation; see R-LANG-MODALITY, record 62.13). The three distance-component weights remain reserved as before, now constrained to sum to one by deterministic residual absorption into the lexical component. The reserved list is in the audit log. The honest limits stand: the dawn dynamic coordinates a differentiated prime floor under a hard safety cap (33.9), which trims realism for guaranteed determinism; divergent salience is conserved as mass but not as identity across demotion; and the modality generalization itself, signed and non-vocal languages as first-class with per-being produce and perceive channels, is the separate item R-LANG-MODALITY (since resolved, record 62.13). The grammar typological parameters remain a closed set pending R-LANG-TYPOLOGY, flagged below.

> Needs research, item R-LANG-TYPOLOGY in the research backlog. The grammar parameters of the generation model (word order, head direction, morphological type, alignment) are a closed set of Rust enums, a list authored one level below the emergent layer, where the project's discipline is a data-defined registry whose membership grows with the world (the sibling pattern of the value substrate of Part 21, the semantic substrate of 33.1, the institution-function substrate of Part 36, and the access-channel registry of Part 40). A culture that grammaticalises a word order or an alignment outside the enumerated set cannot denote it, the same closed-list seam the institution-kind and second-order-evidence work opened elsewhere. A session should harden the typological parameters into a data-defined typological substrate, the mechanism fixed and the parameter values with their harmonic correlations data, keeping the seeded sampling and the determinism of 33.4 intact. Determinism and the data-driven substrates hold. Couples to Parts 33, 25. Flagged, not changed.

### 33.5 Language distance and mutual intelligibility

How far two languages have diverged is a structural distance over a structured space, and it reuses the value-distance machinery (Part 21) rather than inventing a new metric. It has three fixed-point components. Phonological distance is a feature-based distance between the two inventories and phonotactic profiles over the shared phonetic substrate, with sounds one side cannot produce or perceive contributing to a phonetic incommensurability term. Lexical distance is the dominant component: over the core concepts both cultures hold, it is the normalised integer edit distance between their word-forms (phoneme-index strings), averaged, with cognate recognition falling out of low edit distance and concepts one side lacks adding a capped distance through the incommensurability floor rather than being dropped. Grammatical distance is the distance over the typological parameter vector (word order, head direction, morphological type, alignment). The three combine under reserved weights, with lexical distance weighted highest because it dominates real mutual-intelligibility measurement; the shared-concept comparison runs over the shared semantic substrate rather than over per-culture ids, by the symmetric, matching-free measure pinned in 33.4, with a single canonical rounding at the final narrowing. Mutual intelligibility is a decreasing function of the combined distance, consumed as a friction and throughput multiplier on the transmission of belief and knowledge across a language barrier (Parts 9, 23): dialects understand each other nearly fully, separate families barely at all.
...
The first language is neither fully templated nor simulated from nothing at full cost. A thin coordination dynamic runs once per culture at its dawn, coordinating the first form-meaning pairings over anchor concepts grounded in the NSM semantic primes (the primitive axes of 33.1, the universal grounding floor toward which languages converge) and a discrete dispersion game that settles a seed sound system, then hands the inventory and anchors to the seeded generator (33.4) to fill the full lexicon and to the drift engine thereafter. The dynamic is bounded by a hard round cap with counter-RNG keying and id-ordered aggregation, so it is bit-deterministic by construction, and it runs only at the culture or pool tier, so it is cheap. The founding coverage of the floor is differentiated, not uniform: each prime carries a data-defined founding resilience (33.1), and the dawn coordinates the resilient primes (the concrete, referential, and desiderative core the language-emergence record shows established from the start), while a lagging subset (the epistemic predicates, the logical and subordinating connectives, and exact quantification beyond a small set, the categories the record shows arriving a generation or more later) is left uncoordinated at the dawn. A prime left uncoordinated closes later through the same discrimination-and-coordination dynamic that fixes any concept (33.1), when the community's accumulated transmission (a function of its size, its generations of depth, and its contact, all simulated state) crosses that prime's reserved closing threshold, so the floor fills toward completeness over generations rather than at a stroke, by one uniform mechanism read from state and never a per-culture authored rule. The round cap collapses to pure seeded generation at zero, and it is a determinism safety bound on the per-episode coordination, never a thinner-language lever tied to cohort size (a rejected steer, since it would author which peoples stay thin and runs backwards to the finding that small isolated communities trend toward more complexity, not less). Cross-language richness above the filling floor is carried by semantic molecules (33.1), the composite concepts built on the primes, not by differing prime inventories. This is the chosen origination model among the three considered (pure seeding, full bootstrapping, and this hybrid); it is the one that honours order emerging from coordination as a real if abstracted mechanism while staying inside the determinism and performance budgets.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general, data-driven, and per-race differentiable, following a fully-blind audit against the primary sources that the owner asked run so the founding-coverage rule would not steer. The NSM semantic prime set is the universal grounding floor toward which languages converge, an adopted, citable, revisable axiom rather than a fixed constant: the inventory has moved across the program's history and carries real challengers (Bohnemeyer's Yucatec Maya gap for BEFORE and AFTER; Riemer's method critique), so its count and membership are data. Founding coverage is differentiated: the resilient primes coordinate at the dawn and a lagging subset closes over generations through one uniform mechanism read from simulated state (community size, generational depth, contact), never a per-culture lookup, which is the emergence record's finding (Nicaraguan Sign Language cohort one produced few or no THINK and KNOW forms, the category closing only in the second cohort, Pyers and Senghas 2009; the same generational lag for IF and BECAUSE clause-linking, Sandler and colleagues 2011, the pidgin-to-creole cycle, and child first-language acquisition, epistemic primes acquired last, Bartsch and Wellman 1995). The round cap is a determinism safety bound and never a cohort-size thinner-language lever, the rejected steer (it would author which peoples stay thin and runs backwards to the finding that small isolated communities trend toward more complexity, Trudgill 2011). Semantic molecules (Goddard 2007, 2012) carry richness above the floor. The mechanism is fixed Rust; the prime membership, each prime's founding resilience, the molecule inventory, and the closing rule's rates are data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: each prime's founding-resilience rank (basis: the language-emergence record, concrete and desiderative primes resilient from the start, the epistemic, logical, and exact-quantification primes documented as lagging a generation or more, so the rank is grounded and per-race adjustable rather than a closed list); the closing threshold that fills a lagging prime as accumulated transmission crosses it (basis: the one-to-two-generation lag the emergence record shows, expressed over community size, generational depth, and contact); and the dawn round cap as a determinism safety bound (basis: the performance-against-emergence budget, the same scaling for every modality). The reserved list is in the audit log. The honest limits stand: the floor membership and count are one research program's revisable estimate, adopted with citation rather than as settled fact; applying a human-typology floor to non-human sapient races is a separate, explicit convergent-cognition worldbuilding axiom the owner supplies; and the lagging-subset closing is modelled at the culture or pool tier, an aggregate as the rest of the model is. Couples to 33.1, 33.3, 33.4, Parts 20 and 25, and records 62.6 and 62.13.

### 33.10 Determinism

All canonical language state is integer and fixed-point: every distance, proficiency, salience, prototype point, and threshold is fixed-point, and no floating point touches it. Every stochastic choice, phoneme selection, word-form sampling, sound-change selection, the dawn dynamic's tie-breaks, and the mistranslation draws, is keyed on a hash of the master seed, the culture or entity id, the phase, and an event ordinal, so it is reproducible regardless of evaluation order or thread count. Sounds are integer indices into an inventory, never platform strings, so edit distance, hashing, and the sound-change rewrites are integer operations with defined results, and the edit-distance computation walks a canonical order; the lexicon is a sorted vector, not a hashed map, so iteration is deterministic. The English gloss lemmas are authored constants, and any rich text rendering happens at view time. Exactly one thing is non-deterministic and it is fully quarantined: the optional external interpretation layer, carried as a view-only type that canonical code cannot accept. The honest limits are that the dawn dynamic coordinates a differentiated prime floor under a hard safety cap, the resilient primes at the dawn and a lagging subset over later generations (33.9), which slightly trims realism for guaranteed determinism; that the typological priors and implicational filters keep generated languages in the plausible region but cannot certify every one as natural; and that a concept far from any authored anchor on the substrate receives a coarse gist rather than a crisp gloss, so legibility is guaranteed but precision is not.
...
```rust
pub struct Body {
    pub parts: Vec<BodyPart>,             // promoted from the data anatomy (Part 25.14, Part 40)
    pub fluids: Vec<FluidPool>,           // what this race runs on, data-defined (blood, ichor, sap, none)
}

pub struct BodyPart {
    pub kind: PartKindId,                 // data-defined anatomy: a limb, organ, carapace segment, wing
    pub tissues: Vec<TissueLayer>,        // layered tissue materials, each a Substance over the floor axes
    pub vital: bool,                      // destruction is lethal
    pub functions: Vec<FunctionId>,       // data-defined: grip, sight, locomotion, a natural weapon
    pub condition: PartCondition,         // derived integrity in [0,1] and severed, from the wounds applied
    pub carries_fluid: Option<FluidKindId>,
}

// A damage mode is DATA, keyed to a physics measurement family the engine runs against the tissue:
pub enum MeasureKind { Penetration, Fracture, Thermal, Exotic }   // fixed engine measurements
pub struct DamageModeDef { pub id: DamageModeId, pub measure: MeasureKind }
// grounded members cut/pierce (Penetration), blunt (Fracture), burn (Thermal); a magical world adds
// corrosion, freeze, or a curse as data with Exotic (a reserved, audited floor extension).

// A fluid is a DATA-defined substance with a conserved volume stock:
pub struct FluidDef { pub id: FluidKindId, pub critical_fraction: Fixed, pub clot_rate: Fixed /* + a composition over the biology floor */ }
pub struct FluidPool { pub kind: FluidKindId, pub volume: Stock, pub bleed_rate: Fixed }

// A wound is MEASURED, not stored: an insult over the floor yields a condition change and an outcome.
pub struct Insult { pub mode: DamageModeId, pub force: Fixed, pub contact_area: Fixed, pub delivered_energy: Fixed, pub delta_t: Fixed }
pub struct WoundRecord { pub mode: DamageModeId, pub severity: Fixed, pub severed: bool, pub fractured: bool, pub vessel_breached: bool }
```

A wound is not a stored kind but a measured consequence, the on-ideology endpoint sibling to edibility-as-a-measured-relation (Part 25.13). An insult carries a mode, a force and delivered energy over a contact area, and (for a thermal mode) a temperature rise; the mode's measurement family selects which resolved floor law computes the damage against the part's tissue material (Part 41, R-PHYS-MECH): penetration walks the layers from the surface (contact pressure then cut depth, cut and pierce differing only by the contact area, the crush-versus-pierce geometry, and a bone core stopping a weak cut), fracture applies the dual stress-and-energy criterion to the strongest structural layer, and thermal reads the constrained thermal stress. The measured damage reduces the part's integrity, a cut clean through severs it, and a deep wound breaches the fluid vessel the part carries. The damage-mode taxonomy is a data registry rather than a closed enum, so a magical world adds corrosion or a curse as data (its measurement a reserved, audited floor extension, never a fabricated effect), and the wound model reads the data-driven anatomy rather than one body plan, so what a wound means differs across races by their parts and tissues (R-WOUND). A body fluid is a data-defined kind with a composition over the biology floor and a conserved volume stock: its loss bleeds from a breached part at a rate the breach severity sets less a reserved clot rate, its ignition rides the resolved combustion law over its composition, and a pool drained past its critical fraction is fatal, so a race runs on blood, ichor, sap, or nothing as data (R-FLUID). The physical build promoted from the aggregate anatomy vector is the ground the mechanical stats are read from, not a set of authored numbers: reach is measured from the limb segments through the resolved reach law and effective strength from the muscle and its material, so the raw morphology is the primitive and the mechanical stats are derived (R-BUILD-PHYS).

> Decided and reserved. The mechanism is settled and signed off, and on your condition it is general across data-defined anatomies, per-race differentiable, and data-driven. A body is a per-part assembly of layered tissue materials promoted from the Part 25.14 anatomy vector on demand (Principle 1); a wound is the measured consequence of an insult meeting a part's tissue, computed by the resolved R-PHYS-MECH floor laws (contact pressure and cut penetration, the dual fracture criterion, constrained thermal stress) and recorded as the part's condition change, never a stored damage kind; a fluid is a data-defined substance with a conserved volume stock whose loss is a wound consequence; and integrity is the derived aggregate of part condition and fluid levels, never a competing store (a body is alive while no vital part is destroyed and no pool is past its critical fraction). The mechanism is fixed Rust (the promotion, the layered penetration walk, the fracture and thermal reads, the bleed and the integrity aggregate, and the measurement families); the tissue materials, the damage modes, the fluid kinds, the body functions, and the anatomy are data (Principle 11), and the wound measurement is race-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the tissue material properties per material (hardness, fracture strength and energy, specific cutting energy, elastic modulus, expansion; basis: real material datasheets, the floor's provenance discipline); the damage caps (pressure, energy, stress; basis: the floor axis maxima); the fracture-damage and burn-severity mappings from the measured margin to the integrity drop (basis: the loss of structural function when a part breaks or burns); the fluid critical fractions and clot rates and the breach bleed rate (basis: the survivable fractional loss and the coagulation and haemorrhage timescales, per race); the body promotion parameters (the mass distribution and tissue thicknesses; basis: real body-segment fractions and tissue depths scaled allometrically); and the strike energetics (the force, velocity, and contact area of a blow; basis: the wielder's derived strength and the weapon's geometry). The reserved list is in the audit log. The honest limits stand: the penetration read uses the surface tissue's cutting resistance capped by the layer walk rather than depleting energy layer by layer (a refinement); the environmental thermal exchange that would drive a core-temperature band waits on the located world and a medium floor (the temperature axis is a reserved two-sided band); corrosion and full fluid chemistry (clotting beyond a rate, corrosion) wait on wave-2; and the digestion model R-BIOSPHERE deferred here couples to the consumer physiology (R-PHYS-BIO) and is reserved. This resolves R-BUILD-PHYS, R-WOUND, and R-FLUID together as the body arc, the unified affliction harm sink R-BIOSPHERE and the edibility floor defer to, and it closes the R-BEHAVIOR-EVOLVE loop: a wound lowers the integrity the evolved controller reads (Part 8.4), and a natural weapon affords a strike whose wound is measured here, so predator-prey behaviour is real and evolvable.
...
The update rule is the inference engine of Part 9.10 run recursively, with the subject re-pointed and the question changed from whether a thing is true to whether a given mind believes it. For a question the modeller cares to model about a target, a subject and attribute it wants to know the target's belief on, the modeller holds a meta-frame that is the same `InferenceFrame` of 9.10 with its subject set to the target and its hypotheses ranging over the target's possible belief-values, including the same explicit unknown read here as "I cannot model what the target believes." The combiner is the unchanged 9.10 accumulator: signed integer weights add into clamped log-odds totals that are clamped only at read, so the committed meta-belief is independent of the order evidence arrived in, and the meta-facet commits when the leader clears a reserved meta threshold and beats the runner-up by a reserved meta margin, else it stays unknown. No new arithmetic and no division enter the authoritative path, so the recursion inherits the order-independence of 9.10 and the bit-identity of 9.15 unchanged, and it is a nested belief store exactly as the structure above already declares.

What makes the nested model diverge from projection is an admissibility rule on what may feed it, and the rule is made a typed, audited property rather than a coding convention. Every belief and every piece of evidence carries an order, a data attribute that records what the evidence is about: world evidence is about the world (the marble is in the box), and access evidence is about a particular mind's epistemic access to the world (that mind was at the river, was told, said a thing, could reach a fact, lacked access, or was denied it). The meta-frame for a target admits only access evidence about that target on the same subject and attribute, and never the modeller's own world belief about that subject and attribute, which carries the world order and is rejected by construction; ground truth never enters any belief store at all (9.1). This is the anti-projection guarantee the consumers depend on, and it is enforced structurally, by a nested-store write path that accepts only access evidence and by a determinism-harness assertion that no world facet ever contributes to a nested frame, so two minds with different histories of a target's access reach provably different conclusions about what the target believes, which is the Perner-Wimmer property that a false belief is unreachable by any first-order shortcut. The membership of access-relation kinds, witnessed access, told, said, reachable, absence of access, and denied access, is data, an access-channel registry (Part 40) sibling to the trace-kind registry of 9.9: each entry maps a believed access relation about a target to the meta-hypothesis it supports and the weight it carries, exactly as a trace kind maps a perceived trace to a proposed first-order belief, so a new sense or a new medium of access is a data entry and the engine enumerates no closed set of second-order evidence. The order tag itself is the recursion-level discriminator, fixed Rust mechanism parameterised by the stable id of whose access is meant, not a catalogue of world content. The inputs to a meta-frame are therefore drawn from the modeller's own first-order beliefs about the target's access events, the modeller reasoning about what the target believes from what the modeller believes about the target's access, never from what the modeller believes about the world fact.

```rust
// What a piece of evidence is ABOUT: the world, or a specific mind's access to the
// world. A recursion-level tag on evidence (fixed mechanism), not a closed catalogue
// of evidence kinds; `of` is open over every mind, so nothing here is world content.
pub enum EvidenceOrder { World, Access { of: StableId } }

// A nested model reuses the first-order store, but its write path admits ONLY access
// evidence about the modeled mind, which is the typed anti-projection guarantee: the
// modeller's own world belief about (subject, attr) can never feed it. The harness
// asserts no World facet ever contributes to a nested frame.
pub struct NestedBeliefs { pub of: StableId, store: BeliefStore }
impl NestedBeliefs {
    fn admit(&mut self, ev: Evidence, order: EvidenceOrder) -> Result<(), Rejected> {
        // rejected unless order == Access { of: self.of }
        todo!()
    }
}

// The membership of access-relation kinds is data (Part 40), sibling to TraceKindDef
// (9.9): a believed access relation about a target maps to the meta-hypothesis it
// supports and its weight. A new sense or medium is a data entry, not an engine change.
pub struct AccessChannelDef {
    pub id: AccessChannelId,
    pub implies: SmallVec<[MetaImplication; 2]>, // (target-belief hypothesis, weight)
}
pub struct MetaImplication { pub attr: AttrKind, pub value: AttrValue, pub weight: Fixed }
```

Seeing through a lie is a third frame rather than a special case, the question of whether a speaker is sincere about a claim, fed by the gap between an access-built meta-facet of the speaker's own true belief and the speaker's assertion; the modeller's world belief may enter that frame only on a heard assertion together with a committed and divergent meta-facet, never as a blanket read of its own corpus, so the Sally-Anne false belief and the seen-through lie are both produced by one mechanism. A single utterance updates two disjoint frames in a fixed phase order: first the second-order update from the assertion, so the model of the speaker reflects what was said; then the deception frame judged against a snapshot of the meta-commit taken before that write, the read-old write-new discipline of 9.15, so the assertion is judged against the prior model of the speaker rather than against itself; then the first-order world update of 9.5 with the deception verdict gating the told weight. The partition that keeps the gossip loop parallel-safe is re-derived over the pair of listener and modeled subject rather than the listener alone, since the co-update writes a model of a third party, and the order of conflicting writes to one nested store is one of the non-associative combines pinned by R-REDUCE-ORDER. Every roll keys counter-based RNG on a hash that extends the 9.15 key with the nesting depth and the modeled subject, so draws at different depths never alias and the second-order phase tags stay disjoint from the first-order ones.

A fresh nested store is not blank: its prior is common ground, the floor of what is mutually evident before specific access evidence arrives, and common ground is relational rather than a per-facet flag. It is scoped to the witnessing set that mutually observed an event, reflects the value as of the tick the set last jointly re-witnessed it, is re-opened to unknown by any private first-order update to the facet by a tracked party, and admits only co-witnessed physical events; public declarations and shared gossip are evidence of what was said, the deception channel, and are kept out of common ground as belief evidence. The nesting depth is bounded per mind from genome reasoning acuity (Part 25), with the counting convention pinned so the modeller's own belief is depth zero, the first nested store is depth one, and the nest of a nest is depth two; the bound is set so the canonical second-order tasks the consumers require sit below it, two as the evidence-driven depth for most minds and three for high-acuity minds where higher-order deception and brinkmanship need the model of one mind's model of another. Beyond the bound the model degrades to common ground and never to raw projection of an access-fed frame: projection survives only as the prior of a truly unfed frame at the bound, held below the lead a frame needs to commit on its own and tagged so a deception probe can tell a projected facet from an evidence-backed one, so absence of access yields unknown rather than a fabricated belief, and an agent actively running a deception instantiates full access-driven frames to its acuity depth rather than the cheap default. The depth-bound selector is a capability parameter keyed to acuity, and the Steering Audit (Principle 9) holds it to that: it is never a per-race switch that pre-decides who can scheme, since a cultural dial over a class of deception would be an authored outcome rather than an affordance.

Cost is bounded the way first-order belief is. A nested store is instantiated only for a load-bearing relationship, gated by the same salience signal first-order belief uses (9.7), and access evidence is event-driven off the perception and gossip events that already fire rather than scanned per tick per relationship, so a quiet farmer carries no nested stores and the cost is a handful of frames per modelled relationship. Breadth is bounded as well as depth: a public event before many witnesses is one common-ground relation over a witnessing set carried by a single set id tested in stable-id order rather than a pairwise flag per observer, and the per-mind count of modelled relationships is capped. The nested stores are conserved across promotion and demotion by registering a relationship-keyed second-order projection with the conserved-projection registry (Part 54, Part 58), sibling to first-order aggregate belief mass and subject to the registration discipline of R-PROJ-REGISTER: the relationship-id set and the prevailing meta-value mass per target and attribute are conserved by the same id-ordered fold and monotone level-to-strength curve the first-order tier uses, with the honest limit that the divergent per-mind component has no sufficient statistic and is lost on demotion, which is why the level-of-detail gate keeps an active schemer and the mark whose false belief the scheme turns on promoted while the scheme is live. The same two systems that differentiate first-order inference differentiate this: a mind's acuity (Part 25) scales the weight it extracts from access evidence and lowers its meta-threshold, and its epistemic stance (Part 28) sets the meta-prior, the margin, and the discount it applies to a possible lie, with that discount read from the intentions staged at the previous phase so the per-tick weights stay constant and the accumulation stays order-independent, so a credulous low-acuity mind takes assertions at face value while a sharp suspicious one discounts them and models others differently from the same evidence.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general and per-race differentiable and data-driven. The nested mental model is the resolved evidence engine (R-EVIDENCE, Part 9.10) run recursively on whether a target believes a thing, fed only by access evidence about the target and never by the modeller's own world belief or by ground truth, which is the typed anti-projection guarantee that gives the Sally-Anne false-belief competence and the seen-through lie from one mechanism. The mechanism is fixed Rust; the access-channel set that says which believed access relations imply which target-belief hypotheses, with their weights, is a data registry (Part 40) sibling to the trace-kind registry, so the engine enumerates no closed set of second-order evidence, and the report's proposed fixed six-kind enum was hardened to this registry on review (Principle 11). Determinism, order-independence, and the conserved projection reuse the resolved spines: the integer log-odds accumulator with clamp-at-read of 9.10 and 9.15, the depth-and-subject-extended counter key, the listener-and-modeled-subject gossip partition whose conflict order is pinned by R-REDUCE-ORDER, and a relationship-keyed conserved second-order projection registered under R-PROJ-REGISTER and Part 54. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the access-channel weights (basis: the weight-of-evidence of each access relation toward a target-belief hypothesis, set relative to the first-order 9.10 weights, with the hard constraints that a witnessed-access weight strictly exceeds a said weight so a contrary assertion is out-ranked and lies are seen through, and that a modelled observation of a target's access never exceeds the same observation taken first-hand); the meta commit threshold and runner-up margin (basis: the balance of over-attributing a belief the target does not hold against leaving the target unknown too often, plausibly wider than first-order because second-order evidence is noisier, set so the false-belief and deception battery commits rather than merely failing to match projection); the meta log-odds clamp (basis: the maximum admissible certainty about another mind, no higher than the first-order clamp, since one cannot know another's mind better than one's own world); the lie-discount and the misperception discount as functions of acuity and modelled trust, staged from the previous phase (basis: how sharply a mind reads another's reliability, grounded in the Part 25 acuity scale and the trust field here); the common-ground prior weight, the per-public-event publicity-salience threshold, and the reach window (basis: the familiarity and diffusion signals 9.11 and 9.14 already use, with common ground scoped per witnessing set and to co-witnessed physical events only, and shared-but-not-common held distinct from common); the projection-fallback coefficient if it is admitted at all (basis: the egocentric curse-of-knowledge bias magnitude scaled down by stance, held strictly below the lead needed to commit on its own and tagged, with common-ground-or-unknown the safer default); the depth bound and its acuity-keyed selector (basis: Dunbar's orders of intentionality as the upper anchor lowered for cost, and the cost of nesting, a performance-and-realism bound, with the steering constraint that it is a capability parameter and never a per-race scheme-enabler); the level-of-detail gating policy, the relationship-salience thresholds, the per-mind breadth cap, and the eviction hysteresis (basis: the per-tick theory-of-mind cost budget against the marginal social value of a modelled relationship, mirroring the 9.7 belief restriction, with an active schemer and mark held promoted while the scheme is live); the conserved second-order projection, its demotion fold, and its promotion reseed (basis: the Part 54 monotone curve and id-ordered fold, conditioned so the population mean reconstructs the pool's second-order knowledge level); the meta-facet decay (basis: confidence in another's belief is more perishable than confidence in a fact, so it decays faster than a first-order facet); and the trust update on a committed deception verdict and its mapping to reduced source-weight (basis: the reputational decay the design wants for betrayal to matter politically, tied to the Part 28 source-weight scale). The reserved list is in the audit log. The honest limits stand: the divergent per-mind meta-belief is conservation-not-identity across demotion and is lost unless the parties stay promoted; meaningful evidence-driven depth is bounded more by evidence supply than by competence, since second-order access evidence is a third-order observation a mind rarely holds, so depth-three frames are evidentially starved in a settlement-scale world and the bound buys headroom more than routine use; and the consumers' use of modelled intentions (the `Intent` content the model also carries) is a separate concern from this belief-update rule and is governed by the utility AI (Part 8).
...
pub struct WorldDefinition {              // the complete moddable starting state
    ...
    pub access_channels: Vec<AccessChannelDef>, // second-order access relations and the target-belief hypotheses they imply, for recursive theory of mind (Part 37)
...
3. The convergence audit. As a standing test in the spirit of the determinism harness, the engine runs many independent worlds from different seeds under identical physics and measures where cultures converge and diverge in artifact morphospace. The invariant is that convergence must be explained by physics, cultures converging only where physics forces a single answer, and diverging in name, form, and method everywhere physics underdetermines it. Convergence that physics does not force is the signature of a designer-baked attractor, and the audit surfaces it. This mechanism is empowered to override the designer, the author of this document included, because the entire value of the system is lost the moment a human vision is pressed onto the world in place of what the world would have found for itself. A sibling invariant covers communication modality (R-LANG-MODALITY): paired worlds identical but for a race's communication channel swapped to a different modality of equal cited physical capacity must show invariant distributions of language richness, cross-cultural exchange, and cultural trajectory, so a difference that survives an equal-capacity swap is a designer-baked modality attractor the audit surfaces, while a difference a real capacity gap forces is allowed as physics. A further sibling invariant covers dialogue pragmatics (R-CONVERSE): paired worlds identical but for a permutation of the etic force decomposition, the move-kind labels, and the felicity-condition encodings, holding the underlying affordances and the belief, theory-of-mind, value, and opinion kernels fixed, must show invariant distributions of belief-transmission fidelity, persuasion success, deception detection, and coordination convergence, so a move's effect on a hearer is a pure function of the affordance it realises and never of its label, register, or any felicity weight, and any persuasive or fidelity difference that survives the permutation is a designer-baked pragmatic attractor the audit surfaces. A channel-swap invariant extends the modality-swap test to dialogue, requiring an equal-capacity change of the channel a conversation runs over to leave turn-taking, uptake, repair success, and persuasion invariant, so a privileged medium is surfaced while a difference a real capacity gap forces is allowed as physics. The content gate accordingly classifies every dialogue-move kind and felicity condition as an affordance or an outcome and refuses any that sets a graded persuasion or fidelity weight.

The red and green audit lifted this mechanism, because the audit's own metric and the authored physics share one coordinate basis, so a steering leak inside the physics would read as physics-forced and a single morphospace clustering measure is a necessary screen rather than a sufficiency proof. The audit therefore runs additionally under a basis-independence test, repeating the convergence measure under permuted and randomized observable bases drawn from the same physics tier and requiring the set of physics-forced convergences to be invariant, since a morphospace built on incommensurate, non-metric axes makes occupation patterns a parameterisation artifact (Gerber 2017). It runs under a law-form-competition test, carrying the alternative cited closed form of each interaction law and requiring the attractors to agree where the regimes overlap, so an attractor that depends on one chosen functional form rather than on physics is surfaced. The morphospace binning is a read-out of each axis's fixed-point resolution rather than an analyst choice, and the verdict must hold across a counter-RNG bin-offset sweep, because the convergence statistics in use can otherwise report spurious convergence (the cautions of Speed and Arbuckle 2017 and Grossnickle et al. 2024). Prerequisite depth needs one further probe, because an edge with a real forcing dependency can still be a conception-bias ladder that a recurrence test passes clean, so the audit adds a negative-space probe over reachable-but-unconceived designs and a conception-layer probe on the diversity of conceived intents. The specifics, the basis equivalence class and permutation count, the form-perturbation family, the seed count and significance margin, the binning bounds, and the probe thresholds, are reserved (the R-DEEPTECH-PHYSICS, non-templatedness, and R-DEEPTECH-DEPTH decisions in the audit working notes), and the deeper open siblings remain open.
...
**The biology-and-composition substrate floor (R-PHYS-BIO, wave 0 of the physics substrate).** The first domain of the physics substrate (Part 58) to be populated is biology and composition, the floor the food web reads (Parts 16, 17), the body is built from (Part 35), and edibility, nutrition, and toxicity are measured over rather than stored as flags. The same berry is food to one race, poison to another, and medicine to a third because the law reads two different consumer vectors, not because the berry carries a verdict. Composition is per-tissue and tiered: a Tier-0 gross-composition floor (water, gross energy density, protein, lipid, non-structural and structural carbohydrate, mineral ash, and a mechanism-neutral coarse toxin load), resolving by demand-closure into Tier-1 essentiality registries (the indispensable amino acids, essential fatty-acid classes, mineral elements, and a terran-chemistry toxin-class registry) and Tier-2 fine constituents, with every partially-resolved lump carrying a residual child so the resolved children plus the residual equal the parent exactly. The axes are registry points over the Part 58 substrate, never a closed struct of named fields. The consumer half is a per-race, heritable (Part 25) extensible registry of relation kinds (demand: requirement, upper bound, synthesis capacity; access: assimilation, fermentation, retention; harm: detoxification, sensitivity, reference tolerance, and the integer dose-response exponent), so digestibility, essentiality, and counter-adaptation are the consumer's data and never a property of the food.

Three closed-form fixed-point interaction laws contract the two halves. Net nutrition is the Liebig minimum across required classes of a per-class satisfaction (absorbed supply over requirement, formed with the checked division so an abundant supply against a tiny requirement saturates rather than wraps), digestibility carried entirely by the consumer's assimilation and fermentation with no food-side digestibility scalar. Harm is the per-class integer-Hill dose response: the dose ratio is formed with the checked division so a ratio beyond the representable range routes to the maximum-harm limit, the saturation guard is evaluated before the power so the power never overflows, the saturation ratio is keyed to the overflow-safe ceiling of the power, the dose-response exponent is per-(toxin-class, consumer), and the per-class harms combine by the order-independent saturating sum and a cap. Edibility is the measured tuple of net nutrition, net harm, an aggregate safety margin (the checked ratio of aggregate tolerance to aggregate dose, saturating to its cap), and a relational medicinal value (the share of nutrition that fills a class the consumer is deficient in, attenuated as harm rises, with no substance-side medicinal map). The law emits no value-laden verdict: the gain-versus-danger weighting is an agent valuation in the decision and drive layer (Parts 8, 20), heritable and selectable, so the one authored layer carries no risk attitude, and any food-or-poison-or-medicine label is a strictly read-time band over the tuple, never stored and never a law input. The kernels reference only axis indices and the two parties' values, so per-race outcomes differ purely from data.

The Tier-0 floor is complete-enough by a demand-closure criterion: the first build proof's food web (the Stage-14 convergence proof) can be made fully relational, every eater-source pairing computing its tuple through an axis the floor already carries with no pairing needing an absent axis, the matter-eating scope stated and the field-and-gradient feeders (photovore, thermovore, chemotroph and lithotroph, mana-feeder) excluded by construction, which is what licenses deferring the source-vector generalization. The cross-tier invariant is scoped to additive mass-fraction quantities: a lumped parent equals the sum of its children for the quantity, so a lump reader and a children reader never disagree on the quantity, but a nonlinear law output over those quantities (harm through the Hill kernel, available energy through per-consumer assimilation) is tier-resolution-dependent by construction, the coarser physics of the aggregate tier, which couples to R-TIER-CONSIST and registers the additive quantity, never the law output, as the conserved projection (Part 58).

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general across organisms and physics and per-race differentiable rather than fitted to one biota. The biology-and-composition floor populates the physics substrate (Part 58): composition is a tiered, per-tissue vector of registry points (Tier-0 gross axes resolving by demand-closure into Tier-1 essentiality and toxin registries and Tier-2 fine constituents, each partial lump carrying a residual child for exact additivity); the consumer half is a per-race heritable, extensible registry of relation kinds; and three closed-form fixed-point interaction laws (net nutrition by the Liebig minimum, harm by the per-(class, consumer) integer-Hill dose response, edibility as a measured tuple) report measured consequences, with the gain-versus-danger valuation moved out of the substrate to the agent layer (Parts 8, 20) so the authored floor carries no risk attitude. The mechanism is fixed Rust; the axes, the toxin and nutrient and relation-kind registries, the substance and consumer vectors, and every threshold are data (Principle 11), and the kernels are race-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: each axis fixed-point range (basis: the natural concentration and dose envelope the cited food-composition and toxicology literature defines, set at or just above the documented maximum); the per-quantity scale of each axis (basis: the smallest physiologically meaningful value the axis must resolve above the Q32.32 floor, an additively-summed axis sharing its parent's scale and an individually-read axis carrying its own); the consumer physiology vectors and their founding-distribution variance (basis: the published figure for an anchoring taxon scaled allometrically, the spread the observed within-taxon range or deferred to Part 25); the per-(toxin-class, consumer) integer Hill exponent (basis: the observed sigmoidicity for the class and consumer); the per-exponent saturation ratio (basis: the minimum of the within-one-epsilon ratio and the overflow-safe ceiling of the power, a hard determinism bound, the cube safe to 1290 and the square to 46340 at Q32.32); the per-class harm cap and the margin cap (basis: the per-class and aggregate ceilings); the relational medicinal-credit form and the reserved hormesis curve (basis: the consumer's own deficiency state, the biphasic shape a modelling choice reserved rather than copied); the allometric scaling exponents (basis: the comparative-physiology literature, an authored Principle-9 affordance or a per-race datum); the tolerance-zero direction (basis: the not-applicable-versus-infinite-sensitivity reading, the chosen direction not silently biasing missing data toward edible); and the relocated risk weights with their founding prior (basis: the consumer's heritable risk tolerance, the prior surfaced so a 1:1-centered default does not reinstate an authored attitude). The reserved list is in the audit log. The honest limits stand: the floor assumes a matter-eating metabolism, so field-and-gradient feeders need the deferred source-vector generalization (R-SOURCE-VECTOR); the nutrient and toxin registries are seeded from terran chemistry and carry an unbroken circular genesis channel the registry-population Steering Audit must broaden (R-BIO-REGISTRY); a nonlinear law output is not tier-invariant, an honest cross-tier difference rather than a leak; and the reach of edibility, nutrition, and toxicity equals the reach of the axes the substrate carries. This resolves the biology-and-composition domain (domain 7) of the physics substrate as a partial advance of R-DEEPTECH-PHYSICS, which stays open for the deepening tiers, the other catalogue domains, and the per-domain Steering Audit; R-BIOSPHERE (species generation over this floor) is the distinct reader.

> Needs research, item R-BIO-REGISTRY in the research backlog. The biology floor (R-PHYS-BIO, above) seeds its nutrient and toxin class registries from terran chemistry (the indispensable amino acids, the essential fatty-acid classes, the terran plant-defense toxin classes) anchored to a terran taxon scaled allometrically, and the demand-closure rule that grows the registries carries an unbroken circular channel: the axis set bounds what a race can need while what races need is supposed to define the axis set. A registry-population session and full Steering Audit should specify a genesis seeding that does not privilege terran biochemistry, or state terran monomer chemistry as an authored Principle-9 affordance with its accepted bound on expressible alien metabolisms, and decide whether the Tier-0 toxin floor and the allometric scaling law are affordances or per-race data. Couples to Parts 16, 17, 20, 25, 35, 41. Flagged, not changed.

**The mechanical-and-materials substrate floor (R-PHYS-MECH, wave 1 of the physics substrate).** The second domain populated is mechanics, materials, and energy-and-thermal, the floor the first technology-convergence proof (Stage 14) selects against and the floor Parts 19, 35, 43, and 45 read. It is one unified registry over the Part 58 substrate, a single quantity axis at one canonical scale per physical quantity, organised into a shared mechanical group (force, mass, velocity, contact and cross-section area, section modulus, arm length, edge radius, the friction and restitution coefficients), a bulk-material group (density, indentation hardness, the separate yield, tensile, compressive, shear, and fracture strengths, elastic modulus, fracture energy, ductility, Poisson's ratio, the wear coefficient), an energy-and-thermal group the energy sub-domain owns (conductivity, specific heat, the separate melting, boiling, and ignition thresholds, thermal expansion, latent heat, fuel value, oxidiser demand), and a shared gravitational-acceleration axis. The material axes subsume the Part 19 `MaterialProps` struct as registry points rather than extending a closed struct, hardness carried as an indentation pressure and edge sharpness as a capacity radius so neither smuggles a ranking. Functional outcomes (chopping, penetration, reach, structural collapse, fire spread) are measured law outputs, never stored fields.

The laws are closed-form fixed-point and race-blind: contact pressure and cut depth (the cut staged so the energy-per-depth product is never formed before the divisor reduces it), bending and axial stress with per-face tension and compression and the collapse margin, the dual stress-and-energy fracture criterion, Euler buckling (the slenderness square guarded before it is formed), direct and torsional shear, Hertzian curved contact, Archard wear, impact (kinetic energy formed half-first and a reduced-mass impulse), the lever (mechanical advantage as the arm ratio directly so an equal-arm lever reads unity), friction force and slip and loss and efficiency, additive reach, the weight law that forms structural load from mass and the shared gravity axis, mechanical power as force times velocity, and the energy-and-thermal set (Fourier conduction, sensible heat, phase change, combustion with its ignition crossing and limiting reactant, and constrained thermal stress reading the shared fracture strength). Crush and pierce are the same blow at different contact geometry, so a mace and a spiked morningstar are one conceived design the laws resolve differently rather than two authored types. The determinism discipline is uniform: the checked arithmetic routes an out-of-range product or quotient to its physical limit rather than wrapping, the saturation guard is ordered before the operation it protects, the pressure-class axes share one canonical scale, and reductions are order-independent.

The Tier-0 floor is complete-enough by the same demand-closure criterion as the biology floor: the first technology-convergence proof can evaluate its conceived artifacts (an edge cutting wood, a hafted weapon striking a body, a beam under load, a lever-and-friction transmission, a fire spreading by contact conduction, a smelting energy budget) through axes and laws the floor carries, with the named deferrals stated rather than faked.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general across materials and physics and per-substance differentiable rather than fitted to one material set. The mechanical-and-materials floor is one unified registry over the Part 58 substrate: about thirty-three quantity axes at one canonical scale per quantity (a shared mechanical group, a bulk-material group subsuming the Part 19 `MaterialProps` struct as registry points, an energy-and-thermal group owned by the energy sub-domain and read cross-domain, and a shared gravitational axis) and about twenty-two closed-form fixed-point interaction laws reporting measured consequences. Crush versus pierce, chopping, penetration, reach, structural collapse, and fire spread are law outputs, never stored, so a mace and a morningstar are one design resolved by contact geometry. The mechanism is fixed Rust; the axes, their ranges and per-quantity scales, the substance vectors, and every law constant are data (Principle 11), and the kernels are race-blind and material-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the one canonical pressure scale shared by every pressure-class axis (basis: the scale that holds the elastic-modulus range with headroom, the megapascal, the owner's pin for the pressure scale (the wider unit-system pinning lives in the resolved R-UNITS-PIN, record 62.25)); the per-quantity scale and range of every axis (basis: the cited datasheet or property-chart maximum, set at or just above it); the law output caps and the storable output scales and their unit-bridge constants for energy, torque, and power (basis: the largest value over the reserved input envelope, below the representable ceiling); the gravitational acceleration (basis: the terran 9.81 with a per-world override); the modelling factors, the crack-front geometry factor and the independent shear strength for anisotropic and brittle substances (basis: a chosen geometry and a reserved per-substance datum); the combined-modulus form and Poisson's ratio for curved contact and a contact-fatigue limit if a consumer reads one (basis: the elastic relation and a deferred fatigue datum); the wear coefficient's scale and the structured representation its six-order range needs; and the exotic and fantasy-reserved slots, each bounded by a cited maximum below the representable ceiling. The reserved list is in the audit log. The honest limits stand: the floor models rigid mechanics, so a chain flail's flexible-link dynamics and rotation in general are approximated through linear rim velocity and arm ratios with no angular-velocity axis; ballistic projectile range, buoyancy and hydrostatics, radiant and convective heat, and heat-to-work for engines are named reach ceilings taken in wave 2 and the thermal deepening; continuous-rotation fatigue, the curved-contact combined-modulus detail, the wear-coefficient split-range, the multi-layer-strike residual-energy bookkeeping, and the soft-tissue cutting-energy basis are surfaced refinements; and the reach of the floor equals the reach of its axes and laws. This resolves the mechanical-and-materials domain (domain 1) of the physics substrate as a second partial advance of R-DEEPTECH-PHYSICS, which stays open for the deepening tiers, the other catalogue domains, and the per-domain Steering Audit; the Part 55 dimensionless Ratio class and the derived-monomial registrations the floor's axes need couple to R-UNITS-PIN (now resolved, record 62.25).

**The fluid, chemistry, and optics substrate floor (wave 2 of the physics substrate).** The third slice populates the classical continuum-and-field floor: fluids (the axes and laws R-FLUID, R-WEATHER, and the body arc's convective exchange read), chemistry (the reaction, corrosion, and dissolution substrate metallurgy, R-WOUND corrosion, and medicine stand on), and optics (emission, transmission, the light field of Part 5, and the perception gating of R-SENSORIUM). All three are registry points over the Part 58 substrate, extending the wave-1 floor rather than reopening it, and all read the two axes the owner signed off: one canonical density axis whose range is widened down to the lightest simulated gas, so atmospheric buoyancy and wind are expressible, and a canonical `therm.temperature` state axis owned by the energy sub-domain and read cross-domain, which grounds the wave-1 conduction and combustion temperature inputs and gives the body its two-sided band. Wave 2 absorbs the reach ceilings wave 1 named: hydrostatics and buoyancy, convective and radiant heat, the ideal heat-to-work Carnot limit, and the dynamic-pressure force family (drag and aerodynamic lift, the same measured law differing only in the coefficient), so a wing, a gliding creature, a sail, and a ballistic arc with air resistance are reduced-order lumped-coefficient laws. A consumer-lens completeness pass folded the weather water cycle (a moisture state axis, an affine saturation-vapour-pressure proxy, a bidirectional evaporation-and-condensation law, thermal buoyancy, and the ideal-gas density coupling that lets temperature drive the density field), acoustics (the speed of sound through the medium, and frequency, source-power, and absorption axes with acoustic impedance and transmission, the physical carrier the acoustic communication channel and hearing read), and colour (a data-defined spectral-band vector with a receptor-weighted response and a chromatic-contrast discrimination, so leaf-green, warning-red, and camouflage are expressible and colour vision emerges per race). The same pass confirmed the wave-1 mechanical-and-materials floor needs no addition: the bow's elastic energy, torque and gearing, and self-heating all compose from the existing elastic-modulus, lever, power, and sensible-heat laws. Every kernel is closed-form integer over the Fixed operations, staged against the Q32.32 ceiling, with no float in canon; the fourth-root radiative-equilibrium temperature is two nested integer square roots rather than a materialised fourth power. The transcendentals stay deferred to R-GPU-CANON-PIN, named rather than smoothed.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general across fluids, substances, and light and per-substance differentiable rather than fitted to one set. The wave-2 floor extends the Part 58 registry with the fluids, chemistry, and optics domains: about thirteen fluid axes and fifteen fluid laws (hydrostatic and dynamic pressure, buoyancy, drag and lift, Poiseuille flow, Reynolds regime, Laplace pressure, compressibility, convective flux, speed of sound and the acoustic pair, and the weather water-cycle laws), about nine chemistry axes and four laws (a Hess-enthalpy reaction, corrosion, the Carnot limit, and dissolution), and about twelve optics axes and nine laws (radiant emission, the Wien peak, inverse-square falloff, optical depth, refractive contrast, the interface split, radiative-equilibrium temperature, and the colour stack), all reserved-with-basis and cited. Two axes the owner signed off shape the floor: the single density axis widened to gases and the canonical `therm.temperature` state axis; the energy-per-mass, activation-barrier, and megapascal scales are unified once. The mechanism is fixed Rust; the axes, their ranges and per-quantity scales, the substance vectors, the data-defined spectral bands and per-race receptor sensitivities, and every law constant are data (Principle 11), and the kernels are substance-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the widened density lower bound (basis: the lightest simulated gas), the fluid property axis ranges (viscosity, surface tension, bulk modulus, the drag and lift coefficients, the convective coefficient, the channel and column and pressure scales; basis: cited datasheet maxima), the saturation-vapour slope and reference and the evaporation coefficients (basis: the psychrometric tables and Penman-Dalton pan calibration), the reaction, corrosion, and dissolution constants and the Carnot form (basis: the thermochemical and electrochemical tables), the optics axis ranges and the Stefan-Boltzmann and Wien constants and the reserved sigma scale (basis: the radiometric constants at the Q32.32 precision floor), the data-defined spectral-band edges and per-race receptor sensitivities and discrimination thresholds (basis: the CIE partition and opsin sensitivity curves), and the representability caps on every law. The reserved list is in the audit log. The honest limits stand: the transcendental forms are deferred to R-GPU-CANON-PIN (the exact Clausius-Clapeyron vapour curve, the Arrhenius reaction and Tafel corrosion rates, angle-resolved Snell refraction and the exp Beer-Lambert transmittance for both light and sound, and Rayleigh scattering), so wave 2 folds their Section-8-sanctioned piecewise-linear or lumped-coefficient proxies; the saturation-vapour proxy holds only within about twenty kelvin of its reference; terminal velocity, precipitation-as-fall, and osmotic and stoichiometric solution chemistry are per-tick integrations or wave-3 deepenings; turbulence and the drag polar stay the permanent categorical reach boundary. This resolves the fluids, chemistry, and optics domains (domains 4, 5, 6) of the physics substrate as a third partial advance of R-DEEPTECH-PHYSICS, which stays open for the deepening tiers (electricity and magnetism next) and the per-domain Steering Audit; the transcendental deferrals ride R-GPU-CANON-PIN (resolved, record 62.23; its transcendentals are now pinned, so authoring the exact forms from the folded proxies is a follow-on) and the unit pins the resolved R-UNITS-PIN (record 62.25).

**The electricity-and-magnetism floor (wave 3 of the physics substrate, R-PHYS-W3).** Wave 3 is the first discoverable deepening tier rather than a Tier-0 floor: a people does not begin knowing that charge separates or that a moving magnet drives a current, it discovers electricity and magnetism through its instruments and theory, so the tier opens in three physics-forced rungs (electrostatics from charge-separation instruments and the inverse-square, current electricity from the pile and the systematic voltage-current measurement that yields Ohm's law, then magnetism and induction from the lodestone, Oersted, and Faraday), each gated by the culture's instrument-and-theory state crossing a discovery threshold that is data, never a per-race unlock table (Principle 8). The wave-2 electrochemistry is the natural discovery pathway: a culture that observes galvanic corrosion between two dissimilar metals already holds the seed of the pile, so the wave-2 corrosion driving margin and the wave-3 battery EMF are the same physics at two tiers. This wave forces one change on the locked representation, taken as an audited Principle-9 act: electricity is the first quantity dimensionally independent of length, mass, time, and temperature, and the Gaussian alternative that avoids a new base gives charge a half-integer dimension the integer monomial cannot hold, so the `Dimension` type gains a fifth base, electric current (the SI ampere, Part 55). The change is backward-compatible and neutrality-preserving: every earlier axis keeps its meaning with a current exponent of zero, so the Steering-Audit neutrality test still holds by construction, now over five bases. The floor adds the electricity axes (charge, current, potential, the source EMF, resistance and its geometry-independent resistivity, capacitance, and the field) and the magnetism axes (flux density, the resident flux state, permeability, magnetic moment, inductance, and the coupling coefficient) with their closed-form kernels (Coulomb's inverse-square with the sign tracked, Ohm and the circuit current whose short is a routed extreme, Joule dissipation that feeds the wave-1 sensible-heat law so a wire heats and then glows by the wave-2 Wien and radiant laws, the capacitor and inductor field energies, the galvanic cell EMF that promotes the wave-2 electrode-potential proxy to its real volt, the solenoid field and the motor and Lorentz forces, the dipole torque envelope, and Faraday and inductive EMF as a per-tick flux and current delta signed by Lenz). Full Maxwell wave propagation, the AC frequency domain and the tuned resonant circuit (the radio gateway), the dipole angular factor, and the concentration and rate transcendentals stay deferred to wave 4 and R-GPU-CANON-PIN, named rather than smoothed.

> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general across conductors, dielectrics, and magnets and per-substance differentiable. The wave-3 floor is a discoverable deepening tier (tier 2) that extends the Part 58 registry with the electricity and magnetism domains: eight electricity axes and seven laws, six magnetism axes and eight laws, all reserved-with-basis and cited, all closed-form integer over the Fixed operations and staged against the Q32.32 ceiling. The owner signed off the load-bearing representation change (S-ELEC-DIM): the `Dimension` monomial gains a fifth base, electric current (the SI ampere), because electricity is dimensionally independent of the four earlier bases and the Gaussian half-integer alternative is unrepresentable; every earlier axis is unchanged at current exponent zero, so neutrality-by-construction is preserved over five bases (Part 55). The wave-2 `chem.standard_potential` is promoted from its dimensionless proxy to the volt, unified with `elec.potential` and `elec.emf`, dimension-and-scale only with no numeric re-fit. `elec.potential` and `elec.emf` are kept role-distinct on the wave-2 pressure-role precedent, an overlap the owner may collapse. The mechanism is fixed Rust; the axes, their ranges and per-quantity scales, the substance vectors, the coil construction data, and every law constant are data (Principle 11), and the kernels are substance-blind. What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the electricity axis ranges and per-quantity scales (charge, current, potential, EMF, resistance, resistivity regime-windowed over its twenty-four-order envelope, capacitance, field; basis: the cited CRC, Griffiths, Kittel, and datasheet maxima), the magnetism axis ranges and scales (flux density, flux, permeability, moment, inductance, coupling; basis: CRC, Griffiths, Jiles, Fink-and-Beaty), the law output caps (V, I, R, B, PHI_MAX and the reused F, E, POWER, TORQUE_MAX; basis: the representable ceiling over the reserved envelope), the Coulomb coefficient on a reserved x1e9 output scale and the vacuum permeability MU_0 (basis: the CODATA constants at the Q32.32 precision floor, the SIGMA precedent), the coil turn count and turn density (basis: device construction geometry), and the tick duration DT the induction finite difference divides by (basis: the engine base tick). The reserved list is in the audit log. The honest limits stand: the induction laws are a first-order finite difference over a resident flux or current state, so their fidelity is tick-rate-dependent; the dipole torque is the perpendicular-orientation envelope with the sin factor deferred; the solenoid field is linear-permeability with the B-H saturation loop deferred; and full Maxwell wave propagation, the AC frequency domain and the tuned LC resonant circuit (radio), and the Nernst, Arrhenius, and Tafel transcendentals are deferred to wave 4 and R-GPU-CANON-PIN. This resolves the electricity and magnetism domains (domains 7 and 8) of the physics substrate as a fourth partial advance of R-DEEPTECH-PHYSICS, which stays open for the discovery-mechanism coupling (R-DEEPTECH-SCIENCE), wave 4, and the per-domain Steering Audit.
...
> Decided and reserved. The mechanism is settled and signed off, and on the owner's condition it is general, per-culture differentiable, and data-driven. The number system is two tiers, the canonical Q32.32 `Fixed` unchanged plus a scale-closed integer tier over an eight-limb wide accumulator rounded once per law-chain, sized by a load-time typed-op-graph (`LawExpr`) scale planner, with a type-level divide-and-log physical-floor invariant on the run-path sites (Principle 10, observer independence), a closed CODATA fundamentals table and a float-free composite compute (a deterministic bignum and rational evaluator, pi by a Machin series) that derives a constant such as the Stefan-Boltzmann sigma, and the flagship byte-neutral sigma lift. The emic (cultural) measurement layer crosses to the absolute base exactly: the unit factor is an exact rational so it contributes no approximation, each conversion is a single terminal round, storage is canonical-absolute so an emic reading is a view that never ratchets, and a `StatedQuantity` exact-rational carry keeps a culture-stated quantity perfectly reversible until it must enter the fixed-point physics path. The per-culture unit store is the id-ordered `MeasurementSystem`, so a canonical walk over it is observer-independent (the units-local instance of R-CANON-WALK), and a unit's provenance is an open data-driven reference rather than a closed enum, so a people measuring by a forearm or by a mana tide is a data row. The mechanism is fixed Rust; the scales, the declared physical floors, the cultures' units and their exact-rational factors, and the provenance kinds are data (Principle 11). What is reserved for your calibration, surfaced rather than fabricated, with its basis given: the composite significance target and guard (the fixed-point scale family that resolves a CODATA composite's roughly ten significant figures); the per-law intermediate widths (each sized from the law's measured exponent interval); and the declared physical floors and limits-at-zero (each a per-world value below which the quantity is absent, derived from the physics where a material or law datum supplies it, surfaced as an owner value only where the physics cannot). The emic layer needs no new reserved value: the conversion factor is per-culture data and the scales are the existing reserved envelope. The reserved list is in the audit log. The honest limits stand: the emic round trip is exact for every value the absolute scale resolves and within one absolute epsilon at the one boundary where the epsilon is coarser than the emic step, a deterministic storage floor stated rather than hidden; the emic layer is built as substrate and not yet wired to a live per-culture measurement, which is a Part 40 content arc; software float (a per-value mantissa and exponent) is specified but not built, reserved for a quantity whose runtime range cannot be declared, which the declare-your-bounds principle prevents; and the byte-neutral flagship lift is a real precision-correctness improvement whose effect the current pinned scenarios do not surface, so the arc moved no canonical hash. Couples to Parts 3, 9, 40; the R-CANON-WALK cluster stays open for its other containers.
...
> Needs research, item R-AGENT-EXEC in the research backlog. The agent execution model, the second foundation above, is scoped in `docs/event_driven_execution_research.md` and should be read before any build. The findings: moving from the poll-everything tick loop to event-driven wakeup is an extension rather than a rewrite, because the engine already carries the pieces: the tick applies a total-canonical-order stimulus batch (a proto event queue), the `DrawKey` schema keys each draw on its coordinate so randomness is independent of when an event runs, and the co-location index is the spatial subscription. An idle agent sleeps and is woken two ways, both from resolved machinery: a self-scheduled wake at the tick a draining homeostatic reserve (R-BEHAVIOR-EVOLVE) crosses its action threshold, solved in closed form rather than polled; and a reactive wake delivered by the place-and-percept index when something happens near it. The reproducible wake order is R-CMD-ORDER's total key (tick, region, id, phase, ordinal) applied to the time axis, which the external literature confirms is the field-standard answer to simultaneous-event reproducibility (the alternative being Parallel DEVS's confluent transition function); the event queue is a solved O(1) structure (calendar queue, ladder queue); and determinism across threads is the deterministic-scheduler-and-draw-key discipline applied to the woken subset. The honest limit, the breadth twin of the temporal fast-forward bound: event-driven execution removes the idle-agent tax and nothing more, so a densely interacting crowd where most agents act every tick is bounded by its own activity and the engine falls back to a flat sweep above a reserved crossover. The load-bearing coupling: the agent wake queue and R-TEMPORAL-LOD's conservative cross-region synchronization core are the same event-queue substrate at agent and region granularity, so the two must be built to one design. Prerequisites a build depends on: R-CMD-ORDER (the total key the queue orders by), the deterministic scheduler (this part's first foundation, which orders the woken subset), and R-HARNESS-COVER (proving an event-driven region agrees bit for bit with the full sweep). Reserved values are surfaced with their bases in the scoping pass (the dense-region crossover and the re-check interval for a non-closed-form wake condition). Couples to R-TIER-CONSIST (the coarse tier an idle region collapses into) and R-BEHAVIOR-EVOLVE (the drives whose predictable drain supplies the self-scheduled wakes). Flagged, not changed.

> Needs research, item R-REDUCE-ORDER in the research backlog. Beyond the scheduler and the agent execution model flagged above, the engine has several canonical combines that are not associative and whose order is unpinned: the gossip sub-batch that applies conflicting writes to one listener (9.5), the weighted pick over a candidate list whose source order is not fixed (Part 9), the migration blend's renormalisation (Part 25), the technology evaluation's product of efficiencies (Part 41), the coupled-stock stepping of ecology and climate (Parts 15, 17), and any traversal over the unified graph substrate (Part 58). A session should mandate a total `StableId` or content-id keyed order for every non-associative canonical combine, settled together with the deterministic scheduler's declared read and write sets, so the order a reduction runs in is a function of the data rather than the thread schedule. Determinism is the requirement. Couples to Parts 9, 15, 17, 25, 41, 58. Flagged, not changed.
...
Conservation and referential integrity, as first-class audited invariants. Promotion and demotion (Parts 11, 54) must conserve totals: the count of individuals plus the counts in the aggregate pools must always equal the whole, with no entity created or lost in the transition, or a world develops silent population and resource leaks that read as nondeterminism. What must be conserved is not a fixed list but a registry of conserved projections that each two-tier subsystem declares for itself, with population, resource stocks, wealth, the institution feature vector (Part 36), aggregate belief mass (Part 9), and the relationship-keyed second-order belief mass of recursive theory of mind (Part 37) among the present entries, so the harness enforces every declared quantity and a future two-tier system, a magic-field network or a disease model or a trade-route graph, is covered the moment it registers its own projection, with nothing special-cased to the entries that exist today (this is the conservation half of the tier-consistency guarantee, R-TIER-CONSIST, Part 54). Cross-store references must survive structural change: a belief, a history entry, an institution, a household, or an artifact that points at an entity must remain valid when entities merge or split, as when two cultures fuse into one or a household dissolves and its members and holdings disperse. These are invariants of the same kind as determinism and the steering audit, and they are enforced the same way, with a standing harness that checks conservation and reference validity across promotion, demotion, merges, and splits, run in continuous integration, so a leak or a dangling reference is caught the moment it is introduced rather than discovered as drift centuries into a world.

> Needs research, item R-PROJ-REGISTER in the research backlog. The invariant above says each two-tier subsystem registers the projection it conserves, but only population and wealth are registered today, and several fold-back operators are means rather than exact partitions: belief mass folded as an id-ordered mean of facet strengths cannot conserve a total and truncates under fixed-point division, and the genetics, ecology, language, and institution tiers imply conserved quantities that are not declared. A session should require every two-tier subsystem (belief, genetics, ecology, language, institutions) to register its conserved projection with an exact integer partition that assigns the remainder to the lowest id rather than a mean, define the merged-strength rule for the concept-snap case (Part 33), and add a continuous-integration assertion that the registered projection set matches the declared-present list, so a new two-tier system cannot ship without its projection. This makes the conservation half of R-TIER-CONSIST enforceable. Couples to Parts 9, 11, 25, 33, 36, 54. Flagged, not changed.
...
The physics-substrate representation, the one authored layer's data floor. The physical model Part 41 requires by its completeness requirement is carried as a foundational substrate of the same shape as the value, semantic, and institution-function substrates: a `QuantityAxis` is a named, unit-bearing, range-bounded, fixed-point scalar dimension at a tier; an `InteractionLaw` is a closed-form integer kernel over the quantity vectors of the participating entities that reports an interval-bounded fixed-point consequence, also tier-tagged; and a `Substance` (a material, a tissue, an organism's part) is a vector of values over the axes plus the laws it participates in plus a provenance tag, real-with-source or fantasy-reserved-with-basis. Everything is content-addressed with stable ids for determinism and memoisation. The axis set and the law set are a bounded but extensible floor, the structural analogue of the semantic primes (Part 33) and the composition leaf primitives (Part 41): a material is representable when its behaviour decomposes onto the axes the substrate carries, and a behaviour no axis carries requires a deliberate, audited Principle-9 floor extension rather than a silent one. Magic and exotic physics ride the same floor as extension axes (Part 34). Every quantity is fixed-point with a declared range and a per-quantity scale (Part 55), every law kernel is a closed-form integer function with an interval bound formed with the checked arithmetic that routes an out-of-range result to its physical limit rather than letting it wrap, and no float enters canonical state; a law that cannot be put in closed-form fixed-point with a bound is out of scope until it can be. This is the shared upstream of materials (Part 19), the body and wound model (Part 35), the food web (Parts 16, 17), the physics-grounded cluster (R-FLUID, R-WEATHER, R-BUILD-PHYS, R-CATASTROPHE, R-WOUND), the composition interface axes (Part 41), and the projected world-axes of value and meaning (Parts 21, 33), so its reach bounds theirs. The wave-0 biology-and-composition population of this floor is resolved (R-PHYS-BIO, Part 41 and record 62.16); the deepening tiers and the other catalogue domains remain the open R-DEEPTECH-PHYSICS.

> Needs research, item R-DIM-HOMO in the research backlog. An `InteractionLaw` declares its output `Dimension`, but nothing checks the declaration against the dimensional combination the kernel forms from its input axes, so a mistyped or hallucinated law can carry a physically-wrong output dimension that the base-monomial neutrality test still accepts (a `voltage` output on a law that multiplies a current by a resistance would pass today). A session should give each law a small symbolic formula over its input axes, a product-and-quotient expression the loader reduces to a dimension by the existing `Dimension` multiply and divide, and make a mismatch with the declared output a load error, so dimensional inhomogeneity is unrepresentable rather than a review miss, the make-it-unrepresentable discipline the axis monomial already applies (Part 55). The formula is data and the check is fixed Rust; determinism is unaffected. This is the deeper follow-on to the hardcoded anti-steering guards already built (the substance content-address label-blindness, the no-float-in-canon scan, the provenance-required load check, and the symmetric-kernel no-hidden-bias check, in `crates/physics/tests/steering.rs`). Couples to Parts 41, 55. Flagged, not changed.
...
### 62.11 Recursive theory of mind: the nested-model update without projection (R-TOM-UPDATE)

The question. Part 37 specified the mental-model structure, a nested belief store per modeled mind at bounded depth, and its consumers (deception, negotiation, and trust over the believed profile), but not the rule that populates and updates the nested store; as written it defaulted to projection, attributing the modeller's own beliefs or the ground truth to the target, which cannot represent a false belief and collapses under any false-belief or deception probe. The session had to specify how the nested store updates from second-order evidence so it provably diverges from projection and carries the Sally-Anne false-belief competence and the seen-through lie, what a fresh store assumes before evidence arrives and at the depth bound, how the gossip loop updates the hearer's model of the speaker alongside the hearer's own belief, how cost is bounded and conserved across promotion and demotion, and how deep the recursion can meaningfully and affordably go. The owner asked to confirm it is broadly generalizable, per-race differentiable, and data-driven, and specifically that it not lean on a stale closed enum or a fixed array of the beliefs that can ever exist.

What was found. A fan-out red and green audit (eighteen agents across eight dimensions with a projection-collapse adversary, recorded in audits/tom-update-redteam-2026-06-29.md) confirmed the substrate is the resolved evidence engine of 9.10 run recursively on whether the target believes a thing, fed by second-order evidence of the target's access, and that this passes the false-belief and believed-lie cases by construction. The false-belief literature grounds the test battery: Wimmer and Perner's unexpected-transfer task, the Baron-Cohen, Leslie and Frith Sally-Anne adaptation, and the Perner and Wimmer second-order ice-cream-van task, whose correct answer is by construction unreachable by any first-order shortcut. Depth is answered from two directions. The competence literature places the human ceiling near a fifth order of intentionality (Kinderman, Dunbar and Bentall; Stiller and Dunbar), with O'Grady and colleagues showing under a forced-choice paradigm that the reach extends to at least the seventh order so the fifth-order limit is partly a task artifact, while strategic reasoning concentrates at one to two steps (Camerer, Ho and Chong, a Poisson mean near one and a half); finitely-nested interactive POMDPs (Gmytrasiewicz and Doshi) terminate the tower at a level-zero sub-intentional model; and epistemic logic forbids true common knowledge (Aumann; Rubinstein's electronic-mail game), so the object at the bound is finitely-iterated common belief, defeasible. Bayesian theory of mind (Baker, Saxe and Tenenbaum) is the inferring-belief-from-access frame the integer engine approximates. The audit's load-bearing catch was that the anti-projection guarantee had no type in the design, and the catch on the audit's own proposal, surfaced under the owner's directive, was that its fix drew a closed six-kind second-order-evidence enum where the engine's own pattern is a data registry.

The decision taken, signed off and generalized. The update is the 9.10 inference engine run recursively, the meta-frame being the same InferenceFrame with its subject re-pointed to the target and its hypotheses over the target's belief-values, inheriting the integer log-odds accumulator with clamp-at-read, the order-independence, and the bit-identity of 9.10 and 9.15. The divergence from projection is a typed, audited admissibility invariant: every piece of evidence carries an order tag (world, or access of a named mind, the recursion-level discriminator, fixed mechanism parameterised by a stable id rather than a content catalogue), the nested store admits only access evidence about its target on the same subject and attribute, and the modeller's own world belief and ground truth are excluded by construction, enforced by the write path and a harness assertion. The first hardening, on the owner's condition and Principle 11, is that the membership of access-relation kinds is a data registry (Part 40, sibling to the trace-kind registry of 9.9), so the report's fixed six-kind enum becomes data and a new sense or medium of access is a data entry, with no closed set of second-order evidence and no fixed array of belief kinds; attributes and values are the data-defined identifiers the first-order engine already uses (9.8). Seeing through a lie is a third sincerity frame fed by the gap between an access-built meta-facet of the speaker's belief and the speaker's assertion. The gossip co-update writes two disjoint frames in a fixed phase order with a pre-write snapshot, on a partition re-derived over the listener and the modeled subject, the counter key extended by depth and subject and the conflict order pinned by R-REDUCE-ORDER. The fresh-store prior is relational common ground over the witnessing set, shared-but-not-common-aware and fed only by co-witnessed physical events; beyond an acuity-keyed depth bound (self zero, first nest one, nest of a nest two, two canonical and three for high-acuity minds) the model degrades to common ground and never to projection of an access-fed frame. Cost is level-of-detail-gated like first-order belief and bounded in breadth as well as depth, and a relationship-keyed second-order belief mass is registered as a conserved projection (Parts 54 and 58, under R-PROJ-REGISTER). The depth-bound selector is a capability parameter the Steering Audit holds to be acuity-keyed and never a per-race scheme-enabler (Principle 9), and acuity (Part 25) and epistemic stance (Part 28) differentiate every mind. This lives in Part 37, with the registry in Part 40 and the conserved projection in Parts 54 and 58.

What remains to be proven. The calibrations are reserved: the access-channel weights with their hard ordering constraints, the meta commit threshold and margin, the meta clamp, the lie-discount and misperception discount, the common-ground prior and publicity threshold and reach window, the projection-fallback coefficient if it is admitted, the depth bound and its selector, the level-of-detail and breadth policy, the conserved projection with its fold and reseed, the meta-facet decay, and the trust update on a caught lie. In-engine the standing battery must pass as a gate: Sally-Anne, a true-belief control as a hard gate against an over-correcting rule, unexpected-contents, first- and second-order false belief, a lie believed, and a lie seen through, with the pass criterion that the nested frame commits the literature-correct value and that value differs from projection. The honest limits stand: the divergent per-mind meta-belief is conserved as mass but not as identity across demotion, so active schemers and their marks must stay promoted while a scheme is live; meaningful evidence-driven depth is bounded more by evidence supply than by competence, since second-order access evidence is a third-order observation a mind rarely holds, so the depth bound buys headroom more than routine use; and the modelled-intentions content the model also carries is governed by the utility AI (Part 8) and is out of scope here. The nine load-bearing citations were verified against primary sources for the record, the O'Grady venue corrected to Evolution and Human Behavior and its claim phrased as a higher, task-dependent ceiling, and the interactive-POMDP decreasing-marginal-depth point marked a characterization rather than a verbatim result.

---

### 62.12 Pinning the order-sensitive and unbounded language procedures (R-LANG-DET)

The question. Part 33 settled the language cluster but left several of its procedures order-sensitive or unbounded: the order among regular sound-change rules and whether a generation's rules apply at once or in sequence, which changes the result under feeding and bleeding; a concept id formed by hashing content rather than minted, so it has no referential identity and can collide; a century-accumulating concept salience with no overflow discipline; the shared-concept traversal in the distance metric and the rounding of its aggregation; and the dawn dynamic's schedule trigger. The owner asked that the resolution be general, data-driven, and modality-neutral, so the still-open generalization to signed and non-vocal languages adds only data and reopens no determinism rule, and chose a deep focused session with two bounded adversarial rounds over a full fan-out.

What was found. Two rounds of adversarial verifiers, each pointed at the actual primitives and asked to refute, hardened a first-pass scheme into the final one. The substantive linguistic question, rule ordering, resolves to the historical model: a regular change is Neogrammarian and exceptionless, the rules of a lineage compose by relative chronology, and feeding, bleeding, and the opacity of counterfeeding and counterbleeding follow from order, which the generative-phonology record formalises (Chomsky and Halle; Kiparsky on the rule interactions and on opacity). The first round caught that the shared-concept set cannot key off per-culture minted ids, that salience as a naive fixed-point leaky integrator stalls below its target, and that a same-tick rule batch and the borrow-versus-pass interleave had no canonical order. The second round caught that the first-pass distance fix, a nearest-prototype region matching, is asymmetric and so wrong for a bidirectional intelligibility, that the round-half-even salience update still leaves a dead-zone, that three equal weights cannot sum to one in fixed point without a residual rule, and that the within-generation phase order omitted three operators. A dedicated modality pass confirmed the determinism keys off integer form-primitive indices and id-ordered walks rather than off a sound, so signed phonology, with its documented regular formational change (Stokoe; Brentari; Frishberg), fits the same mechanism, and that the distance metric must not collapse to its maximum on a modality difference.

The decision taken, signed off and generalized. Concept ids are minted through the registry with culture and birth event kept as provenance, a merge retires the absorbed id and folds its salience, and the cross-culture tiebreak is the lineage-minimal birth event. Regular form changes apply at once and compose in innovation-index order, same-tick rules sorted before append by a canonical content-and-coordinate key, the log append-only, the intra-form scan left to right, and a generation's operators run in a fixed phase order with language splitting last. Salience is a widened leaky accumulator narrowed only at read. Language distance keys off the shared semantic substrate by a symmetric, matching-free coverage walk, exactly as the Part 21 value metric projects to shared coordinates before measuring, with one rounding at the final narrowing, absolute feature differences, and weights normalised to one by residual absorption; a non-shared or cross-modality form substrate returns the reserved incommensurability ceiling while grammar still computes. The dawn dynamic fires deterministically on the culture-genesis event over the race's producible form inventory. The mechanism is fixed Rust; form-substrate membership, concept partitions, and rule content are data, and the pins are worded over the generic form primitive so R-LANG-MODALITY adds only data. The grammar typological parameters, found to be a closed enum one level down, were flagged as the new sibling item R-LANG-TYPOLOGY rather than changed here. This lives in Part 33.4 with the distance refinement in 33.5.

What remains to be proven. The calibrations are reserved: the salience decay rate with its no-underflow lower bound and the merge threshold above the salience floor, the usage-normalisation recency window, the substrate quantisation granularity reused for the walk and the tie window, the per-cell incommensurability contribution set to the existing floor, the no-shared-form ceiling (the optional distinct cross-modality ceiling once noted here was retired by R-LANG-MODALITY on a Steering Audit ground), and the three distance weights now constrained to sum to one. In-engine the gates are convergence of the salience accumulator to its target without the dead-zone, symmetry of the distance under argument swap, and bit-identical replay of a multi-culture drift and contact history across thread counts. The honest limits stand: the dawn dynamic coordinates a differentiated prime floor under a hard safety cap (33.9, record 62.24), divergent salience is conserved as mass but not identity across demotion, the modality generalization itself is R-LANG-MODALITY (since resolved, record 62.13), and the typological-parameter substrate is the open R-LANG-TYPOLOGY. The nine load-bearing citations were verified against primary sources, with the opacity reference pinned to Kiparsky 1973 in Fujimura's volume and the naming-game convergence to Baronchelli and colleagues.

---

### 62.13 Generalizing language to all modalities, signed and non-vocal (R-LANG-MODALITY)

The question. The resolved language cluster modelled one communication channel, the vocal-auditory one, and baked it into its types: a phonetic substrate of sound features, a producible-sound capability, a phonology of consonants and vowels and syllables, and a word form as a phoneme string. A signed language has none of those, its sublexical units being handshape, location, movement, orientation, and non-manual marking, and there was no per-being notion of separate production and reception channels that could differ or be lost. The owner asked to generalize modality to a first-class, data-driven, per-race-differentiable property so signed and non-vocal languages exist and emerge, an individual can lose hearing or voice by birth or injury, and a non-vocalizing race carries its whole language family in another channel, with the result adding only data so no determinism rule from R-LANG-DET reopens, and acquired injury-loss reserved against the now-resolved anatomy and wound model rather than invented ahead of it.

What was found. A five-facet research fan-out (the channel substrate, the articulation model, per-being channels, cross-modal contact and learning, and emergence across modality), each grounded in the literature, fed a synthesis that a three-skeptic verification pass then hardened (a closed-enum seam hunt, a Steering Audit, and a determinism non-reopen check). The determinism floor was already favourable, because R-LANG-DET had been worded over a generic form primitive precisely so this item would add only data. The fan-out established that signed phonology is modality-general and changes regularly (Stokoe, Brentari, Frishberg), that production and reception are separable physical stages with separate organs (Bradbury and Vehrencamp on the animal-communication channel taxonomy, with electroreception the concrete case), that bimodal speakers blend two channels at once rather than switch (Emmorey and colleagues), and that a full language emerges from a deaf cohort with no shared input (the Nicaraguan and Al-Sayyid Bedouin cases; Goldin-Meadow on homesign). The verification caught real defects: the seam hunt found the channel-derivation's medium join key shipping closed and a feature dimension's value set left implicit; the Steering Audit found two leaks, a homesign dawn cap reserved smaller for the modality path (authoring signed languages as thinner) and, auditing the prior resolution, the optional distinct cross-modality ceiling reserved in R-LANG-DET (authoring non-vocal isolation); and the determinism pass found the new modality-migration event and tier-fired dawns lacked a slot in the within-generation phase order, and the code-blend two-stream event lacked a canonical serialization order.

The decision taken, signed off and generalized. Modality is a first-class data property carried by four registries (production modalities, reception senses, feature dimensions with their value sets, and media), with a channel a derived production-and-reception pairing over a shared medium, recognised not enumerated, the institution-kind and access-channel move applied to communication. A form primitive is an integer-indexed segment that is a canonical bundle of simultaneous feature values, so the R-LANG-DET scan over the segment sequence and the same-tick canonical key hold unchanged. Each being carries congenitally-ceilinged produce and perceive capabilities derived from the resolved genome and anatomy through a data-defined capability query, with congenital loss falling out as an ordinary recessive phenotype and acquired loss a reserved one-directional clamp to R-WOUND (now resolved, Part 62.20). Cross-modal learning, the bi-modal interpreter and written-record bridges, code-blending, transmission distortion, and the homesign-to-community and modality-shift emergence are all compositions of the resolved machinery, with the one new in-world event, modality migration, disclosed as a future row in the R-EVENT registry. Every verification catch was folded: the medium became a fourth registry and the feature value set became data; the dawn cap became a modality-blind function of coordinating-unit size; the optional distinct cross-modality ceiling was retired in R-LANG-DET so cross-modal difficulty emerges from form giving no help rather than from an authored constant; the within-generation phase order was extended to place modality migration and tier dawns first; and the two-stream blend event was pinned to a channel-key-ordered serialization. The Steering Audit gained a modality-swap invariant, the analogue of its basis-independence test, requiring an equal-capacity channel swap to leave emergent richness and trajectory invariant. The mechanism is fixed Rust; the channel registries, the feature dimensions and values, the per-race anatomy-to-channel mapping, and the rule content are data. This lives in Part 33.3, with the Steering Audit invariant in Part 41 and the phase-order and ceiling reconciliation in Part 33.4.

What remains to be proven. The calibrations are reserved with their basis: the registry memberships and per-modality feature dimensions and values, the per-modality perceptual-distance geometry (its contrast budget a read-out of cited channel physics under the permuted-basis test, never a free weight), the capability-gate floors and curves, the comprehension-and-production acquisition split, the cross-modal blend propensity (a form-of-mixing knob, never a fidelity multiplier), the modality-blind dawn cap, and the channel-mismatch trigger thresholds; the acquired injury-loss magnitude is reserved to R-WOUND. In-engine the gates are the modality-swap invariant passing, bit-identical replay of a signed-language drift and a modality-shift history across thread counts, and a homesign cohort converging on a full language from no shared input. The honest limits stand: full per-being channel capability exists only for promoted beings; acquired loss waits on R-WOUND (now resolved, Part 62.20), congenital loss being complete now; the sign and exotic articulation contents are reserved data; and the grammar typological parameters remain the open R-LANG-TYPOLOGY. The new citations were verified for the bibliography, and the medium registry was taken inline rather than flagged, on the owner's call, so the channel substrate ships without a closed join key.

---

### 62.14 Modelled dialogue as a data-defined speech-act substrate (R-CONVERSE)

The question. Part 9's gossip loop, Part 33's dawn naming game, and the teaching, governance, trade, and conflict systems across the design all instantiate the outcome of a conversation rather than the conversational process: a transmission was a one-way speaker-to-listener pass, who-spoke-to-whom was chosen by co-location rather than by intent, and an agreed word, doctrine, law, price, or peace was reified without the exchange that produced it. The owner asked, after a deepened-language prototype run, how much of a conversation should become canonical event detail, what the unit of a conversation is, and how deep the non-authoritative interpretation layer (33.2) may go, and chose the speech-act-substrate target (Tier 2), a facet fan-out with a steering-and-determinism verify pass, and a flag-now-resolve-next sequencing.

What was found. A five-facet fan-out (the move substrate and its force-composition, the conversation-as-emergent-unit and its event-log coupling, the listener-response and theory-of-mind coupling with the interpretation boundary, speaking-as-intent and level-of-detail gating, and the deferred deeper couplings) and a two-skeptic verify pass (a steering audit and a determinism non-reopen check) settled and hardened the design. Speech-act theory grounds the substrate: an utterance is an act with felicity conditions that, unmet, make it misfire (Austin), force is structured rather than atomic (Searle), and the taxonomy of acts is contested across accounts (Bach and Harnish) and treated as open and extensible in the computational standards (DAMSL, ISO 24617-2), the decisive ground for a data-defined move registry over a closed enum. Conversation analysis grounds the unit: turn-taking is locally and party-administered rather than externally scheduled (Sacks, Schegloff and Jefferson), the adjacency pair ties a reply to the move it answers (Schegloff), grounding accrues mutual understanding through positive evidence (Clark and Brennan; Clark), and repair is preferentially self-initiated (Schegloff, Jefferson and Sacks). Plan-based and decision-theoretic accounts ground speaking-as-intent (Cohen and Perrault; Allen and Perrault; the utility-AI substrate; the rational-speech-act view of Frank and Goodman). The steering audit caught two leaks: a register knob carried on the canonical move was a latent fidelity channel with no structural bar and a name collision with the Part 39 view-only register, and the proposed conversational common-ground channel reintroduced said-evidence into the one place Part 37 walls it off, admitting a two-colluder corruption of a third party's nested model. The determinism audit caught that the design leaned on five still-open engine items as if settled and added new draw-sites, a three-store write-set, and a long-lived inter-event pointer those items had not been scoped to cover.

The decision taken, signed off and generalized. Modelled dialogue is the promoted-tier refinement of the gossip loop, the same mechanism at higher fidelity. A communicative act is a first-class canonical event over a data-defined dialogue-move registry (a MoveKindDef, sibling to the access-channel, trace-kind, institution-function, value, and semantic substrates), whose force bottoms out at an etic floor of primitive effects that is itself a registry, each effect a call into a mechanism the engine already has (told evidence 9.5, inquiry goal 9.13, naming-game move 33.9, uptake, modelled intent Part 37, contact), so a move adds no authored behaviour, and a culture's emic repertoire emerges by the same coordination dynamic that fixes a word (33.9). Felicity conditions gate whether a move's force lands, never how hard, reading existing role, trust, and value-distance state rather than a graded efficacy. A conversation is an emergent query over co-referenced move events rather than a stored object or a script. Speaking is a utility-AI intent (Part 8) that reconciles the co-location loop and the inquiry interview into one behaviour, with the speaking-versus-acting arbitration a reserved owner decision. A response is the Part 37 single-utterance update applied to a reply, both parties co-updating from access evidence; the steering fix makes grounding said-evidence into the existing first-order and second-order channels with no new common-ground prior, the load-bearing decision that preserves the Part 37 anti-projection guarantee. The canonical record is the move sequence; the interpretation layer elaborates it through the view-only type of 33.2 without entering canon, so a richer move log makes flavour faithful rather than confabulated. Fidelity is significance-gated (Principle 1): move-by-move at the promoted tier, the one-pass outcome and the aggregate diffusion below. The determinism fix pins four dialogue-specific rules (per-draw-site counter slots with a move-scoped and addressee-scoped split, the union gossip partition over the three written stores, the phase-frozen verdict snapshot, and the barrier-ordered move-kind mint) and states the dialogue determinism closes only when the open cluster it extends closes (R-RNG-COORD, R-CANON-WALK, R-CMD-ORDER, R-REDUCE-ORDER). The Steering Audit gains a content-blindness-of-force invariant and a channel-swap invariant, and the content gate classifies every move kind and felicity condition as an affordance or an outcome. This lives in Part 33 (9.5), with the invariants in Part 41 and the move event in Part 7.

What remains to be proven. The calibrations are reserved with their bases (the asserted-claim told-weight, the uptake step, the inquiry motive salience, the contact trust touch, the felicity bands, the conversation contiguity window, the move-logging salience threshold, the repair-trigger distance, the self-repair bias, the grounding-criterion sufficiency, the communicative-drive parameters, the per-tick dialogue budget and promotion thresholds, and the speaking-versus-acting arbitration). In-engine the gates are the two new Steering Audit invariants passing, bit-identical replay of a multi-party conversation and a drift-and-contact history across thread counts, and the false-belief and seen-through-lie battery still passing through a dialogue exchange rather than a bare gossip pass. The honest limits stand: the divergent per-move texture is conserved as mass but not identity across demotion, so the parties to a live load-bearing conversation must stay promoted; the dialogue determinism is pinned at its site but completes only when the open determinism cluster it extends completes, and it obliges the reconciliation of Inconsistency 6 to resolve an in-reply-to into a compacted move to that move's summary; and the deeper couplings (deliberation to Part 36, persuasion to Parts 21 and 28, negotiation to Parts 37, 24, and 8) are scoped and reserved, with negotiation the deepest because its offer-and-concession policy lives in the still-unbuilt decision layer. The new citations were verified against primary sources for the bibliography.

---

### 62.15 The canonical draw-keying schema (R-RNG-COORD)

The question. Part 3.2 keyed each draw on the coordinate of master seed, entity, phase, and counter, but the determinism red and green audit found this omits the canonical tick that Parts 9, 16, 33, and 54 assume, gives no way to separate two draw sites in one entity-and-phase stream so they collide on counter zero, and would let an allocation-order id move the timeline if used as the entity key. R-RNG-COORD asked for a single canonical multi-coordinate keying schema, a phase registry, a counter-namespacing rule, a degrade rule for a missing coordinate, and the camera-independent-id confirmation, all integer and counter-based.

What was found. The existing for_coords primitive already folds an ordered coordinate list order-sensitively, so the fold itself was sound; what was missing was a schema discipline over it. The six draw sites in the simulation tick used hand-assigned phase magic numbers (0x9001 through 0x9006) and an ad-hoc coordinate order that varied site to site, and the older for_entity path omitted the tick entirely. No numeric calibration is involved, because the keying is engine mechanics, the RNG-core exemption Principle 11 grants.

The decision taken, signed off. A canonical DrawKey schema folds a uniform coordinate (region, primary locus, secondary locus, tick, phase, slot) over the for_coords primitive, with the phase drawn from a registered set assigned once, a per-phase slot separating distinct draw sites so they do not collide on counter zero, an absent-coordinate sentinel as the degrade rule distinct from a present zero, the tick always present, and the per-draw counter as the within-stream ordinal. The schema is fixed Rust in crates/core (DrawKey and Phase), and every draw site in the tick (perception, gossip, the naming-game pairing, the innovation roll, coining, and drift) was migrated to it, removing the hand-assigned magic numbers. The keying stays integer and counter-based. This lives in Part 3.2.

What remains to be proven. No calibration is reserved; the phase and slot numbers are stable engine constants, not owner values. Two couplings stay open as honest limits: the schema cannot itself guarantee that a locus is a camera-independent canonical id rather than an allocation-order one, which the id-minting discipline (R-CMD-ORDER, Part 54) must hold, and the order of any non-associative canonical combine consuming these draws is the sibling R-REDUCE-ORDER. In-engine the gate is bit-identical replay across reruns and thread counts, which the determinism harness asserts over the full tick, with the schema's own unit tests proving distinct phases, distinct slots, the tick, and the absent sentinel each separate the stream. The migration changed the specific random streams, so prior replay seeds are not comparable across the change, the expected one-time cost of locking the keying contract.

---

### 62.16 The biology-and-composition substrate floor (R-PHYS-BIO)

The question. The physics substrate Part 41 requires is the one authored layer and the reach-bounding artifact, and its first domain to populate is biology and composition: how a generated world's edibility, nutrition, and toxicity are determined without hand-authoring each species, as a measured relation between an organism's composition and a consumer's physiology rather than a stored flag. R-PHYS-BIO asked for the wave-0 floor of that domain, the composition and toxin axes, the consumer physiology axes, and the closed-form fixed-point laws that contract them, all reserved-with-basis and deterministic, with the representation locked by the substrate guide and only the membership and the law forms to decide.

What was found. A four-facet research fan-out (composition and nutrition, toxicity and defense, consumer physiology, and the law forms) grounded each axis and law in cited food-composition science and toxicology and held them to the locked representation. Two independent red-team Steering Audits followed, the second recomputing the Q32.32 arithmetic rather than trusting the first fold, and it caught that the first ratio-form harm fix relocated the overflow rather than removing it: the dose ratio itself wraps in the unchecked division before its guard can run, the reserved saturation ratio sat above the overflow-safe ceiling of the power, and two further division sites and the trace-axis scales wrapped or underflowed on normal input. On the steering side it caught the edibility law emitting a value-laden verdict, a food-side digestibility scalar and a global medicinal map re-importing a consumer-independent ranking, and a toxin set labelled by mechanism but enumerated by terran chemistry. A verification of the hardening itself caught one self-introduced inconsistency, the additively-summed toxin axes placed on per-quantity scales, now reconciled.

The decision taken, signed off. Composition is a tiered, per-tissue vector of registry points over the Part 58 substrate (a Tier-0 gross floor resolving by demand-closure into Tier-1 essentiality and terran-chemistry toxin registries and Tier-2 fine constituents, each partial lump carrying a residual child for exact additivity); the consumer half is a per-race heritable, extensible registry of relation kinds; and three closed-form fixed-point laws contract them: net nutrition by the Liebig minimum over checked-division satisfaction, harm by the per-(toxin-class, consumer) integer-Hill dose response with the saturation guard ordered before the power and the dose ratio formed by checked division, and edibility as the measured tuple of net nutrition, net harm, an aggregate margin, and a relational medicinal value. On the owner's decision the gain-versus-danger valuation moves out of the substrate to the agent decision and drive layer (Parts 8, 20), so the authored floor reports only measured quantities and carries no risk attitude, and the kernels are race-blind. A Tier-0 completeness criterion (demand-closure over the first build proof's matter-eating food web) is stated and met, with the field-and-gradient feeders excluded by scope. This lives in Part 41 with the substrate representation in Part 58, recorded as a partial advance of R-DEEPTECH-PHYSICS; the calibrations are reserved (the audit log and the Part 41 blockquote).

What remains to be proven. The mechanism is proven in form (overflow-free, panic-free, order-independent, race-blind, twice-audited) but no number is set: every axis range and scale, every consumer physiology vector and its founding variance, the Hill exponents and the saturation ratios and caps, the relational medicinal form and the reserved hormesis curve, the allometric exponents, the tolerance-zero direction, and the relocated risk weights stay reserved-with-basis until the owner sets them and the substrate engine is built to consume them. Three seams are carried open: the source-vector generalization that lifts the eater half off mass-fraction-of-fresh-tissue so field-and-gradient feeders are first-class (R-SOURCE-VECTOR, subsuming the Part 16 and Part 17 enums); the registry-population Steering Audit that must broaden the terran genesis seeding and break its circular channel (R-BIO-REGISTRY); and the cross-tier non-invariance of the nonlinear law outputs, an honest deepening to be reconciled with R-TIER-CONSIST. R-DEEPTECH-PHYSICS stays open for the deepening tiers, the other catalogue domains, and the per-domain Steering Audit, and R-BIOSPHERE is the distinct reader that generates species over this floor.

---

### 62.17 The mechanical-and-materials substrate floor (R-PHYS-MECH)

The question. Wave 1 of the physics-and-materials fan-out is the mechanical-and-materials floor, the domain the first technology-convergence proof (Part 41, Stage 14) selects against and the floor Parts 19, 35, 43, and 45 read. R-PHYS-MECH asked for the Tier-0 axes and closed-form fixed-point laws of mechanics, materials, and energy-and-thermal, reserved-with-basis and deterministic, the floor that bounds what tools, structures, weapons, and machines a culture can conceive and have evaluated.

What was found. A green and red fan-out ran three domain teams (mechanics, materials, energy-and-thermal), each hardened through two red passes that recomputed the Q32.32 arithmetic, then consumer-interface checked against Parts 19, 41, 43, 45, and 55. The dominant finding was the parallel-integration problem: the three teams each defined overlapping axes (density, hardness, the strength set, the thermal stack, fuel value, edge radius) at clashing per-quantity scales, so a consumer reading one quantity hit competing axis points. The wave-0 determinism enemy reappeared too, three mechanical kernels (cut penetration, bending stress, lever torque) forming a product before the division that would reduce it, so a representable result overflowed and routed to a wrong limit. A verification of the hardened synthesis caught one further self-introduced gap (the torque output line shown without its kilonewton-metre bridge) and two presentation seams, all folded.

The decision taken, signed off. One unified registry over the Part 58 substrate, a single axis at one canonical scale per quantity (about thirty-three axes across a shared mechanical group, a bulk-material group subsuming the Part 19 MaterialProps struct, an energy-and-thermal group the energy sub-domain owns and the material sub-domain reads cross-domain, and a shared gravitational axis), and about twenty-two closed-form fixed-point laws reporting measured consequences. The folds: the three product-before-divide kernels reordered so the reducing division precedes the product; one canonical pressure scale (the megapascal, the owner's pin for the pressure scale (the wider unit-system pinning lives in the resolved R-UNITS-PIN, record 62.25)); fuel value and the thermal stack assigned to the energy sub-domain; thermal fracture wired to the shared fracture-strength axis; a shared gravitational axis and a weight law so structural dead-load is derivable; a mechanical-power law; the combustion limiter mislabel fixed; storable output scales for energy, torque, and power; value-density routed to the Part 19 market price. Crush versus pierce is a contact-geometry difference the laws resolve, so a mace and a morningstar are one conceived design rather than two authored types. The kernels are race-blind and material-blind, and a Tier-0 completeness criterion is stated and demonstrated met. This lives in Part 41 with the representation in Part 58, a second partial advance of R-DEEPTECH-PHYSICS; the calibrations are reserved.

What remains to be proven. The mechanism is proven in form (overflow-free, panic-free, order-independent, race-blind, twice red-teamed in the fan-out and the synthesis-write verified) but no number is set: every axis range and scale, the one pressure scale, the law caps and bridge constants, the gravitational acceleration, the modelling factors, the curved-contact modulus, and the wear scale stay reserved-with-basis until the owner sets them and the substrate engine consumes them. The named reach ceilings are wave-2 and thermal-deepening scope under the open R-DEEPTECH-PHYSICS: ballistic projectile range, buoyancy and hydrostatics (the fluids domain), radiant and convective heat, and heat-to-work for engines. The surfaced refinements (rigid-mechanics-only with no angular-velocity axis so a chain flail is approximated, continuous-rotation fatigue, the curved-contact combined-modulus detail, the wear-coefficient split-range, the multi-layer-strike residual-energy bookkeeping, and the soft-tissue cutting-energy basis) are honest limits, and the Part 55 dimensionless Ratio class and derived-monomial registrations couple to R-UNITS-PIN (now resolved, record 62.25). R-DEEPTECH-PHYSICS stays open for the deepening tiers, the other catalogue domains, and the per-domain Steering Audit.

### 62.18 The biosphere: generating and radiating the living world (R-BIOSPHERE)

The question. Once the biology-and-composition floor (R-PHYS-BIO) and the deep-time genome engine (R-GENOME, Part 25) were resolved, R-BIOSPHERE asked how a generated world is populated with many plant and animal varieties, and how edibility, nutrition, toxicity, and medicinal value are determined, without hand-authoring each species, and while staying deterministic, data-driven, and per-race differentiable. The floor already measures edibility as a relation and the genome engine already radiates allele-frequency pools over deep time, so R-BIOSPHERE is their distinct reader rather than a re-resolution: it must author the generative trait space and its validator, wire each organism and consumer to the floor, and drive the deep-time radiation as a pre-dawn epoch, none of it fabricating a value.

What was found. The scope reduced to two gaps, seeding the species set without hand-authoring and wiring organisms and consumers to the floor plus the pre-dawn epoch driver, since the edibility laws, the genome engine, and the zoomable map are built. A trait-axis catalogue fan-out returned eighty-six candidate axes across plant, animal, shared, composition-beyond-floor, and fantasy-profile-gated groups, every range reserved-with-basis and every stored verdict demoted to a read-time derivation, red-teamed for fabrication, steering, and demand-closure and hardened (the biological disease-resistance channel, the animal-tissue material and natural-weaponry and delivered-venom and aquatic-niche axes added, three consumer-less axes dropped, the reference class re-keyed off the per-relationship referent). A four-cluster mechanism fan-out then settled the generator, the consumer wiring, the pre-dawn epoch driver, and the dynamic-edibility refinements, and a red team recomputed the Q32.32 arithmetic, confirmed no fabricated number, that the steering relocations hold, that every law is a bounded closed-form integer kernel with the transcendentals deferred to R-GPU-CANON-PIN, and that the stick-breaking additivity and the least-fixed-point food-web closure are provably conservative, then returned a fix list that was folded (the stock-normaliser citation corrected to a built method, the sample-time niche distance defined in trait space, the selection clamp bounded against the built divide's wrap, the pool-mean products made checked, the zero-denominator routes defined, the speciation roll keyed to carry the generation, and three unbuilt prerequisites named plainly).

The decision taken, signed off, and generalized. The mechanism is a seed-keyed generate-and-validate sampler that emits species as points over the extensible trait-axis registry, validated in a fixed order (fixed-point representability, biome-fit by a Liebig minimum of piecewise-linear tent responses, least-fixed-point food-web closure, and niche openness) with a bounded resample and a closure-preserving fallback; a bounded pre-dawn epoch that radiates the founders by drift, an environment-to-coefficient selection kernel, founder-fork, declared speciation into a parent-pointer lineage tree with an Orr-snowball incompatibility accumulation, and deterministic extinction, before the dawn where the mature pools are promoted and sentient races seed fresh; and edibility as the floor's measured tuple with a relational medicinal credit, dose-and-preparation dependence, and heritable stick-breaking composition. The owner's four forks were confirmed (the hybrid closure with feasibility left to the Part 15 stocks; no mandatory toxin floor, so defensive chemistry emerges; the intelligence-dial cut-points as per-world data; sentient races seeded fresh at the dawn, with full sentient co-evolution recorded as a future candidate), and the recommended forks adopted (matter-eaters first with the validator resolving draws against pools rather than a closed enum; a bounded pre-dawn worldgen pass; a dedicated species-lineage registry keeping the genome engine pure; a genesis-derived stored heritable consumer vector; an axis-keyed composition genome channel; a derived trophic level; a measured domesticability; and a unified affliction harm sink). On the owner's condition the result is general across organisms, per-race differentiable, and data-driven, the mechanism fixed Rust and the axes, edges, relation kinds, per-race bindings, and thresholds data. The stock-and-flow substrate and the species-lineage container are already built (`crates/sim`).

What remains to be proven. The thirty-nine reserved calibrations are surfaced with their bases in the audit log and are the owner's to set and validate. The epoch's reproducible identity is bounded to the interim biallelic genetic-distance measure until the fixation-index-versus-Nei and multi-allele choice of 25.10 is set, with the incompatibility-count speciation criterion as the measure-independent backbone; the exact Wright-Fisher drift gives the pre-dawn generation count a performance ceiling until the cheap large-Ne integer-Gaussian path is built (R-GPU-CANON-PIN); digestion is primitive per-race data until R-BUILD-PHYS and the anatomy model resolve it; and the harm sink defers its representation to R-WOUND. Three pieces are fixed Rust to add during the build: the soil stock, the axis-keyed composition genome channel, and the founder-fork pool method. The mechanism reads around the open siblings R-SOURCE-VECTOR, R-BIO-REGISTRY, R-WOUND, Inconsistency 5, and R-VIEW-ELAB (R-UNITS-PIN and R-GPU-CANON-PIN, formerly siblings, are now resolved, records 62.25 and 62.23), and the eventual build must prove that a generated world produces coherent closed food webs, that edibility stays relational with no stored verdict and no reinstated risk attitude, and that the seeder and the pre-dawn epoch are a pure function of the world seed across machines and thread counts.

---

### 62.19 Evolved behaviour: the homeostatic controller (R-BEHAVIOR-EVOLVE)

The question. The item was surfaced by an owner catch on the emergent-locomotion slice: the utility-AI decision layer (Part 8) selects from an action-and-consideration repertoire that is data-defined per race, which satisfies Principle 11 but not the deeper Principle 9, since a repertoire authored as data is still authored, so a creature seeks water when a thirst drive is high because that action and that curve were written, not because the behaviour emerged. For non-sentient creatures and the pre-sentient substrate that is steering at the level of behaviour. R-BEHAVIOR-EVOLVE asked how behaviour can instead arise the way anatomy does, from physics and selection, with nothing about the behaviour itself authored, while staying deterministic, observer-independent, and data-driven.

What was found. A standalone design pass (`docs/emergent_behavior_design.md`) framed the answer as five layers, the first three authored physics (a homeostatic physiology of draining reserves, the affordance primitives a morphology permits, and the non-omniscient percept the senses provide), the fourth a heritable controller expressed from a new genome channel, and the fifth the selection the pre-dawn epoch already performs, with fitness a consequence of homeostatic survival rather than an authored objective or a resemblance to expected behaviour. The controller representation was the open choice, and the pass recommended a staged one that the owner accepted: a linear reaction norm as the substrate and the determinism proof, graduating to a small fixed-point recurrent network for stateful and conditional behaviour, the step between them the reserved hidden width. The pass also named the crux plainly, that scoring a time-extended behaviour over deep time is the same cost wall as temporal level of detail (Part 32), to be paid on the significance-and-time gradient (Part 54) with a cheap homeostatic-viability proxy for quiet pools and full sampled episodes at the dawn and under significance.

The decision taken, signed off, and generalized. The owner signed off the pass with the standard conditions and a staged build, controller leaning toward the network. The mechanism was built and proven across four stages, each determinism-audited and tested before the next: the homeostatic physiology and the morphological affordance registries (Stage 1); the heritable controller as a new genome channel (`Channel::Controller`, a sibling of the cognition and composition channels), expressed per individual to a clamped-linear reaction norm over a data-defined layout of the homeostatic and affordance registries, wired into the locomotion path in place of the authored decision menu, which is retired there while it remains the deliberative tier's interface (Stage 2); the homeostatic-survival selection coefficient, a cheap spatial-episode scorer and a full search-based episode scorer on the significance gradient, an individual-based evolutionary loop that produces water-seeking behaviour from random controllers without it being authored, and the coefficient wired into the aggregate-tier `GenePool::select` recurrence (Stage 3); and the graduation of the same plumbing to a small fixed-point recurrent network with a carried hidden state, evolving under the same selection, the step a parameter change and not a rewrite (Stage 4). On the owner's condition the result is general across creatures, per-race differentiable, and data-driven: the mechanism is fixed Rust and the homeostatic-axis registry, the affordance registry, and the controller weights are data. The evolved controller underlies the sentient layers (Parts 21, 28, 37) rather than replacing them.

What remains to be proven. The reserved calibrations (the homeostatic rates per axis, the intake yield, the controller representation and hidden width, the controller-allele init spread and mutation rate and step, and the behavioural-scoring population, generations, and episode length) are surfaced with their bases in the audit log and are the owner's to set. Whether adaptive behaviour evolves in feasible compute over a whole world's deep time is an empirical result to be shown at scale, not proven on paper. The proxy's honesty, that scoring well on the cheap episode predicts surviving in the world, is the central risk and must be validated by cross-checking proxy-selected lineages against full episodes, not asserted. The pure-frequency deep-time tier expresses a controller from allele frequencies, which couples to the quantitative breeding-value tier the genome still defers (25.10) and to the open temporal level of detail (Part 32); this work builds and proves the individual and sampled-episode tier and wires the coefficient into the pool recurrence, and scopes the pure-frequency tier as that reserved coupling. The reaction norm cannot gate a response on internal state through a product (the recurrent network lifts that ceiling), and the behavioural-scoring proxy is a temporal-LOD device that must stay consistent with the full-episode tier the way the other coarse-and-fine pairs must (R-TIER-CONSIST, Part 54). The intake yield stands in for the resolved edibility floor's measure (R-PHYS-BIO) until the floor is wired to the located matter.

---

### 62.20 The body arc: physical build, wounds, and fluids (R-BUILD-PHYS, R-WOUND, R-FLUID)

The question. Part 35 modelled a body as parts of layered tissue with fluids, but three pieces stayed open and authored: R-BUILD-PHYS asked which physical build stats are primitive versus derived from the physics substrate; R-WOUND asked how a wound is represented without the closed `Wound { Cut, Blunt, Pierce, Burn }` enum, defined against a data-driven anatomy that differs by race and admits exotic modes (corrosion, freeze, curse); and R-FLUID asked how a body fluid is represented without the closed `FluidKind` enum, its loss and interactions data-defined. The three were flagged to be taken as one session, since they share a body plan that is itself data, and they read three floors that were resolved first (R-PHYS-MECH's material axes and mechanical and thermal laws, R-PHYS-BIO's composition, and the Part 25.14 anatomy vector), so the arc is their distinct reader rather than a new floor. The owner delegated the fork calls for an overnight build.

What was found. A grounding fan-out over Part 35, Part 20, Part 25.13, the resolved floors, and the built substrate confirmed the on-ideology endpoint stated at Part 25.13: a wound's lethality is measured from the physics floor rather than stored as a kind, the sibling of edibility-as-a-measured-relation. The forks resolved on that endpoint and the project's standing defaults (recorded in `docs/working/BODY_ARC_SCOPE.md`): a wound is measured, not a stored variant; damage modes are a data registry keyed to a fixed physics measurement family (penetration, fracture, thermal, and exotic for an unbuilt law); a fluid is a data-defined substance with a conserved volume stock; integrity is derived from part condition and fluids, never a competing store (Part 35's own rule); the raw morphology is the primitive build and the mechanical stats are derived through the floor; and a natural weapon affords a strike whose wound is measured, closing predator-prey.

The decision taken, signed off, and generalized. The mechanism is built and proven in `crates/sim/src/body.rs`: a per-part `Body` promoted from the anatomy vector with layered tissue materials (a vital torso carrying the fluid vessel, a vital head, a limb per locomotion mode, a part per natural weapon); an insult-to-wound measurement over the resolved floor laws (contact pressure and cut penetration walking the tissue layers so a bone core stops a weak cut, the dual fracture criterion on the strongest structural layer, constrained thermal stress), with a cut clean through severing the part, a deep wound breaching the fluid vessel, and an exotic mode with no built law doing nothing rather than fabricating damage; a data-defined fluid registry with a conserved volume `Stock`, bleeding from a breach netted against a clot rate, and death past the critical fraction; a derived `integrity` aggregate and an `is_alive` that reads no vital part destroyed and no pool past critical; a physics-derived reach through the resolved reach law; and a `strike` enactment that forms an insult from a weapon part's mass and the blow through the kinetic-energy law. The loop to R-BEHAVIOR-EVOLVE is closed: a derived integrity homeostatic axis is refreshed from the body so the evolved controller reads its own condition, and a `dev_predator` affordance registry adds a strike output a controller can evolve to use. The mechanism is fixed Rust and the tissues, modes, fluids, functions, and anatomy are data; the wound measurement is race-blind and draws no randomness, so a body's condition reproduces bit for bit. On the owner's condition the result is general across data-defined anatomies, per-race differentiable, and data-driven. This resolves the unified affliction harm sink that R-BIOSPHERE, the edibility floor, and R-LANG-MODALITY's acquired-loss coupling defer to.

What remains to be proven. The reserved calibrations (tissue material properties, damage caps, the fracture and burn integrity mappings, the fluid criticals and clot and bleed rates, the body promotion mass and thickness parameters, and the strike energetics) are surfaced with their bases in the audit log and are the owner's to set. The honest limits, carried openly: the penetration read caps depth by the layer walk rather than depleting energy layer by layer (a refinement); the temperature axis is a reserved two-sided band whose environmental thermal exchange waits on the located world and a medium floor; corrosion and full fluid chemistry wait on wave-2; and the digestion model R-BIOSPHERE deferred here couples to the consumer physiology and is reserved. The build is signed off autonomously on the owner's delegation and stands for his morning review; the fork decisions are recorded in the scope document for that review.

---

### 62.21 The fluid, chemistry, and optics substrate floor (wave 2, R-PHYS-W2)

The question. Waves 0 and 1 populated the biology-and-composition and mechanical-and-materials-and-thermal floors of the physics substrate (Part 58), the one authored layer whose reach bounds all downstream emergence (Principle 9). Wave 2 asked for the next classical slice, the fluids, chemistry, and optics floors, so the consumers blocked on wave 1 could read what they need: R-FLUID and the body arc's fluid interactions and temperature exchange, R-WEATHER's water cycle and wind, the Part 5 light field and R-SENSORIUM's light-gated and colour perception, the acoustic communication channel and hearing, and the metallurgy, corrosion, and medicine that reactions and dissolution stand on. It also asked which reach ceilings wave 1 named (hydrostatics, buoyancy, convective and radiant heat, heat-to-work) wave 2 absorbs, and where the honest ceiling now falls.

What was found. A fan-out of three green domain teams, a red Steering-Audit per domain that recomputed the Q32.32 arithmetic, six consumer-interface checks, and a synthesis produced the hardened proposal (`docs/working/FLUIDS_CHEMISTRY_OPTICS_SUBSTRATE_PROPOSAL.md`), with nineteen confirmed steering, determinism, reach, and fabrication findings folded. Two consumer-lens completeness passes then hardened it further: the first over fluids folded thirteen Tier-0 additions (the weather water cycle and acoustics, catching that the speed of sound was wrongly deferred when `Fixed::sqrt` exists), and the second over chemistry, optics, and the wave-1 floor folded seven more (radiative-equilibrium temperature by two nested square roots, a data-defined colour stack, and dissolution) and confirmed wave 1 needs no addition, since the bow's elastic energy, torque and gearing, and self-heating all compose from the existing floor.

The decision taken, signed off, and generalized. The floor extends the Part 58 registry with the three domains, every axis and law reserved-with-basis and cited, every kernel closed-form integer staged against the Q32.32 ceiling with no float in canon. The owner signed off the two load-bearing unification decisions: one canonical density axis widened down to the lightest gas (folding a confirmed red finding that atmospheric densities fell below the solid-liquid floor), and a canonical `therm.temperature` state axis owned by the energy sub-domain and read cross-domain, which grounds the wave-1 conduction and combustion inputs. Wave 2 absorbs the named reach ceilings and adds the dynamic-pressure force family (drag and aerodynamic lift), so flight and realistic ballistics are reduced-order lumped-coefficient laws. On the owner's condition the result is general across substances and data-driven: the mechanism is fixed Rust, the axes, scales, substance vectors, spectral bands, per-race receptor sensitivities, and law constants are data.

What remains to be proven. The reserved axis ranges and law constants are surfaced with their bases and are the owner's to set through the calibration manifest, and the kernels are to be built into `crates/physics` on the wave-1 recipe with the determinism harness and a load proof. The honest limits are enumerated in the Part 41 blockquote and the proposal: the transcendentals (the exact vapour curve, the Arrhenius and Tafel rates, angle-resolved Snell and the exp Beer-Lambert falloff for light and sound, and Rayleigh scattering) are deferred to R-GPU-CANON-PIN, so wave 2 carries their piecewise-linear and lumped-coefficient proxies; the saturation-vapour proxy is valid only near its reference; and turbulence and the drag polar are the permanent categorical reach boundary. Wave 2 is a third partial advance of R-DEEPTECH-PHYSICS, which stays open for wave 3 (electricity and magnetism) and beyond.

---

### 62.22 The electricity-and-magnetism substrate floor (wave 3, R-PHYS-W3)

The question. Waves 0 through 2 built the grounded Tier-0 floor a culture lives inside from the dawn (biology, mechanics and materials and thermal, and the fluids, chemistry, and optics continuum). Wave 3 asked for the first discoverable deepening tier, electricity and magnetism, the floor a people reaches by instrument and theory rather than begins with: the axes and laws the electrical machine, the telegraph and telephone toward radio (R-COMMS), the motor and generator and battery, and the galvanic link back to the wave-2 electrochemistry all read. It asked how the tier opens without an authored per-race unlock, what one representation seam electricity forces, and where the honest ceiling falls.

What was found. A fan-out of two green domain teams (electrostatics-and-current, magnetism-and-induction), a red Steering-Audit per domain that recomputed the Q32.32 by hand, four consumer-interface checks, and a synthesis produced the hardened proposal (`docs/working/ELECTROMAGNETISM_SUBSTRATE_PROPOSAL.md`), with ten confirmed findings folded (conductivity dropped as the derived reciprocal of resistivity, resistance scoped as a law output rather than a stored datum, mutual inductance derived from the coupling coefficient, and the sharpest catch, the missing self-and-mutual inductance laws the telephone and induction coil need, folded as the inductance axis, coupling coefficient, inductor energy, and inductive EMF). The load-bearing catch was surfaced rather than folded: electricity is the first quantity dimensionally independent of the four earlier bases, so it forces a fifth base dimension, an act on the locked representation that is the owner's Principle-9 call.

The decision taken, signed off, and generalized. The owner signed off the recommendations. The `Dimension` monomial gains a fifth i8 base, electric current (the SI ampere), because the Gaussian half-integer alternative is unrepresentable in the integer exponent; the parser accepts a four-tuple (current zero) or a five-tuple, so every earlier floor's data loads unchanged and neutrality-by-construction is preserved over five bases (proven by test). The `chem.standard_potential` wave-2 proxy is promoted to the volt, dimension-and-scale only. The floor is a discoverable deepening tier (tier 2), its three rungs gated by the culture's instrument-and-theory state, never a per-race table. On the owner's condition the result is general and data-driven: the mechanism is fixed Rust, the axes, scales, substance vectors, coil construction data, and law constants are data.

What remains to be proven. The reserved axis ranges, per-quantity scales, and law constants (the Coulomb coefficient on its x1e9 scale, MU_0, the coil turns, and the tick duration DT) are surfaced with their bases and are the owner's to set through the calibration manifest. The floor is built into `crates/physics` on the wave-2 recipe with the fifth base added and a four-floor load proof (81 axes, 60 laws over the merged registry). The honest limits are in the Part 41 blockquote: the induction laws are a first-order tick-rate-dependent finite difference; the dipole torque defers its angular factor; the solenoid field defers B-H saturation; and full Maxwell wave propagation, the AC frequency domain and the tuned LC circuit (radio), and the concentration and rate transcendentals are deferred to wave 4 and R-GPU-CANON-PIN. Wave 3 is a fourth partial advance of R-DEEPTECH-PHYSICS, which stays open for the discovery-mechanism coupling (R-DEEPTECH-SCIENCE) and wave 4.

---

### 62.23 The canonical GPU arithmetic and transcendental pin (R-GPU-CANON-PIN)

The question. Part 5.4 granted the cross-backend bit-identity argument for integer addition but pinned neither the fixed-point multiply and divide (a sixty-four-by-sixty-four-to-one-hundred-twenty-eight intermediate and a narrowing that WGSL, SPIR-V, Metal, and DX12 lack natively) nor the transcendentals (square root, an exponential, a logarithm, and the trigonometry a distance measure or a physics law needs), and it left the worldgen noise (Part 12) and eikonal flow-field (Part 13) kernels unclassified and the target backend set unenumerated. The physics waves raised the stakes, since the wave-2 and wave-3 floors deferred their transcendental law forms (Clausius-Clapeyron, Arrhenius, Tafel, Beer-Lambert, Nernst, and the trigonometric factors) to this item by name. The two heads are one item, because the same integer arithmetic answers both: the CPU `Fixed` type is the bit-exact oracle the GPU must reproduce, and its operations are the ones the physics laws call.

What was found. A fan-out (three green domain teams, a determinism red-team that recomputed every bit-identity claim, consumer checks, and a max-effort synthesis, thirteen confirmed findings folded) produced the hardened proposal (`docs/working/GPU_CANON_PIN_PROPOSAL.md`). It verified the CPU oracle exact by hand and over a four-thousand-plus-case sweep (the multiply floors via the i128 shift, the divide truncates toward zero, the square root is an exact isqrt), pinned the u32-limb sign-magnitude multiply and the ninety-six-step restoring divide with a proof they extract the same bits, pinned the exponential, logarithm, sine, cosine, arctangent, and arcsine (CORDIC and range-reduced series) and the real power as the exponential of the log product, each buildable as the `crates/core` oracle sibling and gated on one reserved precision target apiece, ruled the noise canonical and the eikonal field canonical-under-monotone-min-else-quarantined, enumerated the backend and adapter set and the Stage 0 gate, and surfaced the multiply-floors-versus-divide-truncates asymmetry for the owner rather than reconciling it silently.

The decision taken, signed off, and generalized. The owner signed off the recommendations (the pins as specified, the noise and eikonal rulings, the CUDA-first backend set, and the preserved rounding convention), general and per-field differentiable, with the reserved precision targets left reserved. The pin is fixed Rust and the precision targets, per-field windows and variants, and backend set are data (Principle 11). It is built: `crates/gpu` stands up CubeCL (the owner's chosen backend, cubecl 0.10, on CUDA) with the multiply, divide, and the fixed-point diffusion field as `#[cube]` kernels, proven bit-identical to the `Fixed` oracle on the reference GPU (the multiply and divide over every corner pair and a million random cases each; the diffusion field over the whole grid, invariant to workgroup and tile size; and the full thousand-square, two-hundred-step field matching the CPU bench checksum exactly at about twenty-two-fold throughput). A no-float source guard and an adversarial determinism review confirmed the kernels integer-only, the two multiply copies faithful to the oracle, and the divide restoring step exact.

What remains to be proven. The Stage 0 arithmetic is now proven bit-identical across three independent CubeCL codegen paths: CUDA (NVRTC), the CPU backend (MLIR/LLVM), and the wgpu Vulkan/SPIR-V backend (cubecl-spirv, run here on lavapipe), all matching the oracle in the cross-backend gate. A true multi-vendor hardware run across discrete-GPU vendors (Metal, DX12, ROCm) stays the residual, since cross-vendor bit-identity of compiled kernels is an empirical property the gate arbitrates rather than a theorem. The field kernel uses native sixty-four-bit arithmetic where the backend has it (the proposal-sanctioned per-kernel layout), so its cross-vendor identity is a Stage 0 matter for each further backend rather than covered by the u32-limb argument. The full transcendental set is now built as `#[cube]` kernels in `crates/gpu`: the exponential, logarithm, real power, integer power, and the CORDIC sine, cosine, arctangent, and arcsine, each proven bit-identical to the oracle on the reference GPU over a domain sweep. The general u128 `Fixed::sqrt` is now built as a 96-bit limb isqrt, proven bit-identical to the oracle over the full magnitude sweep (the arcsine keeps a domain-limited u64 isqrt over its zero-to-one radicand), and the shared limb primitives (multiply, divide, and the isqrt) are consolidated in one module rather than copied per kernel. The transcendentals do not yet cross-backend on the non-CUDA paths: the CPU backend trips an upstream cubecl-cpu optimizer panic on their control flow, and this box's only Vulkan adapter is lavapipe (software), too slow for the large unrolled shaders, though SPIR-V Int64 is supported (a native-i64 probe ran in milliseconds), so a hardware Vulkan device would carry them. That transcendental cross-backend confirmation and a true multi-vendor hardware run stay the residual. The physics-wave law forms that deferred to this item are now unblocked (their transcendentals exist as the oracle), with re-authoring the exact forms from the folded proxies a named follow-on. The Q32.32 representability windows require some laws to be reformulated (coupling to R-UNITS-PIN), and the general canonical multiply-folding reduction rides R-REDUCE-ORDER. The reserved values (the precision targets, the per-field windows and variants, the eikonal criterion, and the backend enumeration) are the owner's to set through the calibration manifest.

---

### 62.24 The differentiated founding coverage of the language dawn (the NSM prime floor)

The question. Part 33.9 resolved the dawn of language as a hybrid that coordinates a seed system and then hands off to seeded generation, with the anchor set and the round cap reserved. A follow-on pass fixed the anchor set as the NSM semantic prime floor (the primitive axes of 33.1) and asked whether every founding cohort establishes the full prime floor at once, with all cross-language thinness pushed into post-foundation elaboration, or whether founding coverage is itself differentiated. The owner asked that the answer be blind-audited against the source so the founding-coverage rule would author no cultural outcome (Principle 9).

What was found. A fully-blind panel (three independent auditors over the primary sources, repo-walled, synthesized) returned a split that resolved on the evidence. Treating the prime set as a universal floor is faithful to the NSM program, but its universality is established for mature, multi-generational languages (Wierzbicka 1972, 1996; Goddard and Wierzbicka 1994, 2002), not for founding events, and the inventory is a revisable estimate with real challengers (Bohnemeyer's Yucatec Maya gap for BEFORE and AFTER; Riemer's method critique). The emergence record shows founding coverage is not instantaneous or complete: Nicaraguan Sign Language cohort one produced few or no THINK and KNOW forms, the category closing only in the second cohort (Pyers and Senghas 2009), with the same generational lag for IF and BECAUSE clause-linking (Sandler and colleagues 2011), the pidgin-to-creole cycle, and child first-language acquisition, where epistemic primes are acquired last (Bartsch and Wellman 1995), while concrete and desiderative primes are resilient from the start. A round cap scaled to cohort size, so a small cohort establishes fewer primes, was rejected on two grounds: it authors a cultural outcome, and it runs backwards to the finding that small isolated communities trend toward more structural complexity, not less (Trudgill 2011). Semantic molecules (Goddard 2007, 2012) are the source-native richness axis above the floor.

The decision taken, signed off. The owner chose the differentiated emergent floor. The prime set is the universal grounding floor toward which languages converge, an adopted, citable, revisable axiom whose count and membership are data. At the dawn the resilient primes coordinate; a lagging subset closes over generations through one uniform mechanism read from simulated state (community size, generational depth, contact), never a per-culture lookup, so cross-language thinness emerges from a community's transmission depth rather than from an authored floor or a cohort-size cap. Each prime carries a data-defined founding resilience (33.1); richness above the floor is semantic molecules. The round cap is a determinism safety bound only. The mechanism is fixed Rust; the prime membership, the per-prime resilience, the molecule inventory, and the closing rates are data (Principle 11). This lives in Part 33.9 and 33.1, reconciling records 62.6 and 62.13, with sources in Part 63.

What remains to be proven. The calibrations are reserved, surfaced rather than fabricated, each with a basis: each prime's founding-resilience rank (grounded in the emergence record, per-race adjustable), the closing threshold that fills a lagging prime as accumulated transmission crosses it (the one-to-two-generation lag over community size, depth, and contact), and the dawn round cap as a safety bound. In-engine it must be shown that two differently-sized founding communities coordinate the same resilient primes at the dawn while the lagging subset closes at different generational depths from their own simulated state, that no per-culture rule authors which language stays thin, that the closing is bit-identical across thread counts, and that the floor membership and count remain data a race or a revised inventory can change. The honest limits stand: the floor is one research program's revisable estimate adopted with citation; applying it to non-human sapient races is a separate explicit convergent-cognition axiom; and the closing is modelled at the culture or pool tier.

---

### 62.25 The R-UNITS-PIN number system: two-tier arithmetic, the floor invariant, and the exact emic layer

The question. Part 55 stated the numeric-backbone rules (each quantity a fixed-point scale, conversions explicit, a per-quantity saturate-or-wrap choice) but pinned none of them, and R-UNITS-PIN asked for the unit substrate to be built. The advisor design then sharpened it to the question the flagship physics forces: how a constant such as the Stefan-Boltzmann sigma, which underflows the canonical Q32.32 grid to about eight significant bits, and a law-chain such as sigma times the fourth power of temperature, whose exact intermediate exceeds sixty-four bits, are computed deterministically with no hardware float, and how a near-zero divisor or log argument is bounded by a physical fact rather than by the storage epsilon (Principle 10).

What was found. The units crate already held the scale registry and absolute storage; the missing pieces were the precision and floor machinery. A deterministic float-free bignum and exact-rational evaluator computes a composite constant to its reference precision, pi by a Machin series, cross-checked against the stored decimal to its own unit in the last place. A two-tier number system, measured against an independent arbitrary-precision oracle over a swept grid, showed the flagship quartic chain reaches about two hundred ten bits, so an eight-limb two-hundred-fifty-six-bit accumulator rounded once per chain is the common case for a chained law, not an edge case. An independent one-hundred-sixty-three-thousand-case fuzz of the hand-rolled wide integer caught three real defects in the narrow single-op helpers (a release-profile panic and two silent wrong-value cases at a shift of one hundred twenty-seven or more), fixed and folded in as a standing regression. A census of the divisor and log sites found the floor invariant largely already satisfied by explicit intentional-limit-at-zero guards rather than by silent sentinels, and a consumer-lift survey found the precision-lift set empty beyond the one flagship.

The decision taken, signed off, and generalized. The number system is two tiers plus a load-time scale planner. Tier 1 is the canonical Q32.32 `Fixed`, unchanged, for the ordinary-range majority. Tier 2 is a scale-closed integer arithmetic over the wide accumulator, rounded once per law-chain, so a chain computes at full precision in one accumulator with a single terminal round. A law's structure is a typed op graph (`LawExpr`) as data; one load-time walk sizes each node's intermediate width and scale by interval arithmetic on the declared exponent envelopes (the wide accumulator the common width for a chained law, never the narrow one by default), and the same walk enforces the floor invariant, failing loud when a divide or log node resolves neither a declared physical floor nor a declared physical-limit-at-zero. A type-level guarded divide and log backstop covers the hand-threaded laws the op graph does not model, and the invariant was populated on the run-path sites: the Fechner-log domain (derived from the compressive law's shape-non-negative contract), the third-law absolute-temperature floor, the Crow-Kimura difference-divisor degenerate limit, and the point-load, massless, and point-source limits of the contact family. The flagship sigma lift routed the radiant law through the full-precision sigma and measured byte-neutral on all four canonical scenarios. On the owner's condition the result is general and data-driven: the mechanism is fixed Rust, the op-graph node kinds are the fixed arithmetic floor, and the laws, the scale envelopes, and the declared physical floors are data (Principle 11). Decision in Part 55, built in `crates/units`, block 1aa; the calibrations are reserved.

The residuals closed, and what remains to be proven. The two residuals the first pass left open are now built, closing R-UNITS-PIN. The emic (cultural) measurement layer (`crates/units/src/emic.rs`) crosses to the absolute base exactly: a unit's factor is an exact rational (`UnitFactor`, numerator and denominator) so it contributes no approximation, `emic_to_absolute` and `absolute_to_emic` compute the whole numerator exactly before one terminal round (the same single-round discipline as the tier-2 arithmetic), storage is canonical-absolute so an emic reading is a view that never ratchets, and a `StatedQuantity` exact-rational carry keeps a culture-stated quantity perfectly reversible until it must enter the fixed-point physics path. The round-trip-exactness rule delivered: exact for every emic value the absolute scale can resolve, and within one absolute epsilon at the one boundary where the epsilon is coarser than the emic step, a deterministic storage floor stated and tested rather than hidden (Principle 10). The per-culture unit store is the id-ordered `MeasurementSystem`, whose `iter_ordered` is the one canonical walk, closing the units-local instance of R-CANON-WALK (the cluster stays open for its other containers), and a unit's provenance is an open `ProvenanceKindRegistry` reference rather than a closed enum, so a people measuring by a forearm or an alien by a mana tide is a data row. The reserved values are surfaced with their bases: the composite significance target and guard (the fixed-point scale family that resolves a CODATA composite's roughly ten significant figures), the per-law intermediate widths (each sized from the law's measured exponent interval), and the declared physical floors and limits-at-zero (each a per-world value below which the quantity is absent, derived from the physics where a material or law datum supplies it, surfaced as an owner value only where the physics cannot); the emic layer adds no new reserved value, since the factor is per-culture data and the scales are the existing reserved envelope. The honest limits stand: the emic layer is built as substrate and not yet wired to a live per-culture measurement, which is a Part 40 content arc; software float (a per-value mantissa and exponent) is specified but not built, reserved for a quantity whose runtime range cannot be declared, which the declare-your-bounds principle prevents; and the byte-neutral flagship lift is a real precision-correctness improvement whose effect the current pinned scenarios do not surface, so the arc moved no canonical hash. Resolved, built in `crates/units`, block 1aa; the calibrations are reserved.

---

### 62.26 The celestial insolation substrate: deriving the sky that drives the climate (genesis-forward Stage 1 and Stage 2)

The question. Part 18 read a per-cell insolation and named seasons, a deep-time drifting baseline, and warm and cold ages, but the insolation was an authored signal and the solar scale an inline 1361 constant, so the sky that drives the climate was a number rather than a derivation. The genesis-forward build asked how the ground-level irradiance of a rotating, tilted world on an elliptical orbit is stood up from first principles, in causal order from the star outward, so that Mirror reconstructs Earth's real sky from cited anchors and an alien world (a different star mass, orbit, tilt, or spin) is a data row rather than a rewrite.

What was found. The flux scale derives from the star: the main-sequence mass-luminosity relation `L = L_sun * (M_star / M_sun)^exponent` anchored at the sun, spread by the inverse-square law `L / (4 * pi * d^2)` to the world's orbital distance, reconstructs Earth's roughly 1361 W/m^2 for a solar-mass star at one astronomical unit. The computation forced a representability finding: the solar luminosity (about 3.828e26 W) and the squared orbital distance overflow the canonical Q32.32 grid by many orders of magnitude, so the derivation was built in an exact arbitrary-precision rational arithmetic with pi by a Machin series, rounding once to the fixed-point solar constant. The daily and seasonal geometry was found to derive wholly from the world's own rotation and orbit: the synodic day as the sidereal spin minus the orbital advance (so tidal locking falls out), the declination `obliquity * sin(2 * pi * orbital_phase)` for seasons and polar day and night, the eccentricity distance term `(a / d)^2 = 1 / (1 - e * cos E)^2` through a deterministic Kepler solve under a fixed iteration cap, and the perihelion phase decoupling the distance minimum from the seasonal reference for the hemispheric amplitude asymmetry. Each added geometry reduces to the identity at the reference world's inert value, so the substrate grew byte-neutrally one term at a time.

The decision taken, signed off, and generalized. The sky is the world's DATA sky (a star-list plus the orbital and surface data) read by a fixed-Rust law (18.1). On the owner's condition the result is general and per-world differentiable: the mechanism is fixed Rust, and the star-list, the two orbital periods (from which the tick cadence derives through the canonical seconds-to-ticks bridge, not a hardcoded 365-day calendar), the obliquity, the eccentricity, the perihelion phase, the Bond albedo, and the back-radiation floor are world data (Principle 11). The stellar-flux derivation retired the inline 1361 constant, re-pinning only the sky-armed living scenario (a stated re-pin to the derived total solar irradiance) and holding the four base pins byte-identical; the eccentricity, precession, and period-derivation slices each re-pinned or held under the same discipline, with the period wiring fully byte-neutral. The calibrations are reserved with their bases in the audit log: the mass-luminosity exponent, the back-radiation floor, the Bond albedo, the uniform emissivity, the representability cap, the obliquity, eccentricity, and perihelion phase, and the two per-world stellar inputs; the solar luminosity, solar mass, and astronomical unit are cited reference constants, and the Kepler iteration cap and pi precision are fixed integer engine bounds. Decision in Part 18.1, built in `crates/sim` (`astro.rs`, `environ.rs`, `clock.rs`); consolidation block in the audit log.

What remains to be proven. The honest limits are named at the site: the surface emissivity and albedo are uniform planetary values, not yet the per-cell per-material floor read; the back-radiation is a reserved floor, not yet derived from the atmosphere's greenhouse optical depth; and one orbital phase serves both the seasonal declination and the mean anomaly, so the finer equation-of-center for the seasonal angle is a later refinement (the precession offset on the distance term is exact). The largest open piece is the deep-time secular drift of the orbital data (the Milankovitch envelope): the derive map requires the orbital-element evolution to be derived from the secular frequency spectrum, not authored as a harmonic curve, so it is flagged R-CELESTIAL-SECULAR for a future celestial-mechanics arc (a many-body secular-perturbation substrate plus the figure torque) rather than fitted here. Resolved, built in `crates/sim`; the calibrations are reserved.
...
**Deterministic language procedures: rule ordering, opacity, and modality-neutral form (Parts 33, 62.12)**
- Chomsky, N., and Halle, M. (1968). The Sound Pattern of English. Harper and Row (reprinted MIT Press, 1991). The ordered-rule model of phonology, the basis for composing regular form changes as a sequence whose result depends on order.
- Kiparsky, P. (1968). Linguistic universals and linguistic change. In Bach, E., and Harms, R. T. (eds.), Universals in Linguistic Theory, 170-202. Holt, Rinehart and Winston; with Kiparsky, P. (1973). Abstractness, opacity, and global rules. In Fujimura, O. (ed.), Three Dimensions of Linguistic Theory, 57-86. TEC, Tokyo. The feeding and bleeding rule interactions and the opacity of counterfeeding and counterbleeding, the linguistic content of the chronological-ordering pin.
- Kenstowicz, M., and Kisseberth, C. (1979). Generative Phonology: Description and Theory. Academic Press. The standard treatment of rule ordering and its determinacy.
- Stokoe, W. C. (1960). Sign Language Structure: An Outline of the Visual Communication Systems of the American Deaf. Studies in Linguistics, Occasional Papers 8. University of Buffalo. That a signed language has contrastive sublexical parameters (location, handshape, movement), the evidence that phonology is modality-general.
- Brentari, D. (1998). A Prosodic Model of Sign Language Phonology. MIT Press. A feature model of sign-language phonology, confirming the form substrate generalizes across modality.
- Frishberg, N. (1975). Arbitrariness and iconicity: historical change in American Sign Language. Language, 51(3), 696-719. Regular, systematic formational change in a signed language over time, the evidence that the drift and rule-ordering mechanism is not specific to sound.

**Communication modality, channels, and signed-language emergence (Parts 33, 62.13)**
- Bradbury, J. W., and Vehrencamp, S. L. (2011). Principles of Animal Communication, 2nd ed. Sinauer Associates / Oxford University Press. The acoustic, visual, chemical, tactile, and electric channel taxonomy and the treatment of each channel as separable production, propagation, and reception stages, the basis for a channel as a derived production-and-reception pairing over a medium rather than a closed list.
- Partan, S. R., and Marler, P. (2005). Issues in the classification of multimodal communication signals. The American Naturalist, 166(2), 231-245. The classification of multimodal signals by receiver response, grounding the independence of production and reception.
- Crampton, W. G. R. (2019). Electroreception, electrogenesis and electric signal evolution. Journal of Fish Biology, 95(1), 92-134. The electric channel with separate production (electric organ) and reception (tuberous and ampullary electroreceptors) pathways, the concrete case that production and reception are separable, with the jamming avoidance response as channel-level interference physics.
- Hockett, C. F. (1960). The origin of speech. Scientific American, 203(3), 88-96. The vocal-auditory channel as one descriptive design feature of language, ranking no channel, the grounding for the Principle 9 guard that the modality substrate carries no sophistication ordering.
- Goldsmith, J. A. (1976). Autosegmental Phonology. PhD thesis, MIT (published 1979, Garland Press). The independence of feature tiers, the basis for representing a form primitive as an order-free bundle of simultaneous feature values.
- Clements, G. N. (1985). The geometry of phonological features. Phonology Yearbook, 2, 225-252. Feature geometry, the structured-feature-bundle basis for the segment.
- Sandler, W., and Lillo-Martin, D. (2006). Sign Language and Linguistic Universals. Cambridge University Press. The modality-general account of phonology and the simultaneity of sign sublexical structure.
- Emmorey, K., Borinstein, H. B., Thompson, R., and Gollan, T. H. (2008). Bimodal bilingualism. Bilingualism: Language and Cognition, 11(1), 43-61. Bimodal bilinguals overwhelmingly code-blend (simultaneous sign and speech) rather than switch, blends being semantically congruent and onset-synchronous with one base grammar, the basis for the code-blend model.
- Goldin-Meadow, S. (2003). The Resilience of Language: What Gesture Creation in Deaf Children Can Tell Us About How All Children Learn Language. Psychology Press. Homesign as structured language from no linguistic input, the basis for the homesign-to-community arc and the retain-the-concept-layer principle in modality shift.
- Sandler, W., Meir, I., Padden, C., and Aronoff, M. (2005). The emergence of grammar: systematic structure in a new language. PNAS, 102(7), 2661-2668. The Al-Sayyid Bedouin Sign Language case, a community language emerging from shared deafness, the village-sign emergence model. Stokoe (1960), Brentari (1998), and Frishberg (1975) above, and Senghas and Coppola (2001) with Kegl, Senghas, and Coppola (1999) in the procedural-language group, are the further grounding for this item.

**Semantic primes and the differentiated founding floor (Parts 33, 62.24)**
- Wierzbicka, A. (1996). Semantics: Primes and Universals. Oxford University Press. The Natural Semantic Metalanguage claim that a set of semantic primes is lexicalised in every language, the basis for the prime set as a universal grounding floor.
- Goddard, C., and Wierzbicka, A. (eds.) (2002). Meaning and Universal Grammar: Theory and Empirical Findings, 2 vols. John Benjamins. The cross-linguistic testing of prime exponents across typologically diverse languages, the evidence that the floor is a mature-language universal (tested on established, not founding, languages).
- Goddard, C. (2012). Semantic primes, semantic molecules, semantic templates: key concepts in the NSM approach to lexical typology. Linguistics, 50(3), 711-743. Semantic molecules, the composite concepts built recursively on the primes, the source-native richness axis above the floor.
- Pyers, J. E., and Senghas, A. (2009). Language promotes false-belief understanding: evidence from learners of a new sign language. Psychological Science, 20(7), 805-812. Nicaraguan Sign Language cohort one produced few or no mental-state (THINK, KNOW) forms while desire (WANT) was universal, the category closing in the second cohort, the decisive evidence that founding coverage of the prime floor is differentiated and accretes across generations.
- Sandler, W., Meir, I., Dachkovsky, S., Padden, C., and Aronoff, M. (2011). The emergence of complexity in prosody and syntax. Lingua, 121(13), 2014-2033. First-generation signers did not systematically mark interclausal dependency (including conditionals), the same generational lag for logical and subordinating (IF, BECAUSE) primes.
- Bartsch, K., and Wellman, H. M. (1995). Children Talk About the Mind. Oxford University Press. The staged order of mental-state vocabulary in child first-language acquisition, desire before epistemic, THINK and KNOW acquired last, the ontogenetic parallel to the founding lag.
- Trudgill, P. (2011). Sociolinguistic Typology: Social Determinants of Linguistic Complexity. Oxford University Press. Small, isolated, low-contact communities trend toward more structural complexity, not less, the finding that rejects a cohort-size cap that thins the floor.
- Bohnemeyer, J. (2003). NSM without the strong lexicalization hypothesis. Theoretical Linguistics, 29(3), 211-222. Yucatec Maya lacks dedicated exponents for BEFORE and AFTER, a challenger to the strict universality of every prime, the basis for treating the floor's membership and count as a revisable, citable axiom rather than a fixed constant.
...
**Recursive theory of mind: false belief, bounded recursion, and computational theory of mind (Parts 37, 62.11)**
- Wimmer, H., and Perner, J. (1983). Beliefs about beliefs: representation and constraining function of wrong beliefs in young children's understanding of deception. Cognition, 13(1), 103-128. The unexpected-transfer (Maxi) false-belief task, the origin of the paradigm the battery rests on.
- Baron-Cohen, S., Leslie, A. M., and Frith, U. (1985). Does the autistic child have a theory of mind? Cognition, 21(1), 37-46. The Sally-Anne adaptation, the canonical first-order false-belief test the battery uses.
- Perner, J., and Wimmer, H. (1985). "John thinks that Mary thinks that...": attribution of second-order beliefs by 5- to 10-year-old children. Journal of Experimental Child Psychology, 39(3), 437-471. The second-order ice-cream-van task whose correct answer is unreachable by any first-order shortcut, the formal divergence-from-projection target.
- Kinderman, P., Dunbar, R., and Bentall, R. P. (1998). Theory-of-mind deficits and causal attributions. British Journal of Psychology, 89(2), 191-204; with Stiller, J., and Dunbar, R. I. M. (2007). Perspective-taking and memory capacity predict social network size. Social Networks, 29(1), 93-104. The orders-of-intentionality ceiling near fifth order, the upper anchor for the depth bound.
- O'Grady, C., Kliesch, C., Smith, K., and Scott-Phillips, T. C. (2015). The ease and extent of recursive mindreading, across implicit and explicit tasks. Evolution and Human Behavior, 36(4), 313-322. Under a forced-choice paradigm mindreading reaches at least the seventh order, so the fifth-order ceiling is partly a task artifact, the caution against treating a clean ceiling as fixed.
- Camerer, C. F., Ho, T.-H., and Chong, J.-K. (2004). A cognitive hierarchy model of games. The Quarterly Journal of Economics, 119(3), 861-898. Strategic reasoning concentrated at one to two steps with a Poisson mean near one and a half, the basis for an acuity distribution that does not recurse to the cap.
- Gmytrasiewicz, P. J., and Doshi, P. (2005). A framework for sequential planning in multi-agent settings (interactive POMDPs). Journal of Artificial Intelligence Research, 24, 49-79. The finitely-nested belief tower terminating at a level-zero sub-intentional model, the basis for a finite depth bound with a principled default below it; the decreasing marginal effect of deeper nesting is a characterization of the framework rather than a verbatim result.
- Aumann, R. J. (1976). Agreeing to disagree. The Annals of Statistics, 4(6), 1236-1239; with Rubinstein, A. (1989). The electronic mail game: strategic behavior under almost common knowledge. The American Economic Review, 79(3), 385-391. That true common knowledge is unreachable, the basis for finitely-iterated common belief and the shared-but-not-common distinction at the depth bound.
- Baker, C. L., Saxe, R., and Tenenbaum, J. B. (2009). Action understanding as inverse planning. Cognition, 113(3), 329-349; with Baker, C. L., Jara-Ettinger, J., Saxe, R., and Tenenbaum, J. B. (2017). Rational quantitative attribution of beliefs, desires and percepts in human mentalizing. Nature Human Behaviour, 1, 0064. Bayesian theory of mind inferring belief from access and percept, the model the integer log-odds engine deterministically approximates.
- Game and simulation precedent for nested belief and deception: Crusader Kings (per-character secrets, schemes, and discoverable hooks, the closest precedent for tracking who knows what and for a fabrication that can be seen through). Adopted and avoided as noted in Part 37.

**Dialogue, speech acts, and conversation (Parts 9, 33, 37, 62.14)**
- Austin, J. L. (1962). How to Do Things with Words. The William James Lectures (1955), ed. J. O. Urmson. Clarendon Press / Harvard University Press. The locution-illocution-perlocution distinction and felicity conditions, the basis for a move as an act that misfires when its conditions fail and for a lie as an insincere assertion rather than a distinct kind.
- Searle, J. R. (1969). Speech Acts: An Essay in the Philosophy of Language. Cambridge University Press. The rule-governed account of illocutionary acts, the basis for force as a structured composition of conditions over existing state.
- Searle, J. R. (1976). A classification of illocutionary acts. Language in Society, 5(1). The five-way taxonomy whose own contested status grounds the data-defined move registry over a closed enum.
- Bach, K., and Harnish, R. M. (1979). Linguistic Communication and Speech Acts. MIT Press. A Gricean inferential account with a different taxonomy, the contrast showing the carve-up is not single.
- Core, M. G., and Allen, J. F. (1997). Coding dialogs with the DAMSL annotation scheme. AAAI Fall Symposium on Communicative Action in Humans and Machines. The multi-layer dialogue-act scheme grounding adjacency and the response loop.
- Bunt, H., et al. (2012). ISO 24617-2: a semantically-based standard for dialogue annotation. LREC 2012; ISO 24617-2:2012. The standard's multidimensional, explicitly extensible dialogue-act sets, the strongest warrant for an open, data-defined move inventory.
- Sacks, H., Schegloff, E. A., and Jefferson, G. (1974). A simplest systematics for the organization of turn-taking for conversation. Language, 50(4). Turn-taking as locally and party-administered, the basis for a turn as one move and a conversation as an emergent query rather than a script.
- Schegloff, E. A. (2007). Sequence Organization in Interaction: A Primer in Conversation Analysis, Vol. 1. Cambridge University Press. The adjacency pair, the basis for the in-reply-to link and threading.
- Clark, H. H., and Brennan, S. E. (1991). Grounding in communication. In Perspectives on Socially Shared Cognition. American Psychological Association. The contribution model and the grounding criterion through positive evidence, the basis for grounding as said-evidence convergence.
- Clark, H. H. (1996). Using Language. Cambridge University Press. Language use as joint action with grounding and common ground.
- Schegloff, E. A., Jefferson, G., and Sacks, H. (1977). The preference for self-correction in the organization of repair in conversation. Language, 53(2). The organization of repair, the basis for repair as a move and the reserved self-repair bias.
- Sperber, D., and Wilson, D. (1986; 2nd ed. 1995). Relevance: Communication and Cognition. Blackwell. The ostensive-inferential model, the basis for a response as inference over a move rather than a decoded transmission.
- Cohen, P. R., and Perrault, C. R. (1979). Elements of a plan-based theory of speech acts. Cognitive Science, 3(3). Speech acts as planning operators over the hearer's beliefs and goals, the basis for speaking as a utility-AI intent integrated with physical action.
- Allen, J. F., and Perrault, C. R. (1980). Analyzing intention in utterances. Artificial Intelligence, 15(3). Recovering a speaker's intention from an utterance, the listener-side and theory-of-mind half.
- Frank, M. C., and Goodman, N. D. (2012). Predicting pragmatic reasoning in language games. Science, 336(6084). The rational-speech-act view of speaking as a utility choice over informativeness, the conceptual grounding for addressee-and-content selection by utility, used as grounding with the canonical path staying integer log-odds.
- Mark, D. (2009). Behavioral Mathematics for Game AI. Charles River Media. Response curves over considerations combined into a comparable score, the utility-AI substrate the communicative action reuses.
- Dung, P. M. (1995). On the acceptability of arguments and its fundamental role in nonmonotonic reasoning, logic programming and n-person games. Artificial Intelligence, 77(2). Abstract argumentation, the grounding for the deferred deliberation coupling.
- Nash, J. F. (1950). The bargaining problem. Econometrica, 18(2); with Rubinstein, A. (1982), Perfect equilibrium in a bargaining model, Econometrica, 50(1). The cooperative solution and the alternating-offers equilibrium, the convergence grounding for the deferred negotiation coupling, approximated rather than implemented as a protocol. The opinion-dynamics grounding for the deferred persuasion coupling (Friedkin-Johnsen, Deffuant, Hegselmann-Krause) is the set already cited for the axiomatic-belief group (62.4).
...
**Biology, composition, and edibility substrate (Part 41, R-PHYS-BIO)**
- USDA FoodData Central; and Greenfield, H., and Southgate, D. A. T. (2003). Food Composition Data, 2nd ed., FAO/INFOODS. The gross-composition axes and their natural ranges.
- Atwater, W. O., and Merrill, A. L., with Watt, B. K. USDA Handbook 74; and the FAO food-energy methodology papers (77, 66, 91). The energy-density and macronutrient-conversion basis.
- Van Soest, P. J., Robertson, J. B., and Lewis, B. A. (1991). Methods for dietary fiber, neutral detergent fiber, and nonstarch polysaccharides in relation to animal nutrition. Journal of Dairy Science, 74(10). The structural-versus-non-structural carbohydrate split.
- WHO/FAO/UNU (2007). Protein and amino acid requirements in human nutrition, WHO Technical Report Series 935; and WHO/FAO (2004), Vitamin and mineral requirements in human nutrition, 2nd ed. The essentiality registries and the requirement basis.
- Klaassen, C. D. (ed.). Casarett and Doull's Toxicology: The Basic Science of Poisons. The dose-response and per-class toxicology basis; with Calabrese, E. J., and Baldwin, L. A. on hormesis and the biphasic dose response.
- Wink, M. (2003), Evolution of secondary metabolites; Friedman, M. (2006), on glycoalkaloids; Agrawal, A. A. et al. (2012); Francis, G. et al. (2002), on saponins; Barbehenn, R. V., and Constabel, C. P. (2011), on tannins. The plant chemical-defense classes seeding the terran toxin registry.
- Stevens, C. E., and Hume, I. D. Comparative Physiology of the Vertebrate Digestive System; with the Kleiber metabolic-scaling line. The consumer physiology, fermentation, and allometric-scaling basis.
- Alon, U. (2007). An Introduction to Systems Biology, Appendix A (the Hill equation); and Liebig's law of the minimum. The closed-form law shapes.

**Mechanics, materials, and energy substrate (Part 41, R-PHYS-MECH)**
- Ashby, M. F., and Jones, D. R. H. *Engineering Materials 1*; and Ashby, M. F., *Materials Selection in Mechanical Design* (the property charts). The bulk-material axes and their ranges.
- ASM *Metals Handbook*; Roark's *Formulas for Stress and Strain*; Timoshenko, S. P., *Theory of Elastic Stability* (Euler buckling); Gere, J. M., and Goodno, B. J., *Mechanics of Materials*. The strength, stress, and buckling laws.
- Atkins, A. G. (2009). *The Science and Engineering of Cutting*. The cut-penetration work model.
- Bowden, F. P., and Tabor, D. *The Friction and Lubrication of Solids*; Archard, J. F. (1953), contact and rubbing wear; Rabinowicz, E., *Friction and Wear of Materials*. The friction and wear laws.
- Goldsmith, W., *Impact*; Johnson, K. L., *Contact Mechanics* (Hertz); Griffith, A. A. (1921) and Irwin, G. R. (1957) (fracture); von Mises, R. (1913) (the shear-yield criterion). The impact, curved-contact, fracture, and shear laws.
- CRC *Handbook of Chemistry and Physics* (thermophysical tables); Incropera, F. P., and DeWitt, D. P., *Fundamentals of Heat and Mass Transfer*; Carslaw, H. S., and Jaeger, J. C., *Conduction of Heat in Solids*; Turns, S. R., *An Introduction to Combustion*; Babrauskas, V., *Ignition Handbook*. The conduction, sensible-heat, phase-change, and combustion laws.
- The Wood Handbook (FPL-GTR-190) for timber anisotropy. The Q32.32 discipline is the project's own (`crates/core/src/fixed.rs`, Parts 3 and 58).

**Biosphere generation and deep-time radiation (Part 25, R-BIOSPHERE)**

- Hutchinson, G. E. (1957), "Concluding Remarks," *Cold Spring Harbor Symposia* (the n-dimensional niche and the per-axis tolerance response the tent approximates); Liebig, J. von, *Organic Chemistry in its Application to Agriculture and Physiology* (the law of the minimum, for the biome-fit and net-nutrition minimum-fold); MacArthur, R., and Levins, R. (1967) (limiting similarity, for the niche-openness floor); Rosenzweig, M. L., *Species Diversity in Space and Time* (species-area and productivity-diversity, for the diversity targets). The generate-and-validate generator and biome-fit.
- Hill, A. V. (1910), and Alon, U., *An Introduction to Systems Biology* (the Hill equation with the integer-coefficient restriction, for the harm law and hormesis); Calabrese, E. J., and Baldwin, L. A. (the biphasic hormesis dose-response, for the reserved medicinal credit); Casarett and Doull's *Toxicology* (Klaassen, ed.), Wink, M. (2003), and Friedman, M. (2006) (dose-dependent and preparation-dependent toxicity, the preparation-lability matrix). The edibility dynamics.
- Kleiber, M. (1932), "Body size and metabolism," and Peters, R. H. (1983), *The Ecological Implications of Body Size* (the three-quarter metabolic-scaling allometry, for the requirement coupling). The consumer physiology.
- Wright, S. (1931), and Fisher, R. A. (1930) (the Wright-Fisher model); Kimura, M., *The Neutral Theory of Molecular Evolution* (drift, founder effect, and effective size); Orr, H. A. (1995), "The population genetics of speciation" (the snowball accumulation of Dobzhansky-Muller incompatibilities); Coyne, J. A., and Orr, H. A. (2004), *Speciation*, with Dobzhansky (1937) and Muller (1942) (reproductive isolation and the sterilizing-versus-lethal split); Haldane, J. B. S. (1922) (the reserved sex-linked asymmetry, deferred). The pre-dawn radiation, speciation, and extinction.
- Stevens, C. E., and Hume, I. D., *Comparative Physiology of the Vertebrate Digestive System* (primitive assimilation, fermentation, and retention); USDA FoodData Central, FAO/INFOODS, and Greenfield, H., and Southgate, D. A. T., *Food Composition Data* (FAO 2003) (the tissue-composition draw distributions); WHO/FAO, *Protein and Amino Acid Requirements in Human Nutrition*, TRS 935 (2007), with the vitamin and mineral requirement tables (the requirement-binding magnitudes); FAO/WHO amino-acid scoring (DIAAS and PDCAAS) and Van Soest, P. J., et al. (1991) (the per-lump child-share priors and structural carbohydrate). The organism composition and consumer wiring. The Q32.32, counter-RNG, and content-addressing discipline is the project's own (`crates/core`, `crates/sim/src/genome.rs`, `stocks.rs`, `lineage.rs`, Parts 3, 25, 58).

**Evolved behaviour: homeostasis, controllers, and selection (Part 8, R-BEHAVIOR-EVOLVE)**

- Cannon, W. B. (1929), "Organization for physiological homeostasis," *Physiological Reviews*, 9(3), and *The Wisdom of the Body* (1932) (homeostasis, the physiological basis for needs as draining reserves rather than authored drives); Ashby, W. R. (1952), *Design for a Brain: The Origin of Adaptive Behaviour* (ultrastability, adaptive behaviour as the maintenance of essential variables within viable bounds, the conceptual root of survival-as-fitness). The homeostatic physiology and the death-floor semantics.
- Gibson, J. J. (1979), *The Ecological Approach to Visual Perception* (affordances, the operations the environment and the body's morphology permit, the affordance-versus-outcome boundary of Principle 9, also cited under design theory). The affordance layer.
- Braitenberg, V. (1984), *Vehicles: Experiments in Synthetic Psychology* (complex-seeming behaviour from simple sensor-to-motor couplings, the lineage of the reaction-norm controller); Beer, R. D., and Gallagher, J. C. (1992), "Evolving dynamical neural networks for adaptive behavior," *Adaptive Behavior*, 1(1), and Beer, R. D. (1995), "A dynamical systems perspective on agent-environment interaction," *Artificial Intelligence*, 72 (evolved continuous-time recurrent neural controllers for adaptive behaviour, the model for the recurrent-network graduation). The controller representation.
- Sims, K. (1994), "Evolving virtual creatures," *SIGGRAPH '94* (physics-based evolution of morphology and control, the model for physics-as-fitness, also cited under artificial life); McFarland, D., and Bösser, T. (1993), *Intelligent Behavior in Animals and Robots* (behaviour as the management of physiological state); Sibly, R. M., and McFarland, D. (1976), "On the fitness of behaviour sequences," *American Naturalist*, 110 (behaviour scored by its contribution to survival, not by resemblance to an expected act). The homeostatic-survival fitness.
- Schlichting, C. D., and Pigliucci, M. (1998), *Phenotypic Evolution: A Reaction Norm Perspective*, Sinauer (the reaction norm as the genotype-to-phenotype mapping over an environment, the biological grounding of the linear controller); Price, G. R. (1970), "Selection and covariance," *Nature*, 227 (the covariance of a trait with fitness, the selection-gradient basis); DeAngelis, D. L., and Grimm, V. (2014), "Individual-based models in ecology after four decades," *F1000Prime Reports* (the individual and sampled-episode tier for behaviour over deep time); Stephens, D. W., and Krebs, J. R. (1986), *Foraging Theory*, Princeton University Press (optimal foraging, the taxis-to-resource behaviour the evolved controller reconstructs). The selection loop and its scoring. The Wright-Fisher selection recurrence and the counter-RNG and Q32.32 discipline are the project's own (`crates/sim/src/controller.rs`, `evolve.rs`, `genome.rs`, `crates/core`, Parts 3, 25, 58).

**The body: anatomy, tissue materials, wounds, and fluids (Part 35, R-BUILD-PHYS, R-WOUND, R-FLUID)**

- Yamada, H. (1970), *Strength of Biological Materials*, Williams and Wilkins (the canonical compendium of tissue material properties, the basis for the reserved tissue axes); Currey, J. D. (2002), *Bones: Structure and Mechanics*, Princeton University Press (bone stiffness, strength, and fracture, for the hardest structural layer); Gordon, J. E. (1976), *The New Science of Strong Materials*, and (1978), *Structures: Or Why Things Don't Fall Down*, Penguin (a tissue as a material, the materials-as-structures view). The tissue-material floor a wound reads.
- Kroman, A. M., and Symes, S. A. (2013), "Investigation of skeletal trauma," in *Research Methods in Human Skeletal Biology*; DiMaio, V. J. M., and DiMaio, D. (2001), *Forensic Pathology*, CRC Press (the sharp, blunt, and thermal trauma-mechanism distinction as a physical taxonomy read from the insult, not a stored kind, the grounding for the measured damage modes). The wound as a measured mechanism.
- Guyton, A. C., and Hall, J. E., *Textbook of Medical Physiology* (haemorrhagic shock and the survivable fractional blood loss, for the reserved fluid critical fractions and bleed dynamics). The fluid-loss model.
- Adams, T., and Adams, Z., *Dwarf Fortress* (the data-defined material-and-tissue body and the wound model resolved through material properties, the canonical game precedent this arc generalizes to a data-driven anatomy and a physics-measured wound). The design lineage. The mechanical, fracture, thermal, contact, cutting, kinetic-energy, and reach laws a wound and a strike are measured from are the resolved R-PHYS-MECH floor (cited above), read through `crates/physics/src/laws.rs`; the per-part body, the measured wound, the fluids, and the strike are the project's own (`crates/sim/src/body.rs`, Parts 35, 41, 58).

**The fluid, chemistry, and optics floor (Part 41, Part 58, wave 2)**

- Batchelor, G. K., *An Introduction to Fluid Dynamics*; White, F. M., *Fluid Mechanics* and *Viscous Fluid Flow*; Munson et al., *Fundamentals of Fluid Mechanics*; Hoerner, S. F., *Fluid-Dynamic Drag*; Anderson, J. D., *Fundamentals of Aerodynamics* (lift and the lifting-line origin of the coefficient); Fung, Y. C., *Biomechanics: Circulation* (Poiseuille vessel flow); Reynolds, O. (1883) (the laminar-turbulent transition). The fluids axes and laws.
- Brutsaert, W., *Evaporation into the Atmosphere*, and Penman, H. L. (1948) (the Dalton bulk mass-transfer evaporation proxy); the Smithsonian Meteorological Tables and the CRC psychrometric tables (the saturation-vapour reference and slope); Pierrehumbert, R. T., *Principles of Planetary Climate* (radiative equilibrium and the energy balance). The weather water cycle and radiative-equilibrium temperature.
- CRC *Handbook of Chemistry and Physics* (thermochemical, electrochemical, and optical constants); Atkins, P., *Physical Chemistry* (reaction enthalpy by Hess's law, the Carnot limit); the standard electrode-potential and dissolution tables. The chemistry axes and laws.
- Incropera, F. P., and DeWitt, D. P., *Fundamentals of Heat and Mass Transfer* (Newton convective cooling and the Stefan-Boltzmann radiant law); Hecht, E., *Optics*; Wyszecki, G., and Stiles, W. S., *Color Science*; Bowmaker, J. K. (opsin sensitivities). The optics, radiant-heat, and colour axes and laws.
- Kinsler, L. E., et al., *Fundamentals of Acoustics* (the speed of sound, acoustic impedance, and transmission through a medium). The acoustic axes and laws. The Q32.32 closed-form-integer discipline (including the two-nested-sqrt fourth root and the piecewise-linear and lumped-coefficient proxies for the deferred transcendentals) is the project's own (`crates/physics/src/laws.rs`, Parts 3, 55, 58).

**The electricity-and-magnetism floor (Part 41, Part 55, Part 58, wave 3)**

- Griffiths, D. J., *Introduction to Electrodynamics* (Coulomb's law, Ohm's law, capacitor and inductor energy, the Laplace and Lorentz forces, the dipole torque, and Faraday and inductive EMF); Purcell, E. M., and Morin, D. J., *Electricity and Magnetism*. The core electricity and magnetism laws.
- Kittel, C., *Introduction to Solid State Physics*, and the CRC *Handbook of Chemistry and Physics* resistivity and dielectric tables (resistivity, the conductor-to-insulator envelope, dielectric strength); Jiles, D., *Introduction to Magnetism and Magnetic Materials* (permeability, the B-H loop deferred). The material axes.
- Fink, D. G., and Beaty, H. W., *Standard Handbook for Electrical Engineers*, and Horowitz, P., and Hill, W., *The Art of Electronics* (inductance, coupling, the transformer, and practical circuit envelopes). The circuit and induction axes and laws.
- Bard, A. J., and Faulkner, L. R., *Electrochemical Methods*, and Atkins, P., *Physical Chemistry* (the galvanic cell EMF from the electromotive series, the volt promotion, the Nernst correction deferred). The battery-EMF unification.
- Ohm, G. S. (1827); Faraday, M. (1831); NIST CODATA (the Coulomb constant and vacuum permeability). The historical laws and the reserved constants. The five-base `Dimension` monomial and the Q32.32 closed-form-integer discipline (the SI ampere as the fifth base, the per-tick induction finite difference) is the project's own (`crates/physics/src/lib.rs`, `laws.rs`, Parts 3, 55, 58).

**GPU determinism and fixed-point arithmetic (Part 3, Part 5, R-GPU-CANON-PIN)**

- Volder, J. E. (1959). The CORDIC trigonometric computing technique. IRE Transactions on Electronic Computers, EC-8(3). The shift-and-add algorithm pinned for the trigonometric and inverse-trigonometric family.
- Walther, J. S. (1971). A unified algorithm for elementary functions. Proceedings of the Spring Joint Computer Conference. The unified CORDIC that covers the transcendental set from one iteration.
- Muller, J.-M. (2016). Elementary Functions: Algorithms and Implementation (3rd ed.). Birkhauser. Range reduction and fixed-point series for the exponential and the logarithm, the basis for their precision targets.
- Knuth, D. E. (1997). The Art of Computer Programming, Volume 2: Seminumerical Algorithms (3rd ed.). Addison-Wesley. Multiple-precision multiplication and restoring long division, the basis for the u32-limb multiply and the ninety-six-step divide.
- Goldberg, D. (1991). What every computer scientist should know about floating-point arithmetic. ACM Computing Surveys, 23(1). The non-associativity of floating-point summation that mandates the integer-only canonical path.
- Tracel AI, CubeCL: multi-platform GPU compute in Rust (`https://github.com/tracel-ai/cubecl`). The `#[cube]` kernel framework the canonical GPU path is authored in. The u32-limb multiply and ninety-six-step divide emulation, the Stage 0 bit-identity gate, and the diffusion field kernel are the project's own (`crates/gpu`, Parts 3.4, 5.4).

**Celestial mechanics and insolation (Part 18.1, 62.26)**

- International Astronomical Union (2015). Resolution B3 on recommended nominal conversion constants for selected solar and planetary properties. The nominal solar luminosity (3.828e26 W) and nominal solar mass parameter anchoring the stellar-flux derivation.
- International Astronomical Union (2012). Resolution B2 redefining the astronomical unit of length as exactly 149,597,870,700 metres. The orbital-distance anchor.
- Kopp, G., and Lean, J. L. (2011). A new, lower value of total solar irradiance: evidence and climate significance. Geophysical Research Letters, 38(1). The measured total solar irradiance (about 1361 W/m^2) the Mirror derivation reconstructs.
- Kepler, J. (1609). Astronomia Nova. The equal-area law and the equation `M = E - e sin E` the eccentric-anomaly solve inverts.
- Murray, C. D., and Dermott, S. F. (1999). Solar System Dynamics. Cambridge University Press. The eccentric and mean anomaly, the orbital-distance relation, and the fixed-point solution of Kepler's equation.
- Kippenhahn, R., Weigert, A., and Weiss, A. (2012). Stellar Structure and Evolution (2nd ed.). Springer. The main-sequence mass-luminosity relation the per-world stellar flux scales by.
- Milankovitch, M. (1941). Canon of Insolation and the Ice-Age Problem. The classical theory of orbitally-driven insolation change (the outcome the R-CELESTIAL-SECULAR substrate must derive rather than author).
- Berger, A. (1978). Long-term variations of daily insolation and Quaternary climatic changes. Journal of the Atmospheric Sciences, 35(12). The quasi-periodic series representation of eccentricity, obliquity, and precession, and why it is a fit to a specific planetary configuration.
- Laskar, J., and Robutel, P. (1993). The chaotic obliquity of the planets. Nature, 361. The obliquity-stability result: a moonless Earth wanders chaotically in tilt while the Moon pins ours, the reason the secular spectrum (not an authored envelope) governs the evolution.
- Laskar, J., Robutel, P., Joutel, F., Gastineau, M., Correia, A. C. M., and Levrard, B. (2004). A long-term numerical solution for the insolation quantities of the Earth. Astronomy and Astrophysics, 428(1). The secular-perturbation solution the derive-correct Milankovitch envelope (R-CELESTIAL-SECULAR) would emerge from.
