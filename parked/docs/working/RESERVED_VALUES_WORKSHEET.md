...
## §18. Flagged future derivation: R-ORGAN-FLUX, the inter-organ flux graph

The body models anatomy at two tiers today. The aggregate composition vector (`anatomy.rs`) is what reserves, breathing, and (once wired) metabolism derive from, and the promoted per-part physical body (`body.rs`, the R-BUILD-PHYS, R-WOUND, and R-FLUID arc) is a set of typed parts of layered tissue materials that fracture, lose their functions, and bleed by their own fluids. What neither tier carries is plumbing between organs: a circulation that moves fluid between named organs, a respiration that oxygenates specific tissues, so a localized wound (a pierced lung, a severed artery) cascades physiologically through a modeled flow rather than removing a part function and opening a bleed. R-ORGAN-FLUX is that deeper derivation, flagged by the owner as an explicit future item to look into, not scheduled now. The binding constraint is the same no-tag rule the two built tiers already keep: an organ moves fluid because a flux law (a pump, a Fick or Poiseuille transport) reads its geometry and the fluid's properties, never because it is tagged "heart" or "lung." It extends the existing substrate (more composition axes, an organ connectivity graph, per-organ flux laws over the physics floors) with no new tags. Recorded here so the depth question is explicit and the derive-not-tag guardrail travels with it.
