...
- COMMIT AND PUSH ONLY ON YOUR FEATURE BRANCH. NEVER `main`. A MERGE TO `main` IS THE OWNER'S CALL, EVERY TIME, and neither you nor the coordinator can issue one; the owner may delegate a specific landing, and that delegation is spent when it is used and does not generalize to the next one. This corollary was IMPLIED AND UNWRITTEN until an agent read a gate pass as a merge approval, which is the unenforced-validity-domain shape: documented in spirit, absent in fact. It happened to cost nothing only because the owner had covered it directly, and an invariant that survives on someone else's vigilance is not an invariant.
- "GATE PASSED" means your slice is accepted as correct work. It NEVER means land it. The coordinator used to write "SIGNED OFF" for this, and on a PR about the assembly's MERGE HISTORY that word did two jobs on one thread (`the_merge_conserves_mass_to_the_bit`, the merge epoch, "do not merge them"), one of which is a git operation. Renamed rather than explained, because a name that reads two ways will be read both ways: the same discharge the diamond gate applies to `friction` (the coefficient versus the force) and `weight` (mg versus a neural weight).
