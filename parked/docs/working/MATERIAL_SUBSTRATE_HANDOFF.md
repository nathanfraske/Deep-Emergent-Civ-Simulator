...
- **One identity space.** A tool or relic mints a `StableId` from the same `Registry` a being uses (`core/id.rs:87`), so legend and provenance attach exactly as they do to a being. Do not fork identity. FIDELITY GOALS (owner 2026-07-05, deferred, held to emergence, see CONSENSUS_ROADMAP.md): extend the single-Coord3 location to a DERIVED multi-cell EXTENT so a large object or a grown organism occupies its footprint from its geometry against the cell size (a boulder's volume, a tree's grown canopy), never an authored size; and read matter and fire at the PER-PART structure (each part's own geometry and composition) so a thing combusts, wears, and decays part by part with intra-object spread, rather than collapsing to one cell Composition.
...
### Round 4: the completeness pass (three vantages: designer, anthropologist, engineer, 2026-07-05)

Asked what whole CATEGORY the whole survey still missed. All three independently returned INCOMPLETE and converged on PHYSICS-REGIME gaps that the mode-based sweeps could not surface, because these are distinct physical regimes rather than "what a creature does" verbs. After discarding overlaps, the missing dimensions, each still one rule (need + grown affordance + matter-physics, founder-zero):

- CAST / MOLD (negative-space replication): pour a fluid or soft substance into a negative cavity so it sets to that shape, a fabrication paradigm distinct from CRAFT's direct positive shaping (the target never contacts the final form until release). It gives reproducible geometry (standardized forms) and pairs directly with SMELT (cast molten metal). Named by two of three critics. Physics: fluid-to-solid fill, shrinkage, interfacial release. Home: materials-core. The strongest completeness find.
- YIELD DEFORMATION (forge / bend / stamp): plastically deform a ductile material below its melting point and below fracture (cold-hammer metal, bend green wood), the solid-state elastic-plastic regime distinct from KNAP's brittle fracture and SMELT's phase change. This is the ductile-working layer, how a being works metal or bends wood without melting it. Physics: yield stress, work-hardening, dislocation glide. Home: materials-core.
- STORED ELASTIC ENERGY (bow / sling / spring / torsion): deliberately deform matter to store mechanical energy for delayed, directed release, into propulsion (a bow or sling launching a projectile, compounding THROW) or rotation (a bow-drill). Distinct from TRAP/SNARE (capture-and-release) and LEVER (instantaneous). Named by two of three. Physics: elastic strain energy, hysteresis. Home: materials-core.
- DIRECTED FLUID FLOW / HYDRAULICS (syphon / channel / water-power): actively route a fluid by gravity or pressure for transport or work, beyond DAM (block) and WICK (capillary). Irrigation and water-mills. Couples to item 5's hydrology. Home: materials-core, leaning bridges-terrain.
- PROPERTY FRACTIONATION (winnow / settle / distil): separate a mixture by a property gradient (density, volatility, solubility), beyond FILTER's size-exclusion. Winnowing grain, panning ore, distilling. Home: materials-core.

Noted but not counted (overlaps or bridges): diffusion-into-bulk (case-harden, brine-cure) overlaps PROCESS-OVER-TIME's curing; acoustic/resonant sound-making bridges communication-reach.

VERDICT: with these folded in, all three critics agreed no further whole categories remain at this tech horizon. This closes the dimensional space, and the gap survey is complete. The full set now spans solid mechanics (fracture, yield, wear), phase and thermal, fluid (block, route, filter, fractionate), constructive (mix, stack, weave, cast, joinery), energetic (force, stored-elastic), ingestive (nutrient and toxin), locomotive, social, informational (mark, assay), collective, and failure, with the neighbour-arc bridges flagged throughout.


### Steering discipline: primitives are coded, techniques emerge (owner-endorsed 2026-07-05)

Every affordance and every gap-list entry above is held to one test: is it a body-grounded PRIMITIVE or an authored TECHNIQUE? A primitive is what a grown morphology physically affords and the physics gates (a limb closes on matter, force meets the ground's fracture, a mouth ingests and the composition physics decides, a tissue heats). Primitives are coded, because the body affords them and the outcome is physics-gated. A technique is a composite of primitives against matter over time (knapping, casting, forging, fermenting), and a technique must EMERGE from sequencing primitives against real physics, never arrive as its own coded verb. Casting done right is what HAPPENS when a creature melts a substance (fire, item 6), pours it (put-down) into a hollow (an earthwork pit, item 5), and it solidifies to the shape (phase physics): no CAST affordance, discovered by any creature that does the sequence, kept by selection. This is the same north star as the DecayLaw ruling: no registry, the transforms emerge from the substance's own physics.

So the gap addendum is a COMPLETENESS CHECK on the physics substrate ("does the substrate let a creature discover casting?"), never a to-build list of verbs. Build the small set of body-grounded primitives (grasp, release, apply force in a direction, ingest, heat) and let the techniques emerge. Two known debts to walk back toward emergence: CRAFT (knapping) is currently a coded technique (activate craft, get a tool with a reserved edge) rather than an emergent one (strike a core, the fracture throws flakes, the edge geometry emerges), and the tool-geometry-from-skill refinement is the first step; and DIG versus EXTRACT is a near-duplicate to unify into one ground-working primitive (selective versus bulk a parameter, not two verbs).
