...
Owner-authored (2026-07-12), then promoted to the canonical abiotic boundary on
2026-07-20. Every value admitted to the planet runpath carries one of the seven
provenance types below and belongs to one of four tiers. The retired calibration
register remains historical audit evidence only and is not a canonical input.

The canonical implementation lives in `crates/ledger`. `Ledger` accepts only the
seven canonical provenance variants, validates non-empty derived ancestry,
rejects missing parents and cycles, computes effective worst-case provenance,
and exposes the authoring surface. `LedgerValue<T>` binds typed values to their
ledger identities without turning the register into an untyped value store.

The four tiers are Universal, Reference, Residue, and Contingency. They are
accounting layers, not four equivalent ways to feed a run. The runner accepts a
validated absolute floor. A reference or residue can extend that floor only
after derive-first has been exhausted and a derivation-hunter receipt records
the Buckingham-Pi budget. Written state and contingency are run products; a
contingency draw must come from a derived measure and the seed. The Gap Law
governs branch resolution, and the Residual Law keeps conservation,
disequilibrium, fluctuation-dissipation, and dimensional missing-edge checks
visible.

The audited catalog begins in `crates/planet/src/canonical/catalog.rs`; its generated,
checked-in inventory is
`docs/working/CANONICAL_LEDGER_INVENTORY.txt`. The generator always emits all
28 tier/type cells, including zero counts, as well as tier totals and member
identities. Run `just ledger-inventory` to regenerate it and
`just ledger-inventory-check` to verify it.
...
seven-type provenance with the refutability test; four tiers; headline counts as
ledger queries; error-band DAG with regime-marginal surfacing; the solver law
(all memory in named state, memoryless solver); timescale-ladder quasi-statics
with stiffness self-check; pseudo-arclength continuation with deterministic fold
jumps; lattice quantization below error bands; fixed module order and
deterministic reductions; the chaos protocol (detector, seeded stationary draw,
three-way switch); attractor shortcut with basin measures; response surfaces on
slow schedules; ConservedBudget double-entry (L and E, energy posting to the
heat ledger, |L| invariant under precession); memoized kernel on (composition,
E-bucket, path class); lazy anomaly instantiation; status fields
built/stubbed/spec with spec-to-repo diff; compute-once cache; cosmetic
gazetteer; Buckingham pre-counting; the four legal procgen moves with the
incoherence flag.
...
The canonical `Provenance` enum, tier enum, DAG, validation, inventory, and
typed binding live in `crates/ledger`. A derived entry names every ledger parent
through non-empty input edges. The worst-case join walks those edges, so a
derived value inherits the least refutable provenance in its ancestry. The
authoring surface counts effective Closure and Authored entries.

The current catalog admits the audited fundamentals and composites first. Empty
Reference, Residue, or Contingency cells are visible accounting, not a demand to
populate them and not permission to read the old calibration manifest. A tag or
citation alone never moves a value onto the runpath.

## Historical retired-register accounting

The Phase 1 and Phase 2 results below record the old calibration and floor audit
that motivated the canonical ledger. Their counts remain useful archaeology,
but their manifests, profiles, joined register, and biology entries are not
members of the planet runpath.
