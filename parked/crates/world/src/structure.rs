...
//! Parked-method world-structure fixtures retained inside the active crate only until physical world modules can
//! be split from the former worldgen surface. This module is not canonical planet input.
//!
//! The historical path selected a scenario structure and fixed-Rust terrain builder. That ruling is retired:
//! canonical terrain must derive from the sealed floor and upstream planet receipts or refuse. The evidence and
//! migration boundary are recorded in `docs/working/ABIOTIC_EVIDENCE_DEBT.md`. Until extraction, the old
//! STRUCTURE DEFINITIONS remain fixed-Rust builders,
