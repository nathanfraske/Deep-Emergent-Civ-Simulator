...
/// `crates/planet-substrate/src` is included because the parked compatibility crate compiles the active
/// abiotic substrate under its historical module identities.
/// `parked/crates/foundation/src` carries three today (the clock's calendar cell and day cadence, the
/// decomposer's soil-nutrient recovery); `parked/crates/bio/src` carries none yet and is listed so that
...
const EXCLUDED: &[&str] = &["parked/crates/sim/src/derive_gate.rs"];

/// The repository root, derived from this crate's manifest dir (`.../parked/crates/sim`).
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(Path::parent)
        .and_then(Path::parent)
        .expect("parked/crates/sim has a repository root three levels up")
        .to_path_buf()
}
