...
/// (`civsim_foundation::contact_transfer::resolve_delivered_energy`, named and not linked: compose sits BELOW
/// foundation, which depends on compose, so rustdoc cannot check this cross-crate cite), so the grade and the
/// delivery stay in LOCKSTEP
