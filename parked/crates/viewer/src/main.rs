...
use civsim_sim::geodynamics::{
    convecting_mantle_depth_m, derive_mantle_density, generate_derived_tiles, DerivedTile,
};
...
use legacy_fixture::slice0_demo_field;
...
fn derived_terrain_cmd(argv: &[String]) {
    ...
    let tiles = match slice0_demo_field(cols, rows) {
        Some(t) => t,
        None => {
...
mod composition_draw_tests {
    ...
    #[test]
    fn absent_fusion_volume_refuses_the_legacy_causal_scene() {
        match build_derived_scene(Fixed::ONE, Fixed::ONE) {
            Err(message) => assert!(
                message.contains("fusion volume is absent"),
                "the refusal names the missing fusion-volume evidence: {message}"
            ),
            Ok(_) => panic!("the parked causal viewer must not invent a fusion volume"),
        }
    }

    #[test]
    #[ignore = "parked expectation requires fusion-volume evidence that the Gap Law now refuses"]
    ...
    fn alpha_enhancement_lowers_the_derived_bulk_density_the_first_density_lever() {
        ...
        let density_of = |abund: &SolarAbundances| -> Fixed {
            ...
            .expect("pressure-dependent melting does not refuse")
