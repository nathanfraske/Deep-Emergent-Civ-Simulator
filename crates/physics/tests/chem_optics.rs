...
fn the_chem_optics_floor_loads_onto_the_mechanical_and_fluids_floors() {
    ...
    // 38 mech + 15 fluids + 14 chem/optics axes; 19 + 15 + 11 laws; 2 + 2 + 0 substances.
    assert_eq!(reg.axis_count(), 67, "the mechanical, fluids, and chem/optics axes");
    assert_eq!(reg.law_count(), 45, "the wave-1 and wave-2 laws");
...
fn carnot_efficiency_is_the_temperature_ratio_and_bounded() {
    ...
    assert!((0.35..0.45).contains(&eta), "Carnot 500/300 ~0.4, got {eta}");
    // No gradient, no work; and a wider gradient is more efficient.
    assert_eq!(laws::carnot_limit(Fixed::from_int(300), Fixed::from_int(300)), Fixed::ZERO);
...
fn reaction_enthalpy_signs_exothermic_and_gates_on_the_barrier() {
    ...
    let (dh, crossed) = laws::reaction(Fixed::from_int(-400), Fixed::from_int(-100), Fixed::from_int(600), Fixed::from_int(500));
    assert!(dh < Fixed::ZERO, "downhill reaction is exothermic");
    assert!(crossed, "above the barrier temperature the reaction proceeds");
    let (_, cold) = laws::reaction(Fixed::from_int(-400), Fixed::from_int(-100), Fixed::from_int(400), Fixed::from_int(500));
...
fn corrosion_needs_a_favourable_potential_and_rises_with_acidity() {
    ...
    let acidic = laws::corrosion(f(12, 10), f(-4, 10), f(5, 10), Fixed::from_int(3), cap).to_f64_lossy();
    assert!(acidic > mild, "a more aggressive (acid) fluid corrodes faster");
    // A noble material (higher potential than the oxidiser) does not corrode.
    assert_eq!(laws::corrosion(f(-4, 10), f(12, 10), f(5, 10), Fixed::ONE, cap), Fixed::ZERO, "an uphill pairing does not attack");
}
...
fn radiant_emission_rises_steeply_with_temperature_and_caps_at_plasma() {
    ...
    let area = Fixed::ONE;
    let warm = laws::radiant_emission(Fixed::ONE, area, Fixed::from_int(400), Fixed::from_int(300), sigma, flux).to_f64_lossy();
    let hot = laws::radiant_emission(Fixed::ONE, area, Fixed::from_int(800), Fixed::from_int(300), sigma, flux).to_f64_lossy();
    assert!(hot > warm * 8.0, "doubling the surface temperature raises emission steeply (T^4)");
    // A cooler surface than ambient emits nothing net here.
    assert_eq!(laws::radiant_emission(Fixed::ONE, area, Fixed::from_int(300), Fixed::from_int(400), sigma, flux), Fixed::ZERO);
    // A plasma temperature routes to the cap (the honest Tier-0 limit).
    let plasma = laws::radiant_emission(Fixed::ONE, area, Fixed::from_int(30000), Fixed::from_int(300), sigma, flux);
    assert_eq!(plasma, flux, "beyond ~14000 K the emissive power routes to the cap");
}
...
fn radiative_equilibrium_sets_a_temperature_by_the_nested_square_roots() {
    ...
    let t = laws::radiative_equilibrium(Fixed::from_int(239), Fixed::ONE, sigma, t_max).to_f64_lossy();
    assert!((230.0..280.0).contains(&t), "radiative-equilibrium temperature ~255 K, got {t}");
    // More absorbed flux is a hotter equilibrium.
    let hotter = laws::radiative_equilibrium(Fixed::from_int(1000), Fixed::ONE, sigma, t_max).to_f64_lossy();
    assert!(hotter > t, "more absorbed irradiance sets a hotter surface");
    // No light, no temperature.
    assert_eq!(laws::radiative_equilibrium(Fixed::ZERO, Fixed::ONE, sigma, t_max), Fixed::ZERO);
}
...
fn optics_reach_splits_and_attenuates_and_refracts() {
    ...
    let far = laws::inverse_square_falloff(Fixed::from_int(1000), Fixed::from_int(10), f(1256, 100), Fixed::from_int(1_000_000)).to_f64_lossy();
    ...
    assert!(contrast.to_f64_lossy() > 1.0 && !tir, "air to water: n rises, no TIR");
    let (_, tir_back) = laws::refractive_contrast(f(133, 100), Fixed::ONE, Fixed::from_int(100));
    assert!(tir_back, "water to air: total internal reflection is possible");
    // Optical depth and dissolution replay and clamp.
    assert_eq!(laws::dissolution(f(8, 10), f(5, 10)), f(4, 10), "leach fraction is affinity times aggressiveness, clamped");
