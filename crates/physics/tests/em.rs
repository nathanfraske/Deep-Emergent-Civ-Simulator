...
fn the_em_floor_loads_onto_the_earlier_floors() {
    ...
    // 38 mech + 20 fluids + 14 chem/optics + 14 em axes; 21 + 18 + 11 + 15 laws; 2 + 2 + 0 + 2 subs.
    assert_eq!(reg.axis_count(), 86, "the four floors' axes together");
...
fn the_fifth_base_is_backward_compatible_and_the_volt_promotion_holds() {
    ...
    assert_eq!(
        reg.axis("elec.current").unwrap().dimension,
        Dimension::CURRENT
    );
    assert_eq!(
        reg.axis("elec.charge").unwrap().dimension,
        Dimension::CHARGE
    );
    assert_eq!(
        reg.axis("mech.contact_area").unwrap().dimension.current,
        0,
        "an earlier axis is unchanged"
    );
    ...
    assert_eq!(
        reg.axis("chem.standard_potential").unwrap().dimension,
        Dimension::VOLTAGE
    );
    assert_eq!(
        reg.axis("elec.potential").unwrap().dimension,
        Dimension::VOLTAGE
    );
...
fn ohm_and_the_circuit_extremes() {
    ...
    assert_eq!(
        laws::ohm_voltage(Fixed::from_int(2), Fixed::from_int(3), v_max),
        Fixed::from_int(6)
    );
    // A 12 V source across 4 Ohm gives 3 A; a zero-resistance path is a short (the cap).
    assert_eq!(
        laws::circuit_current(Fixed::from_int(12), Fixed::from_int(4), i_max),
        Fixed::from_int(3)
    );
    assert_eq!(
        laws::circuit_current(Fixed::from_int(12), Fixed::ZERO, i_max),
        i_max,
        "a short routes to the cap"
    );
    ...
    assert_eq!(
        laws::resistance(f(1, 100), Fixed::from_int(10), Fixed::ZERO, r_max),
        r_max,
        "an open"
    );
}
...
fn the_daniell_cell_emf_and_coulomb_sign() {
    ...
    assert!(
        (1.05..1.15).contains(&emf),
        "Daniell cell ~1.10 V, got {emf}"
    );
    ...
    let (near, rep) =
        laws::coulomb_force(Fixed::from_int(2), Fixed::from_int(3), Fixed::ONE, k, f_max);
    let (far, _) = laws::coulomb_force(
        Fixed::from_int(2),
        Fixed::from_int(3),
        Fixed::from_int(2),
        k,
    ...
    assert!(
        near.to_f64_lossy() > far.to_f64_lossy(),
        "nearer charges push harder"
    );
    let (_, attract) = laws::coulomb_force(
        Fixed::from_int(2),
        Fixed::from_int(-3),
        Fixed::ONE,
        k,
...
fn the_solenoid_field_and_the_motor_force() {
    ...
    // Motor force F = B*I*L.
    assert_eq!(
        laws::motor_force(Fixed::from_int(2), Fixed::from_int(3), f(1, 2), f_max),
        Fixed::from_int(3)
    );
    // Dipole torque tau = m*B (maximum), and flux linkage Phi = B*A.
    let tau =
        laws::dipole_torque(f(1, 10), Fixed::from_int(2), Fixed::from_int(1000)).to_f64_lossy();
    assert!((0.19..0.21).contains(&tau), "m*B ~0.2, got {tau}");
    assert_eq!(
        laws::flux_linkage(Fixed::from_int(2), f(1, 2), Fixed::from_int(1000)),
        Fixed::ONE
    );
}
...
fn induction_is_a_signed_per_tick_delta_opposing_the_change() {
    ...
    assert!(
        emf < Fixed::ZERO,
        "a rising flux induces an opposing (negative) EMF"
    );
    assert!(
        (900.0..1100.0).contains(&emf.to_f64_lossy().abs()),
        "~1000 V, got {}",
        emf.to_f64_lossy()
    );
    ...
    let small_hi = laws::capacitor_energy(
        f(1, 1_000_000_000),
    ...
    .to_f64_lossy();
    assert!(
        (4.0..6.0).contains(&small_hi),
        "small C at high V stores ~5 J, got {small_hi}"
    );
...
fn the_content_hash_covers_the_fifth_base() {
    ...
    assert_ne!(
        charge.content_id(),
        time.content_id(),
        "the fifth base must change the content hash"
    );
}
...
fn the_electrical_kernels_are_total_on_adversarial_inputs() {
    ...
    let f_max = Fixed::from_int(1_000_000_000);
    assert_eq!(
        laws::lorentz_force(Fixed::MIN, Fixed::MIN, Fixed::MIN, f_max),
        f_max
    );
    let (mag, _) = laws::coulomb_force(Fixed::MAX, Fixed::MAX, f(1, 1000), Fixed::MAX, f_max);
    assert_eq!(
        mag, f_max,
        "an overflowing Coulomb product routes to the cap"
    );
}
