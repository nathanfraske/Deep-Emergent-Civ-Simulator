...
fn main() {
    // The wide-scale sweep below deliberately drives tier2's shift arithmetic past 128 bits to map out
    // the full `div` overflow envelope; if that also reaches the suspected unchecked `1i128 << shift` in
    // `round_half_even_shr`, the resulting panic is caught per-call by `safe_i64_op` above and must not
    // spam the default panic handler's stderr output for what could be many thousands of occurrences.
