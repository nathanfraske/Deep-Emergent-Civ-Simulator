...
//! THE ABIOTIC SECULAR-PERTURBATION SPECTRUM (R-CELESTIAL-SECULAR, task #44): from an assembled multi-planet
...
/// The caller-supplied numerical policy for the secular solver. Each field controls numerical accuracy,
/// iteration budget, or diagnostic classification rather than a physical input. The physical constants
/// the solver uses (the speed of light, the Earth and solar masses, the astronomical unit, and pi) remain
/// cited anchors outside this policy.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct SecularNumericalPolicy {
...
/// term falls below `policy.laplace_term_epsilon` or after `policy.laplace_max_iters` terms.
/// `None` on `alpha >= 1` (the argument must be below one, the convention guarantees this) or an overflow.
fn laplace_coefficient(
    s: Fixed,
    j: u32,
    alpha: Fixed,
    policy: &SecularNumericalPolicy,
) -> Option<Fixed> {
    ...
    for m in 0..policy.laplace_max_iters {
        ...
        if term.abs() < policy.laplace_term_epsilon {
            break;
        }
    }
...
fn build_matrices(
    ...
    for j in 0..n {
        ...
        for k in 0..n {
            ...
            let b1 = laplace_coefficient(s, 1, alpha, policy)?;
            let b2 = laplace_coefficient(s, 2, alpha, policy)?;
...
/// off-diagonal norm falls below `policy.jacobi_offdiag_epsilon` or after
/// `policy.jacobi_max_sweeps` sweeps. Returns the diagonal (the
...
fn jacobi_eigenvalues(
    ...
    for _ in 0..policy.jacobi_max_sweeps {
        if residual < policy.jacobi_offdiag_epsilon {
            break;
        }
        sweeps += 1;
        for p in 0..n {
...
pub fn secular_spectrum(
    ...
    let (a_mat, b_mat, n_rad, gr, smallness) = build_matrices(bodies, star_mass_solar, policy)?;
    let a_sym = symmetrize(&a_mat, bodies, &n_rad)?;
    let b_sym = symmetrize(&b_mat, bodies, &n_rad)?;
    let (g_vals, sweeps_g, resid_g) = jacobi_eigenvalues(&a_sym, policy)?;
    let (s_vals, sweeps_s, resid_s) = jacobi_eigenvalues(&b_sym, policy)?;

    let g_modes = modes_from_eigenvalues(g_vals, policy.jacobi_offdiag_epsilon);
    let s_modes = modes_from_eigenvalues(s_vals, policy.jacobi_offdiag_epsilon);
    ...
    for (family, modes) in [
        ...
        for a in 0..modes.len() {
            for b in (a + 1)..modes.len() {
                ...
                let bound = scale
                    .checked_mul(policy.near_degeneracy_fraction)
    ...
    for (gi, gm) in g_modes.iter().enumerate() {
        for (si, sm) in s_modes.iter().enumerate() {
            let comb = gm
                ...
                .unwrap_or(Fixed::MAX);
            if comb.abs() < policy.secular_resonance_epsilon {
    ...
    for w in order.windows(2) {
        ...
        if let Some(ratio) = n_rad[inner].checked_div(n_rad[outer]) {
            if let Some((p, q, residual)) = nearest_commensurability(ratio, policy.mmr_max_order) {
                if residual < policy.mmr_proximity_fraction {
    ...
    for (planet, &small) in smallness.iter().enumerate() {
        if small > policy.relativity_smallness_flag {
...
mod tests {
    ...
    fn numerical_policy_fixture() -> SecularNumericalPolicy {
        SecularNumericalPolicy {
    ...
    fn laplace_coefficients_match_the_integral_quadrature() {
        ...
        let policy = numerical_policy_fixture();
        ...
        for (alpha_f, b1_ref, b2_ref) in cases {
            ...
            let b1 = laplace_coefficient(s, 1, alpha, &policy).unwrap();
            let b2 = laplace_coefficient(s, 2, alpha, &policy).unwrap();
            assert!(
    ...
    fn two_planet_system_recovers_the_analytic_laplace_lagrange_frequencies() {
        ...
        let spec = secular_spectrum(&bodies, Fixed::ONE, &policy).unwrap();
    ...
    fn the_frequencies_are_real_and_the_spectrum_is_bit_deterministic() {
        ...
        let a = secular_spectrum(&bodies, Fixed::ONE, &policy).unwrap();
        let b = secular_spectrum(&bodies, Fixed::ONE, &policy).unwrap();
        ...
        assert!(a.jacobi_sweeps_g <= policy.jacobi_max_sweeps);
        assert!(a.jacobi_sweeps_s <= policy.jacobi_max_sweeps);
    }
    ...
    fn fewer_than_two_bodies_and_a_dead_star_fail_loud() {
        ...
        assert!(secular_spectrum(&one, Fixed::ONE, &policy).is_none());
        let two = [
            SecularBody::circular(Fixed::ONE, Fixed::ONE),
            SecularBody::circular(Fixed::ONE, Fixed::from_int(2)),
        ];
        assert!(secular_spectrum(&two, Fixed::ZERO, &policy).is_none());
    }
}
