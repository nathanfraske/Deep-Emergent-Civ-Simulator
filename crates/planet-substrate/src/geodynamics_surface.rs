...
//! per-column [`civsim_world::geology::GeodynamicColumn::isostatic_elevation`] the producer derives (the interior convection lane, or
...
use civsim_world::geology::{EarthworkField, GeodynamicField};
use civsim_world::Coord3;

/// Why an isostatic-relaxation pass could not be evaluated.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum IsostaticRelaxationRefusal {
    /// The requested relaxation fraction lies outside the valid interval `(0, 1]`.
    InvalidRelaxationFraction { relaxation: Fixed },
    /// Multiplying the residual by the valid relaxation fraction exceeded fixed-point range.
    FixedPointOverflow,
}
...
/// can oscillate or diverge, so either is refused fail-loud as
/// [`IsostaticRelaxationRefusal::InvalidRelaxationFraction`] rather than silently clamped. A fixed-point
/// overflow in the step multiply is distinguished as [`IsostaticRelaxationRefusal::FixedPointOverflow`],
...
pub fn relax_toward_isostasy<F>(
    ...
    base_elevation: F,
    relaxation: Fixed,
) -> Result<Fixed, IsostaticRelaxationRefusal>
where
    F: Fn(Coord3) -> Fixed,
{
    if relaxation <= Fixed::ZERO || relaxation > Fixed::ONE {
        return Err(IsostaticRelaxationRefusal::InvalidRelaxationFraction { relaxation });
    }
    ...
    for (column, state) in geodynamics.iter() {
        ...
        let step = relaxation
            ...
            .ok_or(IsostaticRelaxationRefusal::FixedPointOverflow)?;
...
mod tests {
    ...
    use civsim_world::geology::GeodynamicColumn;
    ...
    fn an_out_of_range_relaxation_fraction_is_refused_fail_loud() {
        ...
        assert_eq!(
            relax_toward_isostasy(&mut earthwork, &geo, flat_base, Fixed::ZERO),
            Err(IsostaticRelaxationRefusal::InvalidRelaxationFraction {
                relaxation: Fixed::ZERO,
            })
        );
        let above_one = Fixed::from_ratio(3, 2);
        assert_eq!(
            relax_toward_isostasy(&mut earthwork, &geo, flat_base, above_one),
            Err(IsostaticRelaxationRefusal::InvalidRelaxationFraction {
                relaxation: above_one,
            })
        );
    }
