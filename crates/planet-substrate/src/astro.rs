...
/// CODATA fundamentals (`k_B`, `h`, `c`) through
/// [`civsim_units::constants::derived_stefan_boltzmann`], never authored.
///
...
/// [`viscous_dissipation_flux`] uses), `Omega(r)` from the sealed physical-floor `G`, the
...
/// the EUV branch. This is the value-reads-two-ways defect class (the `coulomb_friction_response` rename,
/// the `Delta` unit error) in a return channel, so the channel is typed. Only
/// [`TurnoverRefusal::AboveFitDomain`] is a dispatch seam.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
...
pub fn pre_main_sequence_luminosity_lsun(
    ...
    let ln_sigma = civsim_units::constants::derived_stefan_boltzmann().ln();
    let ln_g = crate::absolute_floor::ln_gravitational_constant()?;
    let ln_m = mass_ratio
        .ln()
        .checked_add(civsim_physics::saha::ln_of_decimal(SOLAR_MASS_KG)?)?;
