...
//!
//! The activity is the saturating sum, clamped to `[0, 1]`, of the per-cell contributions of a
//! data-defined set of DRIVER ROWS ([`DecomposerDriver`]), each binding one of a fixed kernel set
//! ([`DecomposerKernelId`]) to reserved parameters keyed by name. The mechanism is fixed Rust and the
//! membership plus parameters are data (Principle 11), sibling to the transform-kernel substrate
//! ([`crate::trace::TransformKernelId`]), the value, semantic, and institution-function substrates.
...
//!   life is present, so a warm, wet, airy but STERILE cell does not decay, the case an abiotic proxy
//!   cannot express. The biomass source, a generated species whose decomposer role is read off its diet
//!   exactly as a carnivore is read from its prey, is deferred to the biosphere-wiring slice; until then
//!   the stock is hand-seeded (a test) or filled by worldgen, the honest opt-in seam.
//!
//! A world uses either kernel or both, so neither is an engine universal: a life-driven world arms a
//! Life row and its sterile cells preserve; a conditions-driven world arms a Conditions row and its dry
//! or airless cells preserve; a world that arms neither (the default) sees no decomposer registry at
//! all, its matter cycle runs unchanged if armed, and the run is byte-identical. The physics barrier
//! gate stays in the caller and is never touched here, so falsifiability-by-physics survives: only the
//! rate ABOVE the gate is modulated.
//!
    ...
    fn contribution(
        ...
        match self.kernel {
            DecomposerKernelId::Conditions => {
                ...
                // so it is zero at the barrier and rises to full over the reserved span. A non-positive
                // excess (the caller admits only temperatures at or above the barrier, but this function
                // is pure and may be called below it) reads zero.
                let warmth = (temperature - barrier).max(Fixed::ZERO);
...
/// The DECOMPOSER-DRIVER REGISTRY: the data-defined set of driver rows a world arms. Its one entry point,
...
pub struct DecomposerDriverRegistry(Vec<DecomposerDriver>);

impl DecomposerDriverRegistry {
    /// An empty registry: no driver rows, so [`Self::activity_at`] is zero everywhere.
    pub fn new() -> DecomposerDriverRegistry {
        DecomposerDriverRegistry(Vec::new())
    }
    ...
    /// A cell's decomposition ACTIVITY in `[0, 1]`: the saturating sum of every row's contribution,
    /// clamped to the unit interval. The sum lets a world arm several drivers (conditions and life) whose
    /// activities add, while the clamp keeps the factor a fraction of the substance's own susceptibility
    /// rate. The caller multiplies the substance's rate by this before the volume breaks down, so a factor
    /// of one reproduces the unconditional rate and a factor of zero preserves the matter.
    pub fn activity_at(
        &self,
        temperature: Fixed,
        barrier: Fixed,
        ...
        let sum = Fixed::saturating_sum(
            self.0
                .iter()
                .map(|row| row.contribution(temperature, barrier, moisture, oxygen, life_stock)),
        );
        sum.clamp(Fixed::ZERO, Fixed::ONE)
    }

    /// A labelled NON-CANONICAL dev fixture: a registry arming both a Conditions row and a Life row with
    /// stand-in parameters, for tests and examples. Not a set of decided values; the reserved parameters
    /// are the owner's to calibrate (basis on each row constructor and in `calibration/reserved.toml`).
...
mod tests {
    ...
    #[test]
    fn the_activity_is_clamped_to_the_unit_interval() {
        // Two saturated drivers sum past one, and the clamp keeps the factor a fraction of the rate.
        let mut reg = DecomposerDriverRegistry::new();
        reg.push(DecomposerDriver::life(Fixed::ONE));
        reg.push(DecomposerDriver::conditions(
            Fixed::from_ratio(1, 2),
        ...
        assert_eq!(
            ...
            Fixed::ONE,
            "the summed activity of two saturated drivers clamps to full, never above"
        );
    }
