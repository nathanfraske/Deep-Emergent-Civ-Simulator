...
use crate::astro::{
    derive_viscous_time_myr, disk_effective_temperature, disk_era_xray_disk_lifetime_myr,
    hill_radius_au, kepler_orbital_period_years, planet_radius_m,
    shu_inside_out_collapse_accretion_rate_msun_myr, tidally_capped_scale_radius_au,
    viscous_similarity_surface_density, CollapseModel, TruncationEvaluation, TruncationRadiusBand,
    XrayWindFit, ASTRONOMICAL_UNIT_M, EARTH_MASS_KG, SOLAR_MASS_KG,
};
use crate::planetary_assembly::{AngularMomentumProxy, DiskGasLedger, DiskGasProvenance};
...
/// The STANDING INTERIMS the derived disk-evolution chain still stands on, carried on the state so a consumer reads
/// WHICH solar-pinned draws condition the verdict rather than assuming it fully derived (the thermoelastic
/// "name the gap as data" idiom). Each is `true` while solar-pinned; a later layer-4 draw flips one to `false`, a
/// machine-checkable state change.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct DiskEvolutionInterims {
    /// The disk birth radius `R_1` is a per-system draw (disk-size demographics conditioned on stellar mass),
    /// solar-pinned until the layer-4 draw. `t_visc` (and so `tau_disk`) inherit this.
    pub birth_radius_solar_pinned: bool,
    /// The rotation period (`Omega_star_0`) is solar-pinned until the gyrochronology draw; the X-ray clock reads it.
    pub rotation_period_solar_pinned: bool,
    /// The cloud-core temperature (the `Mdot_0` birth condition) is reserved-with-basis until the layer-4 draw.
    pub cloud_core_temperature_solar_pinned: bool,
}

/// The ONE CANONICAL DISK-EVOLUTION STATE (dormant): truncation, the surface-density normalization, the retained
/// gas mass and angular momentum, `t_visc`, `tau_disk`, and the giant verdict, all COMPOSED from one set of birth
/// conditions and all inheriting ONE provenance grade, the P1-4 requirement that these read one state and one
/// provenance DAG rather than a scatter of separately-derived quantities. The connection this makes that the prior
/// [`giant_formation_on_derived_clock`] spine did NOT: the resonant TRUNCATION caps `R_1`, and the capped `R_1`
/// feeds `t_visc` (and so `tau_disk` and the verdict), so a companion's cut on the disk size reaches the
/// giant gate instead of being an orphan derivation. The gas account and the truncation residual come from the same
/// capped disk, so the mass budget and the lifetime read one disk.
///
/// PROVENANCE: every field inherits the single [`DiskGasProvenance`] grade, set to the WEAKEST among the inputs
/// (the same discipline as [`DiskGasLedger`]'s `Overdrawn { bound_provenance }`). The domain is currently
/// PROXY-graded because the disc's INNER edge is still the magnetospheric interim even though its OUTER edge is now
/// the derived truncation radius: a half-derived domain is proxy-graded, not promoted. The standing solar-pinned
/// interims ([`DiskEvolutionInterims`]) are carried explicitly so the state advertises what it is conditioned on.
///
/// DORMANT and BYTE-NEUTRAL: no `run_world` caller (every composed piece is itself dormant), so both pins hold
/// bit-exact. The FLIP that feeds this into the run path and moves the pins is the owner-controlled re-pin, held.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct DiskEvolutionState {
    /// The truncated disk birth radius (AU): `min(R_1, R_t)` as a band, from [`tidally_capped_scale_radius_au`].
    pub capped_scale_radius_au: TruncationRadiusBand,
    /// The viscous time `t_visc` (Myr) at the SHORT (most-truncated) capped-radius edge; the low band edge.
    pub t_visc_myr_short: Fixed,
    /// The viscous time `t_visc` (Myr) at the LONG (least-truncated) capped-radius edge; the high band edge. This is
    /// the connection the prior spine lacked: truncation reaches `t_visc`.
    pub t_visc_myr_long: Fixed,
    /// The RETAINED gas account over `[inner, R_t]` (mass and angular-momentum proxy), provenance-tagged, the disc's
    /// gas budget after the cut.
    pub gas: DiskGasLedger,
    /// The truncation-residual disposition under the selected reading (retained budget, and the removed residual's
    /// named sink under the dynamic reading).
    pub residual: TruncationResidualDisposition,
    /// The giant verdict over the compound (`t_visc` times wind) disk-lifetime band.
    pub verdict: BandedGiantVerdict,
    /// The single domain grade every field inherits (the weakest among the inputs).
    pub provenance: DiskGasProvenance,
    /// The standing solar-pinned interims the chain is conditioned on.
    pub interims: DiskEvolutionInterims,
}

// @derives: the canonical disk-evolution state (truncation, retained gas mass and angular momentum, t_visc, tau_disk, giant verdict) <- one set of birth conditions composed through the truncation cap into the viscous time and the derived disk clock, every field inheriting one DiskGasProvenance grade
/// Compose the canonical [`DiskEvolutionState`] from one set of birth conditions, mirroring
/// [`giant_formation_on_derived_clock`] but threading truncation into `t_visc`. The `star.t_visc_myr` field is
...
/// swallowed.
#[allow(clippy::too_many_arguments)]
pub fn derive_disk_evolution_state(
    embryo: &Embryo,
    disk: &SolidDisk,
    star: &DiskClockState,
    birth_r1_au: Fixed,
    truncation: &TruncationEvaluation,
    reading: TruncationResidualReading,
    wind_fit_a: &XrayWindFit,
    wind_fit_b: &XrayWindFit,
    gas: &GiantGasParams,
    ...
    inner_au: Fixed,
    steps: u32,
) -> Option<DiskEvolutionState> {
    // 1. TRUNCATION: cap the birth radius at the resonant truncation radius (a band).
    let capped = tidally_capped_scale_radius_au(birth_r1_au, truncation)?;

    // 2. t_visc FROM the capped R_1 edges: the disc temperature at that radius feeds the alpha-closure viscous time.
    //    The connection the prior spine lacked. t_visc rises with R_1, so the short (low) capped edge gives the
    //    short t_visc and the long (high) edge the long one.
    let t_visc_at = |scale_radius_au: Fixed| -> Option<Fixed> {
        let disk_temp = disk_effective_temperature(
            disk.thermal.accretion_rate_msun_myr,
            disk.thermal.star_mass_ratio,
            disk.thermal.mass_luminosity_exponent,
            scale_radius_au,
    ...
    let t_visc_short = t_visc_at(capped.lo_au)?;
    let t_visc_long = t_visc_at(capped.hi_au)?;

    // 3. Mdot_0 DERIVES from the birth conditions (Shu collapse), never a stored interim.
    let mdot_0 = star.derived_birth_accretion_msun_myr()?;

    // 4. tau_disk over the COMPOUND (t_visc times wind) band: the four monotone corners, min and max.
    let tau_for = |t_visc_myr: Fixed, fit: &XrayWindFit| -> Option<Fixed> {
        disk_era_xray_disk_lifetime_myr(
            star.mass_ratio,
            star.hayashi_temp_k,
            star.age_myr,
    ...
    let mut tau_low: Option<Fixed> = None;
    let mut tau_high: Option<Fixed> = None;
    for t_visc in [t_visc_short, t_visc_long] {
        for fit in [wind_fit_a, wind_fit_b] {
            let tau = tau_for(t_visc, fit)?;
            tau_low = Some(tau_low.map_or(tau, |lo| if tau < lo { tau } else { lo }));
            tau_high = Some(tau_high.map_or(tau, |hi| if tau > hi { tau } else { hi }));
        }
    }

    // 5. The giant verdict over the compound lifetime band.
    let verdict =
        giant_formation_banded(embryo, disk, star.mass_ratio, gas, kh, tau_low?, tau_high?)?;

    // 6. The RETAINED gas account over [inner, R_t], and the residual disposition, from the SAME capped disc. The
    //    residual reading must be resolved; an unresolved reading cannot pin one gas budget.
    let ledger = truncation_gas_ledger(
        disk,
        inner_au,
        birth_r1_au,
        truncation.radius_au.central()?,
        steps,
    )?;
    let residual = dispose_truncation_residual(&ledger, reading).ok()?;
    // The domain is half-derived (the outer edge is the derived truncation radius, the inner is still the
    // magnetospheric interim), so the account is PROXY-graded: the weakest grade wins.
    let provenance = DiskGasProvenance::ProxyBounds;
    let gas_account = DiskGasLedger::from_snapshot(
        residual.disk_gas_mass_earth,
        AngularMomentumProxy(ledger.retained_proxy_l),
        provenance,
    );

    Some(DiskEvolutionState {
        capped_scale_radius_au: capped,
        t_visc_myr_short: t_visc_short,
        t_visc_myr_long: t_visc_long,
        gas: gas_account,
        residual,
        verdict,
        provenance,
        interims: DiskEvolutionInterims {
            birth_radius_solar_pinned: true,
            rotation_period_solar_pinned: true,
            cloud_core_temperature_solar_pinned: true,
        },
    })
}
...
mod tests {
    ...
    /// A truncation evaluation that BITES: a companion whose Roche lobe puts the resonant truncation radius inside
    /// the 30 AU birth disc, so the cap is load-bearing. `roche_lobe_au` scales the companion separation.
    fn biting_truncation(roche_lobe_au: Fixed) -> crate::astro::TruncationEvaluation {
        crate::astro::pichardo_truncation_evaluation(
            Fixed::from_ratio(3, 10), // e = 0.3
            Fixed::from_ratio(3, 10), // q = 0.3 companion mass fraction
            roche_lobe_au,
            &crate::astro::DiskTruncationFit::pichardo_2005(),
        )
        .expect("the truncation evaluation resolves inside the Pichardo domain")
    }

    #[test]
    fn the_canonical_disk_evolution_state_threads_truncation_into_the_clock() {
        // The P1-4 payoff: ONE state composes truncation, retained gas, t_visc, tau_disk and the giant verdict, and
        // the connection the prior spine lacked is live: the resonant truncation caps R_1 and the capped R_1 feeds
        // t_visc. So a companion's cut on the disc size reaches the giant gate instead of being an orphan.
        let disk = mirror_disk(Fixed::from_int(30));
        let field = oligarchic_embryo_field(
            &disk,
            Fixed::ONE,
        ...
        let inner = r(1, 10);
        let (strong, weak) = (XrayWindFit::owen_equation_9(), XrayWindFit::sellek_2024());

        let state = derive_disk_evolution_state(
            &embryo,
            &disk,
            &star,
            birth_r1,
            &biting_truncation(Fixed::from_int(40)),
        ...
        .expect("the canonical disk-evolution state composes");

        // The capped radius is inside the birth disc (the cut bit) and the t_visc band is ordered short <= long.
        assert!(
            state.capped_scale_radius_au.hi_au < birth_r1,
            "the truncation capped the 30 AU disc"
        );
        assert!(
            state.t_visc_myr_short <= state.t_visc_myr_long,
            "the t_visc band is ordered"
        );

        // THE NEW CONNECTION: a TIGHTER binary (smaller Roche lobe -> smaller R_t -> smaller capped R_1) gives a
        // SHORTER t_visc, so truncation reaches the clock. The prior spine's stored t_visc could not see this.
        let tighter = derive_disk_evolution_state(
            &embryo,
            &disk,
            &star,
            birth_r1,
            &biting_truncation(Fixed::from_int(20)), // half the Roche lobe: a tighter cut
            TruncationResidualReading::InitialCondition,
            &strong,
            &weak,
            &gas_params(),
        ...
        .expect("the tighter-cut state composes");
        assert!(
            tighter.t_visc_myr_long < state.t_visc_myr_long,
            "a tighter truncation gives a shorter viscous time ({} < {}): the cut reached t_visc",
            tighter.t_visc_myr_long.to_f64_lossy(),
            state.t_visc_myr_long.to_f64_lossy()
        );

        // The gas account holds the RETAINED (truncated) mass, less than the full birth disc's gas.
        let (full_mass, _) = disk_gas_content(&disk, inner, birth_r1, 128).unwrap();
        assert!(
            state.gas.mass_earth < full_mass,
            "the retained gas ({}) is less than the full birth disc's ({})",
            state.gas.mass_earth.to_f64_lossy(),
            full_mass.to_f64_lossy()
        );

        // The whole state inherits ONE grade, proxy because the disc's inner edge is still the magnetospheric
        // interim, and it advertises the standing solar-pinned interims rather than claiming full derivation.
        assert_eq!(state.provenance, DiskGasProvenance::ProxyBounds);
        assert_eq!(state.gas.provenance, DiskGasProvenance::ProxyBounds);
        assert!(state.interims.birth_radius_solar_pinned);
        ...
        assert!(
            ...
            .is_none(),
            "an unresolved residual reading refuses to pin the state"
        );
    }
