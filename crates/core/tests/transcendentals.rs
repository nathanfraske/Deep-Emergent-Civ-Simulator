...
fn near(got: Fixed, want: f64, tol: f64, msg: &str) {
    ...
    assert!((g - want).abs() < tol, "{msg}: got {g}, want {want}, |err|={}", (g - want).abs());
}
...
fn exp_matches_known_values_and_is_monotone() {
    ...
    near(Fixed::from_int(2).ln().exp(), 2.0, 1e-6, "exp(ln 2) round trip approx");
    ...
    assert_eq!(Fixed::from_int(100).exp(), Fixed::MAX, "exp overflow saturates");
    assert_eq!(Fixed::from_int(-100).exp(), Fixed::ZERO, "exp underflow to zero");
}
...
fn ln_matches_known_values_and_inverts_exp() {
    ...
    let a = f(7, 2);
    let b = f(3, 1);
    near(a.mul(b).ln(), a.ln().to_f64_lossy() + b.ln().to_f64_lossy(), 1e-6, "ln additivity");
    // exp(ln x) recovers x across a range.
    for &x in &[0.1, 0.5, 1.0, 2.0, 7.5, 100.0, 1000.0] {
        near(f((x * 1000.0) as i64, 1000).ln().exp(), x, x.max(1.0) * 1e-6, "exp(ln x)");
    }
...
fn sin_cos_match_known_values_and_the_pythagorean_identity() {
    ...
    near((Fixed::PI.div(Fixed::from_int(6))).sin(), 0.5, 1e-6, "sin pi/6 = 1/2");
    near((Fixed::PI.div(Fixed::from_int(3))).cos(), 0.5, 1e-6, "cos pi/3 = 1/2");
...
fn atan_and_asin_match_known_values_and_gate_the_domain() {
    ...
    near(Fixed::ONE.atan(), std::f64::consts::FRAC_PI_4, 1e-6, "atan 1 = pi/4");
    near(f(-1, 1).atan(), -std::f64::consts::FRAC_PI_4, 1e-6, "atan -1");
    near(Fixed::from_int(1000).atan(), std::f64::consts::FRAC_PI_2, 1e-3, "atan large -> pi/2");
    near(Fixed::ZERO.asin(), 0.0, TOL, "asin 0");
    near(f(1, 2).asin(), (0.5f64).asin(), 1e-5, "asin 1/2 = pi/6");
    near(Fixed::ONE.asin(), std::f64::consts::FRAC_PI_2, 1e-6, "asin 1 = pi/2");
    near(f(-1, 1).asin(), -std::f64::consts::FRAC_PI_2, 1e-6, "asin -1");
...
fn pow_integer_and_real() {
    assert_eq!(Fixed::from_int(2).powi(10), Fixed::from_int(1024), "2^10 exact");
    near(Fixed::from_int(2).powi(-2), 0.25, TOL, "2^-2");
    assert_eq!(f(5, 1).powi(0), Fixed::ONE, "x^0 = 1");
    near(Fixed::from_int(2).powf(f(1, 2)), 2.0f64.sqrt(), 1e-6, "2^0.5 = sqrt 2");
...
fn the_transcendentals_are_deterministic() {
    ...
    // contract stands on.
    let xs = [f(1, 7), f(22, 10), Fixed::PI, f(-13, 10), Fixed::from_int(5)];
    for x in xs {
...
fn the_deferred_physics_transcendentals_now_compose() {
    ...
    near(f(12, 10).exp().div(Fixed::ONE).ln(), 1.2, 1e-6, "ln . exp identity for Beer-Lambert path");
    let tau = f(7, 10);
    near((Fixed::ZERO - tau).exp(), (-0.7f64).exp(), 1e-6, "exp(-tau) transmittance");
    // Nernst-style ln of a ratio.
    near(f(50, 1).ln(), 50.0f64.ln(), 1e-6, "ln(Q)");
    // Snell asin of a ratio, with the TIR gate.
    assert_eq!(f(15, 10).asin(), Fixed::HALF_PI, "Snell TIR gate past critical angle");
}
