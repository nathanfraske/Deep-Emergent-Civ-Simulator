...
8. **An authority checker cannot certify itself.** Any mechanical pass that can admit a value, close a proof leaf, select a causal branch, classify provenance, or authorize a completed snapshot requires an independently implemented cross-checker under `docs/working/INDEPENDENT_AUTHORITY_RULE.md`. The pair may share canonical bytes and low-level exact primitives, but not semantic expected answers or decision helpers. An unpaired mechanism remains blocked and emits only diagnostics or refusal.
9. **External adverse claims require owner-signed release.** No public claim or author contact may attribute error, invalidity, misattribution, or a need for correction to an external work until `docs/working/EXTERNAL_ADVERSE_CLAIM_RULE.md` passes with five independent evidence lineages, an exact text and destination binding, a human signature, and independent checker agreement. Without release, record the discrepancy neutrally and let the simulator refuse.
...
- **Independent authority.** Every authority-bearing mechanical claim is enrolled in `scripts/authority_watchdog.toml`. Active claims require an independent pair and mutation canaries. Algebra, hashes, and canaries bind scoped evidence but do not prove that a declared variable universe or human-reviewed scientific argument is complete.
