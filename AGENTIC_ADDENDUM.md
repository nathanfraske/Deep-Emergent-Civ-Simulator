...
THE CHANNEL COROLLARY (owner-ruled 2026-07-18, the disk-arc mint): a number that arrives through the CHANNEL, relayed by a spawning agent, a prior summary, or a research report rather than read from held bytes, is a FETCH-SPEC SEED, never a citation. It scopes the fetch (which source to open, which figure to read, what value to expect) and it may ride the code as a provisional under an explicit fetch-flag, but it does not close provenance and it does not retire the flag. ONLY VENDORED BYTES RETIRE A FETCH-FLAG: the flag comes off when, and only when, the source is downloaded, checksummed, held behind the manifest, and the value is read source-verbatim from its anchor. The founding case in this lane: the Larson-Penston collapse eigenvalue rode as a channel-relayed `46.9` under a fetch-flag until Hunter 1977 was vendored (SHA256 `9e187e6d...`) and the printed `46.915` / `P(0)=8.854` were read from pages 837 to 838, at which point, and not before, the flag came off. A channel value entered as though it were cited is the same defect as a URL standing in for a byte: a claim wearing the costume of provenance.
