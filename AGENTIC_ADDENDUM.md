...
**The generic-ifier pass (its first job).** The smoke test reduces the inputs to their neutral core in its own working view: it disregards every comment, gloss, narrative aside, motivating example, and framing line that is not either what the code does in operation or what the concept at its core is. A comment that asserts the property under audit, a variable name or label that editorializes the intended reading, or a sentence that motivates the conclusion the designer wants is treated as absent. The smoke reports what it neutralized so the transformation is auditable and a load-bearing fact is never discarded with the steering. Minor neutralization does not require the author to rebuild byte-equivalent packets. A rewrite is required only when steering cannot be separated from a fact the panel needs or when the panel itself would still receive the leading material.
...
**The protocol.** Run the smoke test on the strongest model against the full construction before the panel runs, or in parallel with a hold on trusting the verdict. A construction it clears may run and its verdict be trusted. A completed smoke blocks only when it identifies a material defect that can plausibly change a substantive lens verdict: a required falsifier or production surface is absent, the panel-visible framing presupposes the conclusion, source traceability is broken, or the lens set cannot reach a consequential failure class. Minor wording, surplus context, harmless over-inclusion, and metadata imperfections are genericized or recorded as nonblocking notes under `CLEAR`. A material block is fixed and re-run. A panel verdict of "sound" or "clean" is not trusted until the smoke test has cleared its inputs. For the most load-bearing runs, run the smoke test across diverse models too, since it is itself a construction and one strong agent is one voice, the same independence discipline the other sections require.

**It fails closed without chasing perfection.** Only an explicit CLEAR verdict on genericized inputs lets the panel proceed. A missing, malformed, errored, or timed-out verdict blocks because no smoke decision exists. A completed smoke returns `BLOCK` only for a material construction defect under the threshold above, not because the packet could be broader, prettier, or more self-describing. The smoke must distinguish an omitted relevant production path from an intentionally bounded process-control path, and must perform safe genericization itself before demanding a rewrite. After one material correction, a repeated objection in the same class must state a new concrete consequence for a lens verdict. Without that consequence it becomes a note under `CLEAR`; with a disputed consequence, recursion stops at the owner rather than cycling through semantically equivalent packets. The active panel workflows require explicit structured `smokeHistory`, including `[]` on the first run, and carry prior defect classes, corrections, normalized consequences, and disputes. Their wrapper refuses pending or malformed history, checks the verdict enum, handles disputes before genericization, compares claimed new consequences against prior normalized text, returns the next history on a material block, demotes non-substantial and consequence-free repeat blocks, returns `OWNER_REVIEW` for disputed materiality, and launches panelists only on the smoke-produced neutral packet. The PreToolUse guard fails closed on malformed hook input, unreadable resumed scripts, marker-only acknowledgments, and non-terminating `CLEAR` checks. Its ten-case self-test covers both active templates.

**Its own limit, and where the recursion stops.** The smoke test is subject to the very flaw it hunts: it is a construction, and a designer who frames it to be reassured gets reassurance. Two things bound this. The negation charge is structural rather than a checklist: it is told to find what the setup omits, so it works against the designer's frame instead of within it. The materiality threshold prevents the meta-audit from turning completeness into an unbounded recursion. The owner is the backstop for a disputed load-bearing call; a smoke test that clears a panel is a licence to trust the panel, never a proof, and a result the owner would want to see is surfaced rather than buried under a green panel.
