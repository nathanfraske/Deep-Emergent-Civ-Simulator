...
- **The Larson-Penston collapse eigenvalue (m0 = 46.915 at A = 8.854).** RESOLVED, VENDORED (2026-07-18, retiring the channel-relayed flag the f369bdf audit restored). Read source-verbatim from the PRIMARY: Hunter, C. 1977, "The collapse of unstable isothermal spheres," ApJ 218, 834, DOI 10.1086/155739, ADS bibcode 1977ApJ...218..834H. Extracted: p.838 (`"Values of m0 are 46.915, 2.577, 1.138, and 0.975 for the Larson-Penston, (11b), (11d), and expansion wave solutions"`, so the LP eigenvalue is 46.915 and the Shu expansion-wave is 0.975), and p.837 (`"values of P(0) ... being 8.854, 2.378, and 2.032"`, so the LP abscissa is Hunter's central-density coefficient P(0) = 8.854; Shu is P(0) = 2). Convention (eqs. 1 and 14, `M = a^3 t m(zeta)/G`, `m -> m0`) gives `Mdot = m0 c_s^3/G`, matching the code. SHA256 `9e187e6d69cccf733734b75c7b974f287532163692514084eb511828d6a70e0f`, scratchpad `lp_collapse_vendor/hunter1977.pdf`. DUAL-CHANNEL: Whitworth & Summers 1985 (MNRAS 214, 1, DOI 10.1093/mnras/214.1.1, SHA256 `ba57e11cdc7b1699c5af8e22b6be8282787a428bb28c17cac6a6f7dc01be9457`) print the same member as `w0 = 46.84` under their `(z0, w0)` parametrization (z0 = 1.672), a 0.16% spread from Hunter's independent integration, far below the factor-48 band; the carried value tracks Hunter. Hunter 1977 also independently corroborates the Shu 0.975 already vendored (Shu 1977, `af390700`). DUAL-CHANNEL CONFIRMED (re-fetch 2026-07-18, same SHA256, independent visual re-read of the page crops agrees with the OCR text layer exactly, 46.915 and 8.854, no digit flip). THE THREE FORMERLY-OPEN COLLAPSE-LIT ITEMS, now VENDORED with two INPUT-AUDIT CORRECTIONS: (a) SHU 1977 TABLE 1 `m0(A)` row confirmed (`af390700`), header "RELATION BETWEEN A AND m0", A = 2.00, 2.20, ..., 4.00 giving m0 = 0.975, 1.45, ..., 5.58; A = 2 is the hydrostatic SIS / expansion wave; LP (Hunter m0 = 46.915, P(0) = 8.854) lies FAR outside Shu's A-range on Hunter's separate secondary branch, so the two families share only the A = 2 expansion-wave limit (no single closed-form A <-> P(0) identity is printed). (b) ORI & PIRAN 1988 (MNRAS 234, 821, SHA256 `968e318bf63b0350072f2fb4704e914990f2ec739c961d1c5a76be9dfec898c1`) CORRECTED: the paper proves a NECESSARY condition only and states verbatim it "does not show that the secondary-direction ... solution is stable"; the primary-direction family (homogeneous collapse) is unstable and ruled out, LP is the surviving candidate, NOT proven-uniquely-stable (the channel over-stated it). The literal inequality direction carries a sign-convention ambiguity, flagged; rely on the thrice-stated verbal conclusion, do not hardcode the inequality. (c) FOSTER & CHEVALIER 1993 (ApJ 416, 303, SHA256 `dfd6f006eff9a11d9d0ab79f56c5702b298d435c8356593d5a93418c30a780eb`) CORRECTED: the printed central peak is `~47 a^3/G` at r=0 immediately after core formation, declining sharply (LP reference 29, Hunter initial 36); there is NO `~13 c_s^3/G` anywhere in the paper (the channel's figure is dropped), and opacity is NOT in the collapse dynamics (isothermal hydro; opacity enters only the transparent line-profile diagnostics). Bytes in scratchpad `collapse_lit_vendor/` pending the physics-lane manifest landing.
