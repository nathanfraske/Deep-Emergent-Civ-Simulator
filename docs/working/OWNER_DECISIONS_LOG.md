...
FORAGING-AND-DISPERSAL framing, TWO RULINGS PUT TO THE GATE, awaiting its ruling (#136 design-first opener, #126 gate comment 4942764584, 2026-07-11, Agent B; section-10 blind framing panel of five panelists across agent types and models, every decisive claim verified at source, the owner independently raised the primary seam). The arc makes the forage-and-disperse policy EMERGE from in-world selection (the `--scenario living` regime goes extinct by gen 30 in a land of plenty because the offline dawn-bootstrap controller does not transfer). The frame-blind caught two seams. FIRST: the existing resource-direction percept PRE-SOLVES foraging in perception (`source_dirs` at locomotion.rs:952 gives one direction slot per reserve axis with the backing resolved, `nearest_known` locomotion.rs:794 is an argmin, `perceive` locomotion.rs:934 learns only sources of axes the being carries, the usability filter), authoring food-identity where P8 says it must emerge (the template case moved one level down). SECOND: the offline synthetic warm start (`recurrent_forage_seeds` -> `reserve_conflict_survival`) is retired by narrative only, and the in-world bootstrap is unproven. RULING 1 (the perception fork): raw UNLABELED per-resource-class field percepts so food-identity, the source-to-reserve mapping, and the approach all emerge from selection (my recommendation, the strongest P8 form, for symmetry with the being-percept the predation arc reads), OR a per-reserve source-direction ruled explicitly an innate P9 sense organ keyed on the being's own metabolism (food-identity innate, only approach/intake emerge). RULING 2 (the warm-start disposition): retire the offline synthetic warm start as binding for canonical runs and prove the in-world bootstrap with a zero-init or P9-generic-gain control, surfacing the bootstrap-viability values (founder pool, mutation bounds, reserve capacity/drain, sense range vs resource density) reserved-with-basis keyed on the floor's own survival-viability boundary (my recommendation: retire it, adopt a P9 generic symmetric orienting gain if founder-zero cannot bootstrap, with a standing no-fallback rule). A secondary admit-the-alien flag carried forward: whether a resource class can be a non-depleting, passively-replenishing supply (ambient light, a mana field) as data, so a photosynthetic or field-feeding being is a data row. No code until the gate rules the fork.
