...
RESOLVED framing, creature-selection STEP 2b (the percept kind-feature FLOOR arc, #126), 2026-07-11 (blind section-11 input-bias smoke, FOUR fail-closed passes to CLEAR, each catch verified at source; then the section-10 blind panel, 6 of 6 significant-flaw-fixable, UNANIMOUS across three agent types and two models; every decisive claim verified against source, Prime Directive 1). The gate ruled the DIRECTION in the step-2 framing (the being-signal must carry a FEATURE correlating with emitter-kind that a heritable weight keys on, the creature evolving the sign); this frame-blind settles the exact feature design and surfaces that it splits into a value-authoring breach and a physics-carrier gap, neither a value to tune. The smoke's four passes hardened the packet before the panel ever saw it: it caught (round 2) that the packet omitted covering-material PROVENANCE (verified at anatomy.rs:775: covering is an emergent genome-expressed heritable draw weighted by the being's own evolved morphological profile, inherited and selected, NOT an authored per-role or per-species label, so any covering-to-consequence correlation emerges through shared heritable causes, the P8-licensed proxy path), overstated the live emissivity read (corrected: the live emission is one reserved coefficient today, the per-covering read a flagged follow-on), and let the statement assert refractive index is emitted (it is a reflectance property, not radiated); (round 3) the one-level-up authoring risk (is the feature-CHANNEL set open data-driven or a fixed slot: verified at BeingPerceptField, learn.rs:1002, a single channel slot today with a registry-id channel identity) and the no-covering-analogue alien case (covering is a required filled slot, anatomy.rs:774); (round 4) the feature-EXTRACTION P11 provenance (fact 10, the direct-read-versus-composite and open-registry-versus-fixed-set question). THE PANEL'S UNANIMOUS FINDING, in three source-verified layers. LAYER A, the value-authoring-line breach (all six panelists): the phrase "an optical signature of its covering material" smuggles an authored COMPOSITE of several floor optical axes under an unstated weighting recipe, computed UPSTREAM of the discrimination buckets, which the selection-filled per-bucket weights cannot launder because the composite is fixed before selection ever sees it; a composite hand-tuned to separate a wood covering from a chitin covering is a de-facto material-KIND classifier carrying no kind label, reopening the P8 template violation the statement's disclaimer guards only in its explicit form (gp-sonnet's sharpest catch). The CLEAN FORM the panel converges on: the carried feature must be a DIRECT read of exactly ONE floor optical axis, each axis its own perceivable channel drawn from an OPEN per-property registry (sibling to the channel registry's MATERIAL_FEATURE_CHANNEL_BASE partition in learn.rs and the covering registry), with ALL cross-axis combination deferred to SELECTION over the per-bucket weights (to make more than one axis matter, open more single-axis channels, never pre-fold); a later composite is admissible only if authored as a physics-floor LAW (as the refractive-contrast law already is), its output then a floor derivation rather than a perception-layer recipe. The RESPONSE machinery (per-bucket JND discrimination plus a heritable freely-signed weight per bucket, sign and prevalence set by selection alone, no kind-label) is unanimously SOUND, the correct generalization of the accepted strength-weight (fact 2), passing the P8 template test. LAYER B, the physics-carrier gap (all six): the "optical signature" example is physically incoherent on the only channel that exists. Verified at source: emissivity is ALREADY the strength scalar (being_signal_emission folds it into the emission coefficient, the linear scale of the blackbody radiant flux), so it carries no strength-INDEPENDENT feature on the thermal channel; refractive index governs how EXTERNAL light refracts at the surface (the floor's refractive_contrast interface law, two indices across a boundary) and is not radiated by the body; and the being-perception pathway carries only thermal self-emission, with NO incident-illumination field feeding a reflected being-signal (albedo IS a floor axis, opt.albedo, and a reflect-absorb-transmit law exists, chem_optics_floor.toml, but nothing wires an ambient light source onto the being-percept path). So the statement's payoff (two beings at equal strength fall in different feature-buckets) is UNFOUNDED on the current single thermal channel until a strength-independent carried optical axis exists: either a SPECTRAL emissivity (a wavelength-resolved self-emission read per band, needing a spectral-emission substrate, since emissivity is one scalar today) or a REFLECTANCE channel (an albedo direct-read on an AUTHORED incident-illumination floor axis plus interface geometry, mirroring the existing refractive-contrast law). Physics may be authored (P9), so this floor addition is legitimate; the behavioural response stays emergent. LAYER C, the alien key (all six): key the read on the being's OWN surface material (its covering when present, its body surface otherwise, a null feature when neither, the feature ABSENT when its carrier signal is absent, never a synthesized default), rather than a fixed "covering" slot, so a coveringless, plasma, or field being is a data row rather than a rewrite (closing the fact-7 gap for this feature). THE RESOLVED FRAMING (for the gate's ruling): the mechanism's RESPONSE layer is sound and ready to build; 2b's blocking work is the FLOOR and percept substrate, exactly as the gate framed 2b (a percept-FLOOR arc, not a selection mechanism), in three parts: (1) an OPEN per-property perceivable-feature registry, one direct-read floor axis per channel, no composite; (2) a strength-independent CARRIED optical axis (spectral emissivity or an authored reflectance channel with its illumination physics) so the feature is physically real rather than parasitic on strength; (3) the read keyed on the being's own surface data with graceful absence, admitting the alien. The honest limit stands: on the current single thermal channel NO strength-independent emitter feature exists, so FLEEING cannot emerge until 2b builds the carried feature-axis, the open registry, and the response wiring, the response machinery already being correct. A caution folded in for the build: the gate's "correlating with emitter-kind" must remain an EMERGENT correlation (a covering axis that happens to correlate with predator-versus-mate through shared heritable causes and selection), never a composite engineered to separate kinds, which would author the very classifier the disclaimer forbids. Posted on #126 for the gate's ruling before code.

RESOLVED framing, creature-selection STEP 2 (the reproduction and behaviour-selection substrate, #126), 2026-07-10 (blind section-11 smoke FAILED on my own packet, caught three construction biases, fixed, re-smoke CLEAR; then the section-10 blind panel, 6 of 6 significant-flaw-fixable, unanimous across three agent types and three models; every decisive claim verified against source, Prime Directive 1). The frame-blind dissolved the proposed mechanism and surfaced a BLOCKING floor dependency. Two findings, both source-verified. FINDING 1, the blocking floor seam (the deepest catch, gp-opus, verified at source): a mind-less CREATURE cannot perceptually distinguish the authored predator from a conspecific, so the arc's stated payoff (fleeing emerges from the predation pressure) cannot emerge from ANY selection regime. Verified: `learn::creature_being_direction` sums a magnitude-weighted toward-vector over ALL perceived emitters with no belief filter and no per-source feature (a creature has no `Mind`, so unlike the founder's belief-keyed `being_avoidance_gradient`/`being_attraction_gradient` which filter on a LEARNED HARMS/REWARDS belief, the creature sees only an undifferentiated magnitude aggregate), and the predator emits the SAME body-signal as any warm walker (`spawn_predator` seeds `body_temp`). So the predator-signal and the mate-signal reach the controller as the same magnitude-at-a-position through ONE freely-signed being-weight: a negative weight flees the predator AND every mate (never co-locates, never reproduces), a positive weight approaches mates AND walks into the predator, and the equilibrium sign is set by the ratio of the survival and reproduction pressures rather than a selective fact of the world (P8 breach), so a being that approaches mates and flees predators is unrepresentable as data (admit-the-alien breach). This is a physics-floor / percept representational limit, not a value to tune: the emitted being-signal (or the percept) must carry a FEATURE that correlates with emitter-kind and that a heritable weight can key on separately (a per-source component on the field, plus the deferred B2 per-bucket or per-feature discrimination weights), the creature evolving the sign, never an authored which-feature-means-predator. The founder tier does not have this problem because it LEARNS a per-emitter valence; the creature is definitionally non-learning (mechanism (i) rejected in the creatures-react frame-blind), so the feature must be perceptual, not learned. FINDING 2, the reproduction-substrate framing needs revision (unanimous, verified): my proposed framing partitioned the controller weights into solo-expressed (routed to the offline single-agent scorer) and social (routed to the new in-run reproduction), which is an authored closed classification (P8, P11) and incoherent (the being-weight is both, via the hazard). The converged fix: ONE unified in-run selection over the WHOLE controller in the populated deployment world (hazard, conspecifics, and solitude all present); retire the offline empty-arena scorer (verified single-walker `vec![walker]` in `evolve.rs`, so it shapes weights against a landscape the creature never lives in) or demote it to a bootstrap-only variance seed; which weights matter falls out of which situations each agent encounters (a weight on a channel that is zero-valued alone gets no gradient alone, structural, never a tag). Plus break the zero-init deadlock with a bounded zero-mean mint-time perturbation (physics-floor-legitimate) so founders move and occasionally co-locate. Plus state pairing eligibility as a STRUCTURAL predicate (both occupants carry a heritable controller of the population schema and a draining reserve) and specify the hazard as structurally carrying no heritable controller, so it fails pairing for the same reason it cannot adapt, replacing the current authored `PREDATOR_ID_BIT` id tag (a type tag the framing disclaims) with a structural fact. THE RESOLVED SEQUENCING (for the gate's ruling): (2a) build the unified in-run selection substrate (it makes the creature's SOLO-expressed loci, forage and metabolism, selectable in-run, the correct single-loop architecture, the structural pairing predicate, the mint-time variance seed), which is buildable and correct but does NOT deliver the fleeing payoff; and (2b), FLAGGED as the blocking dependency, the perceptual-distinguishing-feature floor/percept arc (the being-signal feature plus B2), which unblocks the being-directed flee and approach emergence and which no reproduction mechanism can substitute for, its own frame-blind, not built in 2a. Posted on #126 for the gate's ruling before code.
---
...
GRADE-BINDING UNIFICATION, GATE-SIGNED-OFF to build 2026-07-10 (Agent A, PR #129), the priority follow-on after stroke-rate step 2. The seam three step-2 section-9 lenses flagged: the capability GRADE path binds its axis ids POSITIONALLY (`FunctionLawDef`'s `geometry_axes`/`material_axes` `Vec<String>` order) while the delivery path binds them by NAMED fields (`ContactTransfer`'s six `*_axis` Strings), so a springy alien whose `material_axes` omits the rigid-strength slot has its yield read into the strength slot, fabricating a rigid blow (an alien-feasibility defect, verified at source, not a cosmetic desync). GATE HARDENING (the decisive ruling): do NOT unify onto `ContactTransfer`'s CLOSED six-field struct, which is still a closed role set (a seventh alien actuation role could not be a data row); unify BOTH paths onto ONE shared, DATA-DEFINED binding, a role-name -> axis-id MAP whose role SET is data and extensible, sibling to the value substrate (Part 21), the semantic substrate (Part 33), and the institution-function substrate (Part 36). Both paths look up the binding by role NAME; a kernel declares its required roles; a missing role is caught at LOAD VALIDATION, so a desync is a load error and the lockstep is MECHANICALLY enforced, retiring the positional caveat AND the named-struct closed set in one pass. Refactor all six capability-grade kernels (Pierce, Locomote, Refract, Shear, Crush, Impact) plus the three delivery kernels onto the shared map. BYTE-NEUTRAL: the default map carries today's roles mapped to today's axis-ids in today's order, so every capability score and all four pins (`4bbf6b59`/`1db633b3`/`c9d5cc17`/`ad69f2bf`) stay bit-identical; a data-representation refactor, no new floor axis, no new law, no value in the world-content path. FRAME-BLIND NOT REQUIRED (the gate ruled): constrained to the data-defined extensible map, the arc authors no cultural or emergent outcome and bakes no body plan, it improves alien-feasibility; the mandatory section-9 five-lens (alien-feasibility and derive-versus-author lenses live) plus the byte-neutral pins are the discipline. ONE TRIGGER to escalate to the section-11-then-section-10 frame-blind: if the shared type forces a role that must live in code, not data (an emergence seam). NORTH-STAR sibling seam, FLAGGED not scoped here: the deeper general form is the role-to-axis binding DERIVING from physics, each kernel declaring the physical quantity it needs by dimension or physical role and the binding resolving by matching the axis's own declared meaning, so no authored role map is needed at all, a larger arc touching the axis-declaration substrate. Building on #129 under the hardening; the gate gates asynchronously, each step's result posted as it lands.
GRADE-BINDING UNIFICATION, GATE-SIGNED-OFF to build 2026-07-10 (Agent A, PR #129), the priority follow-on after stroke-rate step 2. The seam three step-2 section-9 lenses flagged: the capability GRADE path binds its axis ids POSITIONALLY (`FunctionLawDef`'s `geometry_axes`/`material_axes` `Vec<String>` order) while the delivery path binds them by NAMED fields (`ContactTransfer`'s six `*_axis` Strings), so a springy alien whose `material_axes` omits the rigid-strength slot has its yield read into the strength slot, fabricating a rigid blow (an alien-feasibility defect, verified at source, not a cosmetic desync). GATE HARDENING (the decisive ruling): do NOT unify onto `ContactTransfer`'s CLOSED six-field struct, which is still a closed role set (a seventh alien actuation role could not be a data row); unify BOTH paths onto ONE shared, DATA-DEFINED binding, a role-name -> axis-id MAP whose role SET is data and extensible, sibling to the value substrate (Part 21), the semantic substrate (Part 33), and the institution-function substrate (Part 36). Both paths look up the binding by role NAME; a kernel declares its required roles; a missing role is caught at LOAD VALIDATION, so a desync is a load error and the lockstep is MECHANICALLY enforced, retiring the positional caveat AND the named-struct closed set in one pass. Refactor all six capability-grade kernels (Pierce, Locomote, Refract, Shear, Crush, Impact) plus the three delivery kernels onto the shared map. BYTE-NEUTRAL: the default map carries today's roles mapped to today's axis-ids in today's order, so every capability score and all four pins (`4bbf6b59`/`1db633b3`/`c9d5cc17`/`ad69f2bf`) stay bit-identical; a data-representation refactor, no new floor axis, no new law, no value in the world-content path. FRAME-BLIND NOT REQUIRED (the gate ruled): constrained to the data-defined extensible map, the arc authors no cultural or emergent outcome and bakes no body plan, it improves alien-feasibility; the mandatory section-9 five-lens (alien-feasibility and derive-versus-author lenses live) plus the byte-neutral pins are the discipline. ONE TRIGGER to escalate to the section-11-then-section-10 frame-blind: if the shared type forces a role that must live in code, not data (an emergence seam). NORTH-STAR sibling seam, FLAGGED not scoped here: the deeper general form is the role-to-axis binding DERIVING from physics, each kernel declaring the physical quantity it needs by dimension or physical role and the binding resolving by matching the axis's own declared meaning, so no authored role map is needed at all, a larger arc touching the axis-declaration substrate. Building on #129 under the hardening; the gate gates asynchronously, each step's result posted as it lands. BUILT (PR #129, byte-neutral, four pins hold): SLICE A (`eb852de` + `e12cd4e`) the GRADE path, all six kernels reading by role name off the shared `AxisBinding` (a role-name -> axis-id `BTreeMap`, data + extensible); `FunctionLawDef` carries `binding` (positional `geometry_axes`/`material_axes` Vecs retired), `new()` infallible from `kernel.default_binding()`, `with_binding()` validated fail-loud. SLICE B (`0034f90`, fmt `0af05f8`) the DELIVERY path, `ContactTransfer` off its six named `*_axis` fields onto the SAME `AxisBinding`; the three delivery kernels read by role name; `dev_terran` built from `CapabilityKernel::Impact.default_binding()`; `ContactTransfer::new` + `ContactTransferRegistry::validate` validate the role SET against `mechanical_family_roles()` (which references `Impact.roles()`, one definition); the pre-unification IMPACT positional-contract caveat (a springy binding mis-reading yield into the rigid-strength slot) RETIRED, a missing role now a fail-loud LOAD error (new test); the lockstep drift-test retired. SECTION-9 (`3a0b672`, five-lens over the whole unification, findings verified at source): the confirmation-bias and derive-versus-author lenses CAUGHT that the prose overclaimed the lockstep, so it was narrowed to the honest scope: the shared ROLE SET is mechanically enforced at load, but the role-to-axis MAPPING agreement across the two paths is NOT structural (grade and delivery each own an `AxisBinding`, coinciding only because both dev fixtures build from the one `Impact.default_binding()`; a world authoring the two separately could diverge and pass both role-set checks). Added `AxisBinding::agrees_on` (the mapping-agreement check) and strengthened the guarding test to the adversarial divergence case; the STRUCTURAL single-source fix (a delivery row reading the grade law's binding directly, one referenced map) is FLAGGED as the follow-on, the runner not currently holding a persistent grade registry. Derive-versus-author CLEAN (no authored value entered the world-content path; the axis-id strings name floor axes, the one authored place; ENERGY_GUARD untouched). Alien-feasibility a NET IMPROVEMENT (the positional silent-shift retired) with two documented BOUNDARIES on record, neither introduced by this arc: the delivery family is MECHANICAL-only (a non-mechanical channel is the flagged floor extension, not a data row), and the live GRADE consumer `affordance_percept` (Sharpness/Pierce) still passes a hardcoded `default_binding()` with a `mech.contact_area`-only geo closure (the pre-existing Sharpness alien seam, already flagged in that file). One latent seam folded into the north-star follow-on: the geo-vs-mat accessor CLASS per role is fixed Rust restated in each path's kernel bodies, not carried by the binding, so a future added role misclassified across paths would silently diverge (all six current roles classified identically, no live divergence); deriving the accessor class from the axis's declared dimension is exactly the north-star dimension-matched derivation. Four pins hold bit-identical throughout (`4bbf6b59`/`1db633b3`/`c9d5cc17`/`ad69f2bf`); the gate runs its independent pin verify on the complete unification.
