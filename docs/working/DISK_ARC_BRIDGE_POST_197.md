...
> MINTED RULE (2026-07-18, gate ruling on the retirement census, for the ledger since the trio recurs). Test constants sort by ROLE, and role sets the law. A FIXTURE is arbitrary: nothing downstream depends on which number it is, so retired digits in it are pure camouflage, cost-free to move; de-camouflage freely and keep it off retired digits (the `disk_era_xray` fixtures, moved to a labelled arbitrary value). An ORACLE POINT is a load-bearing pair: the input and its hand-computed expectation are one artifact (`L(T_eff=4000) = 1.6686`), true as long as the mapping's math is unchanged, so it is FROZEN with its expectation and moved only when the mapping itself changes, at which point the whole pair recomputes by an independent route; the camouflage concern is cured by LABELING, not moving (the one-line "oracle evaluation point, arbitrary and frozen with its pair, not the retired production constant, which reads the grid" plus the route statement already at the assert site, making the pair tamper-evident by review). A PHYSICAL CLAIM does not live in a test at all: anything asserting world-truth routes to the hindcast machinery.
