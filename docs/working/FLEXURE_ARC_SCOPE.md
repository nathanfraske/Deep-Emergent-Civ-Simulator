...
Status: scoped, fetch-gated on the Turcotte-Schubert flexure formulation. Written-arc-first per the owner's split (truly fetch-gated, unlike slices 1-4). This fills the FLEXURAL MIDDLE the first surface plan missed: the band between the coarse convective provinces (~1000 km, the resolved field) and the fine crater rows (the discrete objects), which is exactly the ~100-1000 km wavelength band a human reads as mountain ranges, foreland basins, rifts, and flexural bulges. Without it the surface is a smooth ball plus craters and skips the scale terrain lives at.
...
- FLEXURAL RIGIDITY `D = E * T_e^3 / (12 * (1 - nu^2))`. `E` (Young's modulus) and `nu` (Poisson's ratio) come from the banked material moduli (the same modulus tier the support-bound yield derive-down used, `phase_bulk_modulus_ionic` and the shear/Pugh chain); `T_e` is the DERIVED elastic lid thickness (the depth to the brittle-ductile / mechanical base of the lithosphere, which the deep-time thermal state already carries: a young hot world has a thin lid, an aged cool world a thick lid). No authored rigidity.
