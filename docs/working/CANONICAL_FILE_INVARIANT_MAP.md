...
| `stellar_birth_species/law_premise/` | Exact claim-scoped premise selection, one active derived `eps_0` relation, a paired bounded generic derive-first route, and one claim-local registered-catalog protocol pair for each enrolled profile. | Focused tests cover the live derived relation, exact role and content selection, target-scoped minimal grounded ancestry, cyclic non-minting, independent search completeness, exact work preflight, both typed irreducible branches, unrelated alien extension, occupied-slot collision, target-scoped catalog injection, malformed and bounded refusal, and cross-algorithm agreement. | Keep selection, derived-relation authority, generic routing, and claim-local protocol scanning in separate child boundaries. The generic route cannot close an arbitrary target or rule universe. Every unfamiliar profile needs a new catalog, coverage, protocol, slot, owner boundary, and authority pair. |
| `stellar_birth_species/authority_analysis/` | Permanently non-admitting analysis v11 under watchdog v12, bound to the floor, live premise pair, four profile authority receipts, physical-root pair, exact admission censuses, the 125-artifact vocabulary partition, four local members, and six open obligations. | Focused unit tests plus typed read-only views, canonical CLI coverage, and viewer projection tests cover all profile identities, protocol statuses, four Residue `[A]` plus 117 Residue `[D]` admissions, four members, one zero-member confining profile, zero verified support, and false global coverage. | Keep separate from admitting authorities. Analysis success, profile agreement, route agreement, root-pair agreement, or current-input vocabulary coverage cannot mint support or a global frontier-completeness claim. |
| `stellar_birth_species/physical_registry/` | Conditional admitted-artifact proof graph, independent sealed-floor root projection, four claim-local profile pairs, bounded three-way physical-vocabulary classification, exact mass and dimension validation, and local species closure. | Root-pair, profile, vocabulary, and physical-registry tests cover independent floor extraction, exact rational vectors, source order and identity binding, protocol and semantic receipts, unfamiliar and massless graphs, exact-zero shape, derived and irreducible routes, tier and provenance censuses, closure defects, exact arithmetic, depth, resource caps, and authority escalation refusal. | Keep root projection, each profile admission, vocabulary classification, registry closure, conditioned support, and the reducer in their child boundaries. The repository closes exactly four local members plus one zero-member confining profile with `authority_effect=none`; global vocabulary, confining-sector constituent content, complete membership, and support remain open. Integrity failures remain hard errors. |
