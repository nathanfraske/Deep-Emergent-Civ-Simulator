...
| `stellar_birth_species/law_premise/` | Exact claim-scoped premise selection, one active derived `eps_0` relation, a paired bounded generic derive-first route, and the separately enrolled theory-profile protocol authority. | Focused tests cover the live derived relation, exact role and content selection, target-scoped minimal grounded ancestry, cyclic non-minting, independent search completeness, exact work preflight, both typed irreducible branches, unrelated alien extension, occupied-slot collision, target-scoped catalog injection, malformed and bounded refusal, and cross-algorithm agreement. | Keep selection, derived-relation authority, generic routing, and the enrolled protocol in separate child boundaries. The generic route remains blocked and cannot close an arbitrary target or rule universe. Every unfamiliar profile needs a new catalog, coverage, protocol, slot, owner boundary, and authority pair. |
| `stellar_birth_species/law_premise/admission_route/theory_profile_protocol/` | Independently paired forward and reverse reconstruction of one raw theory-profile request, catalog, residual slot, and irreducible protocol. | Focused producer, watchdog, and agreement tests cover raw-request binding, malformed and colliding inventory refusal, unfamiliar extension, paired receipt integrity, and non-minting authority flags. | May seal only claim-scoped structural capability evidence. The four outer physical-profile pairs consume that opaque evidence after validating its identity and false authority flags; they do not call this pair's semantic decision helper. |
| `stellar_birth_species/authority_analysis/` | Permanently non-admitting analysis v12 under watchdog v13, bound to the floor, live premise pair, four profile authority receipts, the paired confining-constituent frontier, physical-root pair, exact admission censuses, the 125-artifact vocabulary partition, four local members, and thirteen open obligations. | Focused unit tests plus typed read-only views, canonical CLI coverage, and viewer projection tests cover all profile identities, protocol statuses, four Residue `[A]` plus 117 Residue `[D]` admissions, four members, one zero-member confining profile, one opaque internal seed, eight exact constituent gaps, zero constituent candidates, zero verified support, and false global coverage. | Keep separate from admitting authorities. Analysis success, profile agreement, route agreement, root-pair agreement, constituent-frontier agreement, or current-input vocabulary coverage cannot mint support or a global frontier-completeness claim. |
| `stellar_birth_species/physical_registry/` | Conditional admitted-artifact proof graph, independent sealed-floor root projection, four claim-local profile pairs, paired confining-constituent frontier, bounded three-way physical-vocabulary classification, exact mass and dimension validation, and local species closure. | Root-pair, profile, constituent-frontier, vocabulary, and physical-registry tests cover independent floor extraction, exact rational vectors, source order and identity binding, protocol and semantic receipts, unfamiliar and thaumic extensions, exact-zero shape, derived and irreducible routes, tier and provenance censuses, closure defects, exact arithmetic, depth, resource caps, and authority escalation refusal. | Keep root projection, each profile admission, the non-authorizing constituent frontier, vocabulary classification, registry closure, conditioned support, and the reducer in their child boundaries. The repository closes exactly four local members plus one zero-member confining profile. The constituent frontier exposes one opaque internal seed and eight typed missing authorities with zero candidates and `authority_effect=none`; global vocabulary, constituent authority, complete membership, and support remain open. Integrity failures remain hard errors. |
...
non-admitting analyses, the paired derive-first decision and missing-role
blocker, and the complete physical-root agreement frontier.
