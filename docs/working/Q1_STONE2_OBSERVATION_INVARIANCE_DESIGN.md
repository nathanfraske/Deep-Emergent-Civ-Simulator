...
## The one design question for the gate

Which observation ops to schedule, and whether to add the `viewer`-import gate line now. My recommendation: schedule all four available observer reads (`to_snapshot`, the conservation snapshot, the living-world projection, a render-to-buffer) so the guard covers the whole observation surface, and add the `viewer`-import ratchet line now (it is cheap and it closes the structural boundary as a merge gate). If you would rather keep the render out of the test crate's dependency set, I schedule the three `sim`-internal reads and add the import gate to cover the render path instead.
