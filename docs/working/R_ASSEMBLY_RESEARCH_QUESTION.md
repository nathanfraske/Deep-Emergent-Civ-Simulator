...
## SELF-AUDIT REFINEMENTS (researcher's audit of the R-ASSEMBLY ruling, 2026-07-15)

The researcher ran the same three lenses on the ruling itself. Verdict: FIX, not retract. The C kernel holds and strengthened under verification; the defects are undeclared domain branches plus one rider-enforcement gap. The unifying pattern (findings 2, 3, 7): a BRANCH INSTANCE presented as the mechanism, the same Terran-fact-as-mechanism defect the five-lens panel caught in the texture commit. Findings 1 and 6 are one defect (estimator grade sneaking into an exponent) seen from two sides.

1. EXPONENT-RIDER BREACH in B (sharpest). The `t_inst` coefficients live in an exponent; at calibration nodes they are compute-once (legal), but off-node interpolation between ensemble nodes is estimator grade, which the ladder forbids in an exponent. FIX: the analytic overlap form (Petit) is the MANDATORY exponent carrier (derived structure legally carries extrapolation), its own scaling variable mandatory (mutual Hill radii demoted to a display convention), coefficients compute-once at nodes, off-domain queries ESCALATE through the amortization loop (run the node) and NEVER interpolate. Lands before the spacing-law code, so it costs nothing.
2. BRANCH SMUGGLING in A. `b ~ 10 r_H` is the class constant of the PLANETESIMAL-driven oligarchic branch specifically; the rival is pebble accretion (terminates at the pebble isolation mass, not feeding-zone exhaustion). Which branch dominates DERIVES per-system from the planetesimal-to-pebble surface-density (flux) ratio (a pebble flux near 110 Earth masses/Myr yields Mars-sized embryos; doubling it yields close-in super-Earths). FIX: `b` stays [M class] and universal WITHIN the oligarchic branch; branch membership derives per-system from the flux ratio; the pebble-branch embryo field is its own calibration target or a declared wall. Slice-1 inheritance is one doc line (applied to `OLIGARCHIC_SPACING_HILL_WIDTHS`).
3. CALIBRATION-DOMAIN HOLES in gate 4 (now resolvable, not walled). The pinned ensembles are gas-free, comparable-mass, rocky, 0.5 to 1.5 AU. Two branches sat outside: (a) the GAS-RICH branch, where migration builds resonant chains and roughly 95 to 99 percent go unstable after gas dispersal, converging onto the SAME instability-sculpting map (an initial-configuration branch, chain versus oligarchic field at dispersal, conditioned by the damping group already in the Pi list; the surviving few percent is the stable-chain TRAPPIST-1 draw) and STRENGTHENING C, since intrasystem uniformity also emerges from chain-breaking; (b) the GIANT-PERTURBED branch, where the F-race resolves during the gas disk, so the F-to-assembly coupling is BIDIRECTIONAL in time and the map must condition on the resolved giant configuration. Also the perfect-merge (Kokubo-Genda) insensitivity holds only near the merging boundary; in the high-velocity erosive regime fragmentation feeds back on architecture, so the map branches on `v_imp/v_esc`.
4. PI ENUMERATION INCOMPLETE (the enumeration is the discipline). Missing groups: exterior-giant forcing (derived from F output), the material-class selector for the collision map (derived from condensation feedstock, the rubble-pile band its visible edge), and the feedstock branch ratio (finding 2). Corrected count near FIFTEEN. The zero-new-authored-scalars conclusion survives (every addition is derived), but a closed count was presented that was not closed.
5. ECHO-VERSUS-HINDCAST FIREWALL (implied, now a rule). Any observationally fit distribution entering the sampler converts the corresponding battery row from HINDCAST to ECHO, and the row retags. Companion: the battery validates the COMPOSITE sampler-plus-projector, because conservation projection deforms the raw measure (validating the unprojected map validates the wrong object).
6. ESTIMATOR TIER UNDER-ENABLED (this hid finding 1). Between-node surrogate error was never registered as an [E] entry with a band and node-distance escalation; register the tier so the exponent rider bites visibly instead of silently.
7. GATE G (minor). Deriving `gamma` and `r_c` from the viscous similarity family wires the alpha-viscous branch as mechanism; wind-driven disk evolution is the live alternative. Same treatment as the type II dispute: a declared model-structure band on the disk-evolution closure, fetch task before gate G closes.

VERIFIED SOUND: the C core (sub-resolution typestate forcing SeededDraw, the chaos-protocol grounding, option 1 demoted to calibrator and option 3 to projector) stands and emerged STRONGER (an independent formation branch lands on the same instability-sculpting machinery); B's one-surface unification stands with the three candidate numbers as its evaluations; the conservation gates, seed discipline, forward-model rider, and layer-4 accounting all stand. Added fetch items: the pebble-vs-planetesimal flux-ratio branch condition (Lambrechts-Johansen class), the resonant-chain-breaking ensembles (Izidoro et al.), and the wind-driven disk-evolution closure for gate G.
