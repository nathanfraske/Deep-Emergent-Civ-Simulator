...
SETTLED FOR THE REPOSITORY 2026-07-18: our earlier sentence welded two row-specific length definitions together and has been withdrawn. The paper was vendored under the fetch discipline (`docs/working/VENDORING_CHECKLIST.md`, artifact `flexure_tafi`) and its Table 1 was re-read on the held bytes: the point-load row's flexural parameter, rendered at 900 dpi, prints `[D/((rho_m - rho_i) + E Te/R^2)]^(1/4)` with the numerator `D` alone, against `[4D/((rho_m - rho_i) g)]^(1/4)` in both line-load rows. The four rows are cited row by row, the line-load and sinusoidal rows to Turcotte and Schubert (2002) and the point-load row to Brotchie and Silvester (1969), so the one symbol `alpha` is bound to four definitions by row and equation (6) never supplied the line-load value for every row. The authors' toolbox computes `alpha=(D/((E*Te/R^2)+gamma))^0.25` for the point-load branch and `alpha = (4*(D)/(gamma))^0.25` for the rest after setting `gamma = gamma*g`. The printed point-load cell and the software expose a remaining dimensional question. It is not load-bearing here, no external correctness conclusion or contact is authorized, and the neutral review is registered in `sources/external_claims.toml`.
