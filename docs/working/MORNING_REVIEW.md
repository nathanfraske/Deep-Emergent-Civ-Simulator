...
## 2026-07-16 (seam 2 BUILT): the iron dark-crust optics, ferric reads dark-red, mixed-valence near-black

Seam 2 is landed and gated (fmt, clippy on materials/physics/viewer, constructor + floor-provenance gates, both byte pins held verbatim: default 40fe8a72, living be94e310). A ferric iron oxide (hematite, Fe3+) now reads dark-red and a mixed-valence oxide (magnetite, Fe2+/Fe3+) near-black in the viewer, while a ferrous oxide (wustite, FeO) and an iron-free silicate (enstatite) correctly stay light. The mechanism: the iron oxidation state is DERIVED by charge balance over the phase's own composition (`crystal_field::iron_valence_state`, the same periodic-valence primitive the correlation classifier uses), a ferric phase carries the `O2- -> Fe3+` charge-transfer edge, a mixed-valence phase adds the `Fe2+ -> Fe3+` intervalence band, the band widths are the Marcus-Hush vibronic width (derived from `k_B T` and the band energy, reserving nothing), and the viewer projects the resulting absorption spectrum to colour one-way. The colour is authored nowhere in the canon (the byte pins prove it). This closes the "warm-white Fe-dark-crust limit" the visible-finish note named. Three things need your eyes:

1. THE JORGENSEN DERIVATION IS FLAGGED, not built (the ideal route, blocked on un-openable primary tables). The preferred route was to DERIVE the charge-transfer energy from the banked ligand-field machinery via Jorgensen's optical-electronegativity relation `E_CT[cm^-1] = 30000 * (chi_opt(ligand) - chi_opt(metal))`. The 30 kK scale constant IS primary-citable (Jorgensen, Mol. Phys. 6, 43 (1963); Prog. Inorg. Chem. 12, 101 (1970)), but the per-species optical electronegativities `chi_opt(O2-)`, `chi_opt(Fe3+)`, `chi_opt(Fe2+)` are in Jorgensen's 1969-1971 books and Lever's 1984 monograph, none web-open (RSC/Springer returned 403). A research subagent could not secure a primary cite for any of the three, so I did NOT fabricate them. Instead the band energies are supplied as DIRECT single-crystal optical measurements (a per-chromophore cited [M] column in `crystal_field.toml`, the same idiom as the monoxide `Delta_o` already banked): hematite's O2p->Fe3d LMCT 3.1 eV (Ellis et al., PRB 106, 075101 (2022)) and magnetite's Fe2+->Fe3+ IVCT 0.6 eV (Park, Ishikawa & Tokura, PRB 58, 3717 (1998)). The general Jorgensen derivation (so an ARBITRARY ferric phase gets a class-grade CT energy without a per-composition measurement) is ready to consume the chi column the moment those three values are read from the primary tables. To fetch: Jorgensen 1970 Prog. Inorg. Chem. 12, 101 (the "optical electronegativities of ligands other than halides" table) and Lever 1984 (2nd ed.).

2. RESERVED: the charge-transfer INTENSITY WEIGHT (`CHARGE_TRANSFER_INTENSITY_WEIGHT = 3.0`, in `crates/viewer/src/render.rs`, the non-canon projection, byte-neutral). The optics substrate carries each band's ENERGY (measured, cited) and its broad WIDTH (Marcus-Hush, derived), but NOT a per-feature oscillator STRENGTH, so the intensity that makes an allowed charge-transfer band's tail opaque in the visible (the Laporte-rule intensity gap between an allowed CT and a forbidden d-d) is applied as a class-grade weight in the viewer. Basis for your calibration: the allowed-to-forbidden oscillator-strength ratio, class-grade, bounded below where a ferric oxide would stay light and above where hematite would over-darken to black; 3.0 is the working value that reads hematite dark-red and magnetite near-black. The canon fix is a per-feature oscillator-strength column in the optics substrate (the named follow-on); until then this is a non-canon presentation weight, sibling to `MATERIAL_BANDS_NM` and the relief palette, with zero canon effect.

3. THE fO2 DEPENDENCY (the quantitative accuracy, not the mechanism). The mechanism reads whichever iron phase the condensation/fO2 ladder produced and colours it correctly by valence, so it is sound regardless. But WHICH iron phase the crust contains (metallic Fe vs FeO vs Fe2O3 vs Fe3O4) is set by the oxygen fugacity, which depends on the oxide-thermochemistry data you are verifying (the seam-6-review item 1 magnetite Fe3O4 JANAF-vs-Robie 4.7 kJ/mol split is exactly the redox anchor). So the colour is quantitatively right for the crust IF the condensation produced the right iron phase; that upstream is your verification, not this seam's.

Aside (a real fix found in passing): the viewer's `composition_counts` scaled a stoichiometry to its smallest amount and rounded independently, so `Fe3O4` (3:4) collapsed to `{Fe:1, O:1}` (FeO), which would misread magnetite as ferrous. It now reduces a whole-number composition by its GCD (so `Fe3O4` stays `{Fe:3, O:4}`, distinct from `Fe2O3` and `FeO`), with the fractional-crust path unchanged. Byte-neutral (viewer-only, dormant on the run path; pins held).
...
2. The living-world overlay globe (build_globe_fixture) versus the derived capstone globe. Seam 3 retired the mass/density fixtures in the CAPSTONE path (build_derived_scene, the `--derived` viewer): a pure star+orbit now derives the honest Mars-class planet (~0.08 M_earth, 2964 km, 4.58 g/cm^3, orbit-dependent crust). But the SECONDARY globe that floats above the living-world (Mirror) overview, build_globe_fixture, still passes 1-Earth-mass and 5.514 g/cm^3 to derive_planet. This is defensible (the Mirror is deliberately Earth-calibrated, so Earth's measured mass/density is the right input there, not a lazy capstone fixture), so I did NOT unilaterally change it. Your call: keep the Mirror globe Earth-calibrated (measured, correct for an Earth-mirror), or have it consume the new Mars-class derivation so both globes match the "a pure derived Earth is Mars-shaped" directive. Its doc comment is now slightly stale ("fixtures until the accretion and condensation arcs wire through" when the sibling function wired them through); I will fix the comment to match whichever you choose. Not a gate: the seam chain does not depend on it.

Resolved since the last review (no ruling needed, noted for your tracking): the "crust does not respond to orbit yet" limit from the visible-finish note below is CLOSED by seam 3 (the condensation now reads the derived, orbit-dependent formation-era midplane T; a close orbit condenses spinel, a far orbit enstatite). The Fe-dark-crust limit (item 1 below) is seam 2, queued and gated on your oxide-data verification above.
...
- THE VISIBLE FINISH IS LIVE (--derived [star] [orbit], cherry-picked 6706e6d, byte-neutral, 19 viewer tests): star mass + orbit alone -> the derived planet on screen, no old sim, no life. Run `cargo run --release -p civsim-viewer -- --derived 1 1` (needs a display for the window; the readout prints headless). THREE honest limits the agent reported rather than forced, all named follow-ons, none faked: (1) the derived crust reads WARM-WHITE not dark-basalt because the only banked Fe visible-darkening feature (the d-d line) sits in the near-IR; basalt's real darkening (charge transfer, Fe-Ti opacity) is an optics follow-on; (2) the crust does not respond to orbit yet (condensation reads a labelled ~1400 K formation-era T, the disk-thermal-history is the follow-on); (3) the crust is uniform (fresh-planet honest; lateral topography = the geodynamics follow-on). The agent pinned the Fe-dark limit with a test rather than tuning it away, which is the discipline working.

- SEAM 6 (the partial-melt crust mechanism) WIRED, byte-neutral (pins held; the surface chain is off the run path). The crust/mantle split within the floating silicate fraction is now the banked Schroeder-van Laar melt rung (`crates/physics/src/melting.rs`), not the pure-buoyancy proxy: the crust is the eutectic FIRST MELT (`eutectic_liquid_composition`, enriched in the fusible phases, depleted in olivine), the mantle the refractory residue, and the crustal thickness the McKenzie-Bickle decompression column (`adiabatic_melt_column`). The banked rung was consumed unchanged (no parallel one built), per your ruling. Two flag classes need your eyes:

  1. RESERVED McKenzie-Bickle interior inputs (4, each reserved-with-basis + cited, defined as named consts in the viewer, `crates/viewer/src/main.rs`), the values the surface chain cannot itself derive: the mantle POTENTIAL TEMPERATURE (1588 K, McKenzie-Bickle normal-MORB; per-world and per-epoch, a hotter Hadean/Archean mantle melts a more refractory set); the ADIABAT slope (15.5 K/GPa; a flagged derive-down from the mantle assemblage's alpha, rho, Cp once those land in petrology); the melting PRODUCTIVITY (0.12/GPa; a flagged derive-down from the entropy of fusion and Cp once the full McKenzie 1984 productivity lands); the surface GRAVITY (9.8; a flagged derive-down from the planet's g = GM/R^2, reserved only because the surface chain precedes the planet-gravity derivation in the scene ordering, the 9.80665 convention is NOT a canon anchor, and it is dormant while the viewer set is sub-solidus). The SOLIDUS (surface value and slope) and the SOURCE DENSITY are DERIVED, not reserved (the solidus from the endmember signatures inside the rung, the source density from the floating source assemblage), which is stronger than the task's framing (it listed solidus slope and source density as reserved). For the current refractory solar-condensed floating set (a ~1680 K ideal solidus) 1588 K is sub-solidus, so the viewer scene falls back to buoyancy (byte-identical to pre-seam-6); the melt engages on a fertile or hotter mantle (exercised in the differentiation-module tests).

  2. CITED endmember melting data (`crates/physics/data/melting_endmembers.toml`, `[[endmember]]` block kind, header-cited, out of floor-[[element]] policing per the citation-file-immutability doctrine). A research subagent fetched every value to its PRIMARY source (per your cited-column-fetch-delegation directive); the file is reconciled to those citations. What needs your eyes:
     - THREE GENUINE LITERATURE SPLITS, recorded not averaged: forsterite dH_fus scatters 89-142 kJ/mol (I took Richet et al. 1993 DTA 142000 as primary; the melt rung's own fixture uses the calorimetric 114000); corundum dH_fus splits 111 (JANAF/Chase 1998, taken as primary) vs 120 kJ/mol (Ushakov & Navrotsky levitation calorimetry); anorthite dH_fus 135600 (Weill et al. 1980) vs 133000 (Richet & Bottinga 1984). Your call which compilation each keys off.
     - A DIOPSIDE FUSION-VOLUME DISCREPANCY WITH THE BANKED RUNG (an input-audit catch): the cited primary (Rigden, Ahrens & Stolper 1989, JGR 94, Table 5, a direct V_liquid - V_crystal measurement) gives dV_fus = 13.2 cm^3/mol, a ~19 percent fusion volume correct for a chain silicate that depolymerizes on melting, where the melt rung's OWN test fixture (`crates/physics/src/melting.rs`) uses ~5.2 cm^3/mol and claims a measured Clapeyron slope of 60-75 K/GPa. The 13.2 value gives ~150 K/GPa, closer to the measured ~130 K/GPa diopside melting curve, so the rung fixture's 5.2 appears to be an underestimate. I used the cited 13.2 in the data file and flagged this; I did NOT alter the banked rung (you told me to consume it, not fix it), but its diopside fixture warrants your look.
     - FOUR HONESTLY UNSOURCED FUSION VOLUMES, zeroed (the pressure-insensitive surface-only rung, `fusion_volume_sourced = false`), never fabricated: anorthite, fayalite, spinel, corundum (no primary reports dV_fus directly). The three sourced ones are forsterite (3.84, Ghiorso 2004), enstatite (4.81/MgSiO3, Ghiorso 2004, HALVED from his Mg2Si2O6 basis), diopside (13.2, Rigden et al. 1989).
     - Enstatite and fayalite are labelled `congruent = false` (incongruent/peritectic melters; their dH_fus/dV_fus are metastable-congruent reference quantities, never a fabricated congruent value). The loader gates every row: citation-required, physical-validity (positive T_m, non-negative dH_fus), and a forsterite Clapeyron-slope coherence cross-check (~58 K/GPa, catching a corrupt T_m/dH/dV triple by its physical consequence).
...
## 2026-07-14 (cont.): owner ratified + ÆSOPUS redirect; alpha column built, two fetch findings

Owner resolved the Ferguson owner-call by REDIRECT: fetch ÆSOPUS, not Ferguson (see [[aesopus-opacity-fetch]]). Executed + captured:
- ÆSOPUS 2.0 Zenodo DOI 10.5281/zenodo.8221362 downloaded + md5-VERIFIED: `optical_constants.tgz` = 45 dust species n,k, each header-cited to primary (SiC=Laor&Draine93, waterice=Warren&Brandt08, silicates=Jager/Fabian, Fe=Palik, am-C=Jager98). These are Rule 1's measured membership AND the phonon estimator's calibration set (owner: all 5 battery rows run against in-repo data, no new fetch). The `.tab` grain tables (grain-COUPLED, held) use R=rho/T6^3 on (logT,logR), MATCHING molecular_opacity.rs exactly.
- INPUT-AUDIT CATCH (owner ratified as his standing inventory-claims-are-spec-tier rule applying to external repos): the gas-only molecular grid is NOT in the 2.0 DOI (grain-coupled); gas-only = ÆSOPUS 1.0, the web CGI. SERVICE SCOPED: current form is ÆSOPUS 2.1 at `/cgi-bin/aesopus`; the gas-only 1.0 is at `/cgi-bin/aesopus_1.0` (confirmed "Marigo & Aringer", lgtmin=3.2, gas-only). Params: solmix (7=AGSS09), zeta_ref (Z), xhmin (X), the C/O ladder via fco1/fc1/fn1, T/R grid via lgtmin/lgtmax/dlgt1/lgtchange/dlgt2/lgrmin/lgrmax/dlgr. POST multipart/form-data, async (wait for output file). Owner's ~10-composition set: AGSS09 X0.7 Z0.0165 (the Fig-7 gate), protosolar preset, a Z ladder at X0.70 {0.1,0.3,1,2}xsolar, one metal-poor X~0.75, and THE PAYOFF C/O ladder {0.55,0.8,1.0,1.2} at solar Z. Provenance protocol: query + service version banner/date from the returned header + OUR md5 + vendored immutable copy. Owner caution: verify the 1.0 output header format, expect a light adapter (do not assume it matches the 2.0 .tab).

PHONON SUBSTRATE (owner shrank it): the existing lorentzian_response (materials/optics.rs:206, eV lineshape) is the right function; two input columns feed it. BUILT: the ALPHA COLUMN (`polarizability.rs`, byte-neutral, 5 tests) = single-oscillator alpha = N (E_h/IE)^2 [a0^3] from the banked IE column + main_group_valence, E_h=m_e e^3/(4 eps0^2 h^2) DERIVED from the register (27.211 eV), hydrogen unit test 4.0 a0^3 = the exact ~11%-low bound the owner pinned. GROUND-BEFORE-BUILD CATCH on the FORCE-CONSTANT column: the "banked Pyykko lengths" are banked-in-SPEC not banked-in-repo (only Shannon ionic + Clementi-Raimondi d-state radii exist, neither is a covalent single-bond radius); so the force-constant column needs Pyykko & Atsumi 2009 covalent radii as a cited [M] column (fetch, like the optical constants) + Badger constants + a handful of Huber-Herzberg/NIST diatomic anchors. Below-sublimation books as declared reduction (clamp + grain-dominance guard kappa_grain/kappa_gas>1e2, Freedman 2008/2014 the named on-demand grain-free extension if tripped). Rule 3 de-stressed (grain size dist marginal for kappa_R). Crate boundary ratified (physics=memoryless primitives, disposer->grain wire up-stack in materials/sim).

STATE: 8 commits, all byte-neutral (pins 40fe8a72/be94e310/d05a6488 held), 373 physics tests, pushed to `claude/kappa-r-assembly` (#191). Next actions (all fetches/vendoring, owner ratified proceed): (1) ÆSOPUS 1.0 async fetch of the ~10 compositions w/ provenance protocol + adapter -> the molecular grid loader + convergence + global-positivity rows; (2) vendor the 45 optical constants (calibration set + Rule 1 membership); (3) Pyykko covalent-radii [M] fetch -> the force-constant column -> the omega_TO/LST/Szigeti/gamma generator + 5 battery rows; (4) the disposer->grain wire (materials/sim).
