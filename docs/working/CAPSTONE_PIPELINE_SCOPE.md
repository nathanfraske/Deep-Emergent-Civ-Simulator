...
- **Then deepen each layer behind the spine, in dependency order:** (1) the stellar `T_eff` solve so the star's output fully derives from its mass; (2) the disk thermal + surface-density structure; (3) the disk-condensation disposer (the gas-phase extension, section 7) so the composition-by-orbit derives; (4) the accretion arc so the planet's mass, composition, and radius derive and the stand-in fixture retires (this closes the Hadean bulk and gravity targets); (5) the full geodynamics field (interior convection and differentiation, beyond surface isostasy alone, closing the Hadean interior target); (6) the atmosphere composition arc so the gas mix, albedo, and greenhouse emerge rather than being authored scalars (closing the Hadean atmosphere target).
