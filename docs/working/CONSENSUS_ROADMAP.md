...
- 2026-07-19  Mountains / mid-band (increment 3) -> the thermoelastic ladder is COMPLETE and WIRED: rung 3 (`crates/materials/src/mie_gruneisen_debye.rs`, BM3 + Debye thermal pressure, spinodal-searched bracket) answers at depth, and `derive_column_thermal_properties` now returns all SEVEN fields for a mantle column from ONE state (density 3354.5, k 2.461, c_p 1241.0, alpha 25.9 ppm/K, ln eta 54.66 at 1600 K / 11.17 GPa). `ln_convection_onset` lands the log-domain Rayleigh + latch, twinned against the linear form, byte-neutral. RETRACTED: my "theta_D derives, so rung 3 needs one fetch" claim was WRONG (elastic vs entropy-fit effective Debye temperature, 0.83-1.22 across the phases; forsterite, my only spot-check, sits on the crossover). Both are now carried and separated BY TYPE. NEXT: retire the fixture cluster in `province_column_params` (density=1, k=2, kappa=0.01 vs derived 0.2, c_p=10, eta=1), switch `convection_step` to the log forms, retire the ra_max/v_max representability clamps, re-baseline via `realization_digest`, move BOTH pins ONCE with a ledger entry, then Seams C/D. Standing: the SI-lift retraction (I tested SI-LINEAR, not the ruled SI/LOG-SPACE); the ruled plan stands.
