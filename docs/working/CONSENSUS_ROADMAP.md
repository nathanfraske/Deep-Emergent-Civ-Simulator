...
- 2026-07-19  Mountains / mid-band (increment 3) -> #205 merged; the thermoelastic ladder is `crates/materials/src/thermoelastic.rs` and RUNG 3 IS BUILT as `crates/materials/src/mie_gruneisen_debye.rs` (BM3 cold isotherm + Debye thermal pressure; forsterite reproduces V_0/K_0 at the reference state and ~40 ppm/K at 1000 K). All six anchors banked: K' was banked-not-loaded, theta_0 + q are `thermoelastic_anchors.toml`. RETRACTED: my "theta_D derives, so rung 3 needs one fetch" claim was WRONG (fetch-agent-caught). The derivation yields the ELASTIC Debye temperature; MGD consumes the entropy-fit EFFECTIVE one, and they run 0.83-1.22 across the phases. Forsterite, my only spot-check, sits on the crossover. Both are now carried and separated BY TYPE (no conversion), and theta_0 needed no fetch: it was already in the held manifest extracts. Next: wire rung 3 into `derive_column_thermal_properties`, thread the census through `build_deep_time_provinces`, move the 7 fields + their 3 uncompared copies, re-baseline via `realization_digest`, then Seams C/D. Also standing: the SI-lift retraction (I tested SI-LINEAR, not the ruled SI/LOG-SPACE); the ruled plan stands.
