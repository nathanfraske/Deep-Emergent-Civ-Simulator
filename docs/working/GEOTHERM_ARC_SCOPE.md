...
Status: historical planetary design record from the 2026-07-16 geotherm arc. It is retained for its mechanism corrections, source pointers, and failure findings, not as canonical authority. Current admission rules live in `CLAUDE.md`, and the surviving evidence work lives in `ABIOTIC_EVIDENCE_DEBT.md`. No ruling, citation, reserved field, class row, or value in this document may supply a canonical magnitude.

## The defect chain, recorded

The claim "the derived elastic lid thickness out of the thermal state" originated in the owner's coarseness ruling, propagated into `FLEXURE_ARC_SCOPE.md` and the now-retired `parked/docs/working/CONSOLIDATED_SURFACE_PIPELINE.md` through the coordinator's transcription, and is carried verbatim at `crates/physics/src/flexure.rs:312`. The codebase never had a derived elastic lid. `ColumnState` is `{temperature, convecting}`, one lumped scalar per column; nothing carries temperature on a depth axis. The render agent held rather than ship either authoring shortcut, and surfaced the cross-lane false comment rather than quietly fix it. Both were correct.
...
**Conditioning line.** Thermal conductivity has retained candidate material rows, but presence in that bank is not admission. The convicting body is an ice shell: ice conductivity is strongly temperature-dependent where the older scope treated rock as weakly dependent, so any future row must carry its material and thermodynamic domain. A canonical adapter derives from admitted phase properties, completes admission for one residual, or refuses. It never selects a Europa or rock identity as a causal shortcut.
...
2. **The pressure-dependent brittle branch: BYERLEE'S LAW.** The built `derived_crust_yield_pa` is independent of both temperature and pressure.
   ...
   **AUTHORITY CORRECTION 2026-07-20.** `rock_friction_law` is retained candidate physics, not an admitted canonical row. Reading Byerlee 1978 closed a custody gap but did not admit its 0.85 low-stress branch or its `50 + 0.6 sigma_n` high-stress branch. The old `low_stress_band` reserved field and the instruction that an owner set its scatter are prohibited canonical inputs. The live evidence gap is sharper: McNutt and Menard use an 80 MPa intercept while Byerlee 1978 gives 50 MPa, Byerlee 1968 remains unread, and low-stress roughness matters most for thin and low-gravity lids. Normal stress through `rho g z` is the conditioning axis. If no derived or fully admitted branch covers the local state, the lid-strength evaluation refuses.
   ...
   **ICE EVIDENCE REMAINS UNADMITTED.** The Beeman relations were recovered from summary material rather than the paper's full text, and their stated pressure domains leave a gap. The later literature suggests homologous temperature `T/T_m` as a useful conditioning variable, but that hypothesis does not admit the listed coefficients or an ammonia melting point. An ice-lid branch must derive from admitted phase state, complete its own receipt, or refuse.

3. **The silicate creep route.** The historical `g R T_m` prescription is superseded and cannot be revived as a class constant. The retained Form-B freezer route, `E* = f E_coh`, still depends on a caller-supplied vacancy fraction and on unadmitted cohesive-energy evidence. Hirth-Kohlstedt olivine rows are pressure-conditioned candidates, not anchors. A canonical creep adapter must derive the barrier and pressure response from admitted phase state, complete admission for any unique residual, or refuse for the named phase and domain. Element symbols, bonding classes, and familiar olivine identity cannot choose a magnitude.
...
**ONE NAME HELD TWO QUANTITIES, AND BOTH CONSTRUCTIONS SURVIVE IN THE RETAINED SUBSTRATE.** This document said the crossing was `T_e`; the roadmap identified the McNutt moment-equivalence route; the fetch (`GEOTHERM_FETCHES.md` section 2.5) said "Te is not a depth to a boundary ... a geometric analogue of the lithosphere's integrated strength". The reconciliation is a rename, not an admission:
...
- THE CLASS VARIABLE is ATOMS PER PRIMITIVE CELL, which sits in Slack's own signature. ~~and was already banked~~ **RETRACTED 2026-07-18, measured: it is NOT banked.** See "The two retracted premises" below.
...
WHY THE COLLISION PAID. ~~The geotherm's minerals have measured anchors, so the top rung serves the front lane~~ **RETRACTED 2026-07-18, measured: they do not; see below.** The top rung would serve the front lane once they do, and the "Hofmeister replaces Slack" outcome emerges without its defect. But the carbide slice is coming, and exotic condensates will have no `kappa_298` rows at all, so SLACK'S RUNG IS THE ONLY LEGAL CONDUCTIVITY PATH AN ALIEN PHASE WILL EVER HAVE. The ladder built this week is the machinery carbon worlds require next month; Hofmeister bolted beside Slack would have served Earth minerals and stranded every alien one.

ONE FINDING SURFACED BY THE BUILD: the cell-count boundary is UNDERDETERMINED in `2 < n < 6`. The cited set places `n = 2` inside Slack's band and `n = 6` outside it, and says nothing between. `lattice_exponent_for_cell` REFUSES in that gap rather than picking a number the cited evidence does not support, because a boundary chosen there would author a scalar invisibly, inside a classifier.

## The two retracted premises, and the one column that holds the cluster

Recorded 2026-07-18, from an attempt to land the fixture-cluster replacement (the `ColumnParams` thermal cluster tagged on `crates/sim/src/geodynamics.rs`). The attempt HELD rather than landed, and both byte pins stand unmoved at `40fe8a72...` and `be94e310...`. The finding is a DATA gap, never a machinery one: the ladder is built and correct, and nothing feeds it.

RETRACTION ONE. "The class variable was already banked" is false. `atoms_per_primitive_cell` appears in this repo as a function parameter (`lattice_exponent_for_cell`, `lattice_thermal_conductivity_w_per_m_k`), a struct field (`PhaseConductivity`), and a refusal-variant field. It appears in NO data file, for no phase, in any format. Slack's own docstring marks it `(DATA)`. The only construction of a `PhaseConductivity` in the tree writes the count as a literal inside a test fixture, and `assemblage_conductivity` has no production caller at all. A quantity in a signature is a quantity the machinery is READY for; a quantity in a data file is a quantity the machinery can EVALUATE. The scoping conflated the two, which is the premise line applied to a column rather than to a function: presence in a signature was read as presence in the bank.

RETRACTION TWO. "The geotherm's minerals have measured anchors" is false. No per-phase `kappa_298` column exists in any data file either. The measured H-chondrite conductivities that convicted the estimator during the ladder's own design (olivine 4.349 and its five siblings, Henke, Gail and Trieloff 2016 Table 3) live as hardcoded literals in a test fixture, under phase names disjoint from the registry's. So the top rung is blocked on its anchor and the estimator rung is blocked on its class variable, and BOTH rungs of a two-rung ladder are unevaluable for every phase the engine holds.

WHY ONE COLUMN HOLDS THREE FIELDS. `k` gates `kappa = k / (rho * c_p)`, and `kappa` is an INPUT to the `convective_viscosity` solve (`ViscosityInputs::thermal_diffusivity_m2_s`), so conductivity, diffusivity, and viscosity fall together. The other four cluster fields were measured and DO derive: density 3223.2 kg/m^3, specific heat 1241.1 J/(kg*K), volumetric expansivity 40.1 ppm/K at the cited measured `gamma = 1.29 +/- 0.05`, and the parcel radius from the cell scale the struct already carries. Landing those four alone would widen the tagged `kappa` conflict from 20x to roughly 20000x and would move the pins twice for one truth, which the ruling forbids. The cluster is atomic.

THE SENSITIVITY, so the gap is priced rather than asserted. Slack reads `n^(-2/3)`. Forsterite has 7 atoms per formula unit and 28 per primitive cell (Pbnm, `Z = 4`), and the estimator at 1600 K spans 15.15 to 6.01 W/(m*K) across that range. Both ends sit well above the measured silicate value near 3, which is Slack's declared one-sided overstatement on a complex cell behaving exactly as its docstring says it will. Substituting the formula-unit count for the cell count would be a quantity substitution wearing a derivation's clothes, and it is worse than mis-valuing: four of the registry's eight phases (quartz 3, corundum 5, hematite 5, enstatite 5) would land inside the `2 < n < 6` band the cited evidence does not place and REFUSE outright, while their true counts (9, 10, 10, 80) are all comfortably complex-class. The shortcut breaks the mechanism for half the registry rather than degrading it.

THE FETCH THAT UNBLOCKS IT, added to the list below: a cited per-phase `atoms_per_primitive_cell` column (a crystallographic count from the space group and `Z`) for the registry's eight phases, and a cited per-phase `kappa_298` column with its band. The second is what makes the front lane MEASURED-grade; without it the estimator rung would ship a 2x-to-5x-high `k` into the run path with no second rung to compare against, which is the overlap-sentinel case the ladder exists to make impossible.

A SEPARATE FINDING, on the gate rather than the physics. The two `run_world` byte pins do not exercise this cluster: `crates/sim/examples/run_world.rs` names no deeptime, geodynamics, province, or convection symbol. The province path lives in the viewer and is covered by the viewer's own tests. A re-pin gated on those two hashes alone would have proven nothing about the change it was meant to gate.

## Fetch list (this round)

- Hirth and Kohlstedt olivine flow-law rows (unadmitted, pressure-conditioned candidates).
...
- **`atoms_per_primitive_cell` per phase (BLOCKING, added 2026-07-18).** The crystallographic count, from each phase's space group and `Z`, for the eight rows of `crates/physics/data/phase_registry.toml`. Both rungs of the conductivity ladder key on it and no data file carries it. Without this column the whole `ColumnParams` thermal cluster cannot be replaced, because `k` gates `kappa` and `kappa` gates the viscosity solve.
- **`kappa_298` per phase, with its band (added 2026-07-18).** The measured room-temperature conductivity anchor the top rung needs, per registry phase. Henke, Gail and Trieloff (2016) Table 3 already sits in the tree as a test literal under disjoint phase names; the column proper is unfetched. Without it the front lane runs on the estimator rung alone, at 2x to 5x high with no second rung to compare against.

Verify-on-pull discipline: a fetched value is a target to verify against its citation at evidence load, never a digit to trust from this document and never admission by itself.
