...
This file records the primary-source fetch for the five evidence groups the historical geotherm arc named in `GEOTHERM_ARC_SCOPE.md`: the olivine creep candidates, the oceanic elastic-thickness hindcast, the Mars and Venus hindcast rows, the ice shell branch, and Hartmann saturation. It follows the format and the discipline of `PIPELINE_FETCHES.md`. For every entry the quantity, the value or functional form with its band, the units, the primary citation, the validity domain, the population that convicted the fit, and the hazards a consumer must know are given. Where a value could not be pinned to a source that was read rather than summarized, it is flagged under "Defaults taken" rather than guessed.

Verify-on-pull discipline: a value in this document is a target to verify against its citation at evidence load, never a digit to trust from this document. Nothing here is admitted. A citation, grade, literature band, or owner choice cannot supply a canonical magnitude. Each candidate must derive from the sealed floor, complete the full admission receipt in one unique residual slot, or cause a structured refusal.
...
## 1. Hirth and Kohlstedt olivine flow-law rows (unadmitted candidates)
...
This table was first transcribed from the machine-readable UWGeodynamics `ViscousRheologies.json` library (`github.com/underworldcode/UWGeodynamics`, file `UWGeodynamics/ressources/ViscousRheologies.json`), whose entries cite Hirth and Kohlstedt. The primary's Tables 1 and 2 have since been read directly and checked against the reproduction in `HK2003_VERIFICATION.md`. The reproduction remains part of the custody trail, not the source authority.
...
**Stress exponent.** The primary's Table 1 prints `n = 3.5 +/- 0.3` for dislocation creep. The widely circulated `+/- 0.5` is a hypothetical extrapolation-sensitivity example in the primary's prose, not a fitted band. `HK2003_VERIFICATION.md` records the adjudication.

**The GBS rows.** The primary carries two dry GBS rows split at 1250 degrees C, with `E* = 600` and `400` kJ/mol. Both activation energies are assumed from an easy-slip dislocation mechanism rather than fitted to GBS measurements. Dixon and Durham's Figure 4 caption, not Figure 6, combines `E* = 400` from dry GBS with `r = 1.2` from wet dislocation creep, so the printed triple belongs to no single Hirth-Kohlstedt row. `HK2003_VERIFICATION.md` section 5 carries the source text and the resulting refusal obligation.

### 1.4 Validity domain (part of the row, per the ruling)

The flow-law experiments behind the compilation used gas confinement near 300 MPa. The primary's Table 2 activation-volume determinations span pressure chords from `1e-4` to 15 GPa; Dixon and Durham's `P <= 2 GPa` range is their filter over that table, not the primary's full pressure limit. Each `V*` must retain its own fitted interval. Grain sizes in the underlying dislocation-creep experiments are of order micrometres to tens of micrometres. Laboratory strain rates are of order `1e-5` s^-1, while geological rates of interest are `1e-15` to `1e-10` s^-1. The extrapolation therefore spans roughly ten orders of magnitude in strain rate. Hirth and Kohlstedt identify stress extrapolation as the dominant accuracy limit.

### 1.5 Hazards and a disagreement to report

**RESOLVED AS A SOURCE CONTRADICTION, OPEN AS A PHYSICAL VALUE.** The reproduction's 6 cm3/mol is the primary's recovery chord over `1e-4` to 10 GPa. Dixon and Durham's 13 to 27 cm3/mol is the subset of primary determinations whose intervals lie at or below 2 GPa. The sources agree once the pressure conditioning is restored. The primary prints no single dry-dislocation `V*`, so neither number may be promoted to one. The wet reproduction value 23 lies inside the primary's `22 +/- 11` band but is a different row and digit, not rounding.

**HAZARD (the composite Dixon row).** Dixon and Durham's Figure 4 caption labels the `E* = 400` kJ/mol, `V* = 14` cm3/mol, `r = 1.2` set as wet dislocation creep, while their body identifies the mechanism as GBS. The primary shows that `E* = 400` belongs to dry GBS and `r = 1.2` belongs to wet dislocation creep. The triple is an assembled conversion set, not a measured or fitted row, and cannot enter an exponent as `[M]`.

**HAZARD (`V*` does not extrapolate).** Dixon and Durham identify `V*` as the weakest-constrained parameter. Their own measurement is `15 +/- 5` cm3/mol for dry olivine over 2 to 9 GPa. Because `V*` sits in an exponential multiplied by `P`, its band dominates the deep extrapolation. Outside every retained pressure chord, the candidate has no supported value and canonical evaluation must derive a new relation, complete admission, or refuse.
...
**Confidence.** High on every primary Table 1 and Table 2 cell after the dual-channel verification in `HK2003_VERIFICATION.md`. High that the apparent dry-`V*` source conflict dissolves when each pressure interval is restored. High that no single dry-dislocation activation volume is printed. The candidate's admission state remains unchanged.
...
600 K is 327 °C, which sits BELOW the bottom of every band above except the trench row. The arc's "~600 K" and the literature's "~600 °C" are not the same quantity, and they differ by 273 K in a place where the historical scope put this number inside a yield-strength-envelope construction. This is retained as an evidence finding, not a choice to resolve. The authored `~600 K` retires, and no owner correction can admit either reading. The coincidence that the trench-derived isotherm (342-349 °C, or 615-622 K) has a KELVIN value near 600 is a numerical accident of two different conventions and must not be used to reconcile the two readings.
...
**Validity domain.** 77 to 115 K. This is a COLD-ice fit: it is Ganymede-and-outward temperatures, not the warm ice near a Europa-class shell's base. The abstract's claim of `T`-independence is asserted over that 38 K window only, and must not be extrapolated to ice near its melting point, where premelting and the whole ice-friction-at-high-homologous-temperature literature take over.

**HAZARD (a gap in the fit).** The two branches are reported for `P >= 10` MPa and `P <= 5` MPa. The interval `5 < P < 10` MPa is covered by neither expression. The two lines cross where `0.20 sigma_n + 8.3 = 0.55 sigma_n + 1.0`, that is at `sigma_n = 20.9` MPa, which is OUTSIDE both stated domains. The branches cannot be joined at their intersection without leaving the fitted domain, and a naive `min` or `max` will produce a discontinuity or an unphysical kink somewhere in `5` to `10` MPa. Canonical evaluation in that interval must derive a relation, complete admission for one unique residual, or refuse.

### 4.3 Byerlee's law, the rock comparison the ice deviates from (PRIMARY, read)
...
Gault's "1 to 10% of geometric saturation" is the historical empirical answer to "what fraction". Minton et al. read it as "an empirical estimate of the line of `n_eq,r = 0.021 +/- 0.017` and `beta = 2`", so the `+/- 0.017` is Gault's 1-to-10% band re-expressed as a coefficient, not an independent uncertainty.
...
Nothing in this document was defaulted, guessed, or interpolated. What follows is the original unresolved-source list, reconciled against later verification. Blocker 1's source-access failure is resolved, while its pressure-conditioned physical spread remains open. Blockers 2 and 3 remain source-custody failures. None of these records grants admission.

**BLOCKER 1, SOURCE ACCESS RESOLVED; VALUE STILL UNADMITTED.** `HK2003_VERIFICATION.md` records a held primary copy, checksum, and dual-channel reads of Tables 1 and 2. The former 6 versus 13 to 27 cm3/mol conflict is two pressure-conditioned selections from the same table, not a disagreement between sources. The primary offers nine determinations over distinct chords and no single dry-dislocation value. Preserve every interval and refuse unsupported pressure use.
...
**11. The unit finding in section 2.4 is surfaced, not resolved.** The historical scope's `~600 K` does not match any isotherm the literature states, all of which are in degrees Celsius and none of which is 327 degrees C. The authored number must retire; no owner correction can admit either unit reading. A canonical comparison requires a derived quantity or a fully admitted diagnostic candidate in its explicit source frame, otherwise it refuses.
