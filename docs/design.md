...
> Needs research, item R-UNITS-PIN in the research backlog. ADVANCED as a partial advance, not closed (record 62.25, audit block 1aa): the number-system core is built, the flag stays open for the conversion-exactness and ordered-store residual below. What is built: the `units` crate (Part 59) carries the per-quantity scale registry, the single canonical absolute storage (`AbsoluteQuantity`), the explicit per-quantity saturate-or-wrap policy, and a checked round-trip rescale primitive; the closed CODATA fundamentals table and an exact float-free composite compute (a deterministic bignum and rational evaluator, pi by a Machin series, cross-checked against a stored reference) derive a composite constant such as the Stefan-Boltzmann sigma; a two-tier number system, the canonical Q32.32 `Fixed` unchanged plus a scale-closed integer tier over an eight-limb `i256` accumulator rounded once per law-chain, with a load-time typed-op-graph (`LawExpr`) scale planner that sizes each node's width and scale from its measured exponent interval; a type-level divide-and-log floor invariant, the physical-floor-or-limit-at-zero backstop, populated on the run-path divisor and log sites so a near-zero divisor or log argument is bounded by a declared physical floor rather than the storage epsilon (Principle 10, observer independence); and a consumer-lift survey that found the precision-lift set empty beyond the one flagship sigma lift, which measured byte-neutral. What remains open: the emic-unit conversion is still a hash map with one multiply-and-divide, so an emic to absolute to emic round trip stays lossy by up to one epsilon for a non-power-of-two factor, and the sorted unit store for an ordered canonical walk is unbuilt; the coupling references in Parts 41, 25, and the physics domains correctly read around this open residual. The mechanism is fixed Rust; the scales, the declared physical floors, and the cultures' units are data. Couples to Parts 3, 9, 40. Flagged, advanced, not closed.
...
### 62.25 The R-UNITS-PIN number system: two-tier arithmetic and the floor invariant (a partial advance)
...
What remains to be proven. The reserved values are surfaced with their bases: the composite significance target and guard (the fixed-point scale family that resolves a CODATA composite's roughly ten significant figures), the per-law intermediate widths (each sized from the law's measured exponent interval), and the declared physical floors and limits-at-zero (each a per-world value below which the quantity is absent, derived from the physics where a material or law datum supplies it, surfaced as an owner value only where the physics cannot). R-UNITS-PIN stays open as a partial advance: the emic-unit conversion is still a hash map with one multiply-and-divide, so an emic to absolute to emic round trip stays lossy by up to one epsilon for a non-power-of-two factor, and the sorted unit store for an ordered canonical walk is unbuilt; the coupling references in Parts 41 and 25 and the physics domains correctly read around this open residual. The honest limits stand: software float (a per-value mantissa and exponent) is specified but not built, reserved for a quantity whose runtime range cannot be declared, which the declare-your-bounds principle prevents; and the byte-neutral flagship lift is a real precision-correctness improvement whose effect the current pinned scenarios do not surface, so the arc moved no canonical hash. A partial advance of R-UNITS-PIN, which stays open for the conversion-exactness and ordered-store residual.
