...
> Needs research, item R-GPU-CANON-PIN in the research backlog. The cross-backend bit-identity argument is sound for integer addition, but a canonical kernel also needs fixed-point multiply and divide (a sixty-four by sixty-four to one-hundred-twenty-eight intermediate and a narrowing, which WGSL, SPIR-V, Metal, and DX12 lack natively and must emulate) and the transcendentals (square root, an exponential series, and a logarithm where a distance measure needs one), and this part pins neither the emulation algorithm nor the rounding direction of each. The worldgen noise (Part 12) and the eikonal flow-field (Part 13) are also candidate canonical kernels whose status is unstated. A session should pin one canonical Q32.32 multiply and divide emulation and rounding mode, fixed iteration counts, precision, and rounding for the transcendentals, declare the noise and flow-field kernels canonical fixed-point or quarantine them as non-authoritative, and enumerate the full set of target backends and adapters the Stage 0 bit-identity gate must cover, including any software adapter the launcher can select. The discipline stays integer-only on the canonical path. Couples to Parts 3, 12, 13, 60. Flagged, not changed.
