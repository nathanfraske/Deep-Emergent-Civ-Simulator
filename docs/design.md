...
> Needs research, item R-LANG-DET in the research backlog. Several of the language procedures are order-sensitive or unbounded without a pinned rule. A regular sound change fixes that one rule sweeps the lexicon, but not the order among rules or whether a generation's rules apply at once or in sequence, which changes the result under feeding and bleeding. The concept salience is a century-accumulating fixed-point value with no overflow discipline (Part 55). A concept id formed by hashing content rather than minted through the registry has no referential identity and can collide. The shared-concept traversal in the distance metric (33.5) and the dawn dynamic's schedule trigger (33.9) are also unpinned. A session should pin the inter-rule sound-change order and the at-once-or-sequential semantics, mint or register concept ids so they resolve and do not collide, apply the Part 55 overflow discipline to salience, and fix the shared-concept traversal order and a single canonical rounding for the distance aggregation. Determinism and the data-driven substrates hold. Couples to Parts 9, 21, 55. Flagged, not changed.
