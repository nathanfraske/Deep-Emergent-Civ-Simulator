...
> Needs research, item R-RNG-COORD in the research backlog. The draw coordinate here is `(master_seed, entity, phase, counter)`, but later parts assume more: a canonical tick (Parts 9, 16, 33, 54), and a per-object or per-locus and per-ordinal sub-coordinate so that two draw sites in one entity-and-phase stream do not collide on counter zero. A session should specify a single canonical multi-coordinate keying primitive, a documented total fold over seed, entity, region, tick, phase, object or locus, ordinal, and slot; a phase-number registry so phase ids are assigned once rather than by hand; a counter-namespacing rule so distinct draw sites are separable; and a degrade rule for a missing coordinate. It must also confirm that an entity stream is keyed on a camera-independent canonical coordinate rather than an allocation-order id, so promotion order cannot move the timeline (Part 54, Principle 10). The keying stays integer and counter-based. Couples to Parts 9, 11, 16, 33, 54. Flagged, not changed.
