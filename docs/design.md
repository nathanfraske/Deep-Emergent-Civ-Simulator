...
> Needs research, item R-CONVERSE in the research backlog. The gossip loop here, the dawn naming game (33.9), and the teaching, deliberation, and negotiation systems across the design (Parts 10, 23, 34, and 41 for transmission, Part 36 for governance and trade, Part 21 for value disputes, Part 24 for terms of war) instantiate the outcome of a conversation rather than the conversational process: a transmission is a one-way speaker-to-listener pass with no modelled response, turn, or repair, who-speaks-what-to-whom is chosen by co-location rather than by intent (the decision layer of Part 8 carries no communicative action), and a coordinated result (an agreed word, a shared doctrine, a passed law, a settled price, accepted terms) is reified without the exchange that produced it, the same abstraction 33.9 makes explicit when it instantiates its outcome rather than paying for the full continuous simulation. The chosen target, signed off, is a speech-act substrate (Tier 2 of the scoping): communicative acts become first-class canonical events over a data-defined dialogue-move registry, recognised not enumerated, sibling to the access-channel registry (Part 40, Part 37), the institution-function substrate (Part 36), the trace-kind registry (9.9), the value substrate (Part 21), and the semantic substrate (33.1), so a culture's repertoire of moves grows with the world rather than being authored one level down. The mechanism is fixed Rust and the move membership is data, and the illocutionary force of each move composes from primitives the engine already has: an assertion is a told-evidence injection (9.5), a question raises an inquiry goal in the hearer (9.13), a form proposal is the naming-game coordination move (33.9), an uptake is acceptance, a promise or threat is a conditional intent the hearer models (Part 37), and a lie is an assertion judged by the sincerity frame already specified at the single-utterance update (Part 37). A conversation is an emergent query over co-referenced move events linked by participants, topic, and time, the way an artifact's saga is a query over its tagged events (Principle 5), rather than an authored dialogue script (Principle 8); speaking becomes an intent the utility AI can select (Part 8), with the open sub-question of whether a communicative intent competes with physical actions in one utility pass or runs as a parallel channel; and the listener-response loop (uptake, doubt, counter-assertion, acceptance, repair) feeds both the first-order belief layer (Part 9) and both parties' theory of mind (Part 37). The facets to research are the move substrate and its force-composition; the conversation-as-emergent-unit and its coupling to the event registry (the open R-EVENT, whether a move is a row there); the listener-response and theory-of-mind coupling, with the canonical-versus-interpretation boundary (33.2, how far the non-authoritative flavor layer may elaborate a real move sequence before it invents canon, which a richer move log makes faithful rather than confabulated); speaking-as-intent and level-of-detail gating, with full move-by-move dialogue only at the promoted and significant tier while the quiet majority keep the aggregate diffusion (9.14) and the one-pass gossip outcome here; and the deferred deeper couplings, reserved for when their layers exist and can be proven small: multi-party deliberation feeding institution decisions (Part 36), persuasion over value-distance using the opinion-dynamics models already cited for R-AXIOM (Friedkin-Johnsen and bounded-confidence), and negotiation, alliance, and terms of peace (Parts 37, 24). The constraints hold: determinism (moves are events in the single-threaded Events phase, counter-RNG keyed and id-ordered, extending the within-tick phase order pinned by R-LANG-MODALITY and R-TOM-UPDATE rather than adding a parallel one), data-driven membership (Principle 11), emergent content (Principle 8, the moves are mechanism and their content is the culture's), and the Steering Audit (Principle 9, no authored pragmatics or persuasion efficacy). To be taken next as a facet fan-out with a steering-and-determinism verify pass, the R-LANG-MODALITY shape. Couples to Parts 9, 33, 37, 8, 7, and the deferred 36, 21, 24. Flagged, not changed.
