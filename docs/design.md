...
A generated species carries its anatomy the way it carries its niche: as a point over trait axes, not as a hand-authored body. At the aggregate tier where the biosphere lives, a species' anatomy is a morphological trait vector over the fauna axes the trait-axis catalogue declares, body mass the master trait, natural weaponry and armour, tissue density and toughness, sensory acuity, encephalization, diet breadth, and the temperament axes, and it is heritable through the composition-and-trait channels of this Part so anatomy drifts and responds to selection over the pre-dawn epoch (a prey line pressured toward heavier armour, a predator toward better weaponry). This is the level-of-detail principle applied to bodies (Principle 1, Part 35): the masses are statistics that carry a trait vector, and the full per-part Body of Part 35, an assembly of layered tissues with functions and fluids that is the canonical physical condition of an individual, is instantiated only when a notable organism is promoted, built from the species' trait vector and the race's data-defined anatomy (Part 40). So a wolf and a beetle differ at the aggregate tier by their trait vectors, in mass, weaponry, armour, and senses, and at the promoted tier by the bodies those vectors build. The morphological axes and their ranges are the catalogue's data, reserved with their basis; the aggregate trait vector over them is fixed Rust. Two honest limits stand and are the deferred siblings: which build attributes of a sentient race are primitive rather than derived from the physics substrate is R-BUILD-PHYS (Part 20), and how a wound propagates through a data-defined body of differing plans is R-WOUND (Part 35), so the promoted-tier body and its injuries read around those items rather than resolving them.
