...
> Needs research, item R-BEHAVIOR-EVOLVE in the research backlog. The action and consideration repertoire above is data-defined rather than hardcoded, which satisfies Principle 11, but a repertoire authored as data is still authored: whoever writes a race's actions and curves has written its behavioural policy, so that a creature seeks water when a thirst drive is high because that action and that curve were written, not because the behaviour emerged. For non-sentient creatures and the pre-sentient substrate this is steering at the level of behaviour, which Principle 9 reserves to physics alone. A session must specify how behaviour arises the way anatomy does, from physics and selection: the being's needs as homeostatic states that are consequences of its body (energy, water, integrity, temperature, against the death floors of Part 20 and the biology floor R-PHYS-BIO), its options as the affordance primitives its morphology permits (move, ingest) rather than a named-action menu, its sensing as the channels its organs provide (the non-omniscient perception this layer already needs), and the mapping from state to primitive as a heritable controller expressed from a new genome channel (sibling to the cognition and composition channels of Part 25) that evolves under the pre-dawn epoch's selection, with fitness a consequence of homeostatic survival and reproduction rather than an authored objective or a resemblance to expected behaviour. The controller representation (a fixed-point reaction norm, graduating to a small fixed-point recurrent network) and the deep-time scoring (a homeostatic-viability proxy for quiet pools, full sampled episodes at the dawn and under significance, on the Part 54 gradient) are the open choices, with determinism (no float in canonical state, every draw keyed on the seed and the tick) and data-driven membership the binding constraints. The evolved controller underlies the sentient layers (Parts 21, 28, 37) rather than replacing them: it is what animal behaviour is and what sentient deliberation rests on. Couples to Parts 20, 25, 28, 32, 40, 54, and R-PHYS-BIO. A standalone design pass is in docs/emergent_behavior_design.md. Flagged, not changed.
