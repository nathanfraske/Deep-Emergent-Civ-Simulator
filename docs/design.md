...
> Needs research, item R-UNITS-PIN in the research backlog. This part states the rules (each quantity assigned a scale, conversions explicit, saturation versus wrap a defined per-quantity choice) but pins none of them, and the emic unit map is a hash map with a single multiply-and-divide conversion that makes an emic to absolute to emic round trip lossy by up to one epsilon for a non-power-of-two factor. A session should build the unit substrate (the `units` crate of Part 59) with a per-quantity scale registry, a single canonical storage direction (absolute), an explicit per-quantity saturate-or-wrap policy wired into the accumulation paths, a round-trip exactness rule for conversions, and a sorted unit store so any canonical walk over it is ordered. The mechanism is fixed Rust; the scales and the cultures' units are data. Couples to Parts 3, 9, 40. Flagged, not changed.
