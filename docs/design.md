...
The `MaterialProps` named-field set (density, hardness, sharpness, and the rest) is forward-flagged for the same generalization (item R-SOURCE-VECTOR): a material becomes a vector of points over the physics-substrate axis registry (Part 58, R-PHYS-BIO in Part 41), with the wave-1 mechanical-and-materials floor populating those bulk-property axes, so the closed struct is subsumed rather than extended field by field.
