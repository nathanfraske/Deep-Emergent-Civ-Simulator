...
sys.stderr.write(
    ...
    "reports clean, so do not trust a sound verdict until the smoke test has cleared the inputs. It "
    "also GENERIC-IFIES: it strips every comment, gloss, and framing line that is not what the code "
    "does or the concept core, so the panel sees neutral inputs. It FAILS CLOSED: if it cannot decide, "
    "the model errors, or its output is missing, the panel does NOT run and the construction is "
    "reworked, never read as a pass.\n"
